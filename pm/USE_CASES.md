...
所有實驗的回測分為三段（定義於 `base_config.py`）：

| 區間 | 期間 | 用途 |
|------|------|------|
| **Part A** (In-Sample) | 2019-01-01 ~ 2023-12-31 | 策略開發與參數調整用，允許過擬合 |
| **Part B** (Out-of-Sample) | 2024-01-01 ~ 2025-12-31 | 驗證策略是否泛化，評估是否納入 followup 的主要依據 |
| **Part C** (Live) | 2026-01-01 ~ 至今 | 實盤追蹤，觀察策略在真實環境的表現 |

> 判斷策略好壞主要看 **Part B**，Part A 僅供參考。A/B 之間的一致性（勝率差距 < 15pp）也是重要指標。
