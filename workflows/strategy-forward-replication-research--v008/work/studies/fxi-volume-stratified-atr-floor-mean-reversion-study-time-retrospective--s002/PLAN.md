# Plan: FXI Volume-Stratified ATR-Floor Mean-Reversion Study-Time Retrospective Evaluation

## Inputs and frozen identities

### Governance, lineage, and authority

- Study: `strategy-forward-replication-research@v008/S002` at
...
- Draft-preparation Git HEAD: `54075ef47013d5bc13c88e25ab5519a5a0b43a6c`.
...
The study remains `draft`. These files do not grant preregistration, Development, candidate-freeze,
Evaluation, Shadow, broker, or order authority. Every later authority requires its own guarded
current-time action. The completed S001 artifacts and lifecycle are read-only lineage evidence.

The 2026-08-17 freshness check reports that the FXI AI context has no freshness metadata and that
most cross-asset knowledge is based on data through 2025-12-31, more than six months old. This
study records that limitation and does not refresh data or inspect outcomes during planning.
...
| --- | --- | --- |
| `fxi-atr-floor-mean-reversion/atr-floor-candidate` | selection candidate | `f3f440ca434270b3965bbc38540ef6a45c7a46a4548be8b5d96f128c6e90b141` |
| `fxi-atr-floor-mean-reversion/pullback-wr-baseline` | distinct family baseline | `94f599c50a1c55315e4229959460a7c90c42ecbacdde5adc85084285d6f61e21` |
| `fxi-atr-floor-mean-reversion/s001-atr-band-reference` | robustness-only exact S001 reference | `5e0912e1752fd84d861c1f62b512a39af378a7503b6617288d0a3184d14c74f3` |
| `fxi-atr-floor-mean-reversion/atr-floor-1p10-robustness` | robustness only | `f999a7fcdb7713c8f2c99ebd0599c70dd452e0b14e27962b348f43b4f021b0cb` |
| `fxi-atr-floor-mean-reversion/hold-18-robustness` | robustness only | `c585995355e93b472e20eb37a07e3a735dddc3a8b718b17afa44e6bd0d16e616` |
| `fxi-atr-floor-mean-reversion/delay-one-session-robustness` | robustness only | `31b60fc2c764e46aeb195120fa662c271baefd396ab31bab2b9f80d3fe830e0d` |
...
`3b17e0e934f1dd7f696a8f36b81fb7435e36b2e77e48810e89de5d4a98c91e72`.
...
maximum drawdown is at most 20%. It is selected if and only if eligible. Volume cannot affect this
decision. Otherwise the governed no-candidate terminal-evidence path applies.
...
- `ClosePos=(close-low)/(high-low)` must be at least 0.40; a zero session range maps to 0.50.
...
- Accepted signals use a ten-completed-session cooldown from the previous accepted signal.
...
- Random-entry and deterministic missed-entry seed: `20260817`.
...
The validator accepts exactly nine challenge IDs. No volume-specific tenth ID is introduced.
Volume terciles are an applicable market-regime dimension and are frozen inside the existing
`market-regimes` method target together with the five annual folds. `parameter-perturbation`
contains the exact S001 ATR-band reference, ATR-floor 1.10, and hold-18 members; `delayed-entry`
keeps its separate method target. Every challenge has a unique content-addressed artifact and raw
values sufficient to recompute its typed gate.

## Method and stages

### 1. Draft and preregistration readiness

While draft, verify v008, policies, exact source hashes, family, calendar, stale-context warning,
known contamination, volume semantics, registries, and challenge mapping. Run provider-free tests
only. Do not refresh data, create a snapshot, run a definition, calculate Development/Evaluation
metrics, rank trials, or mutate a registry.
...
family history and the deterministic eligibility decision. Do not compute volume gates or inspect
2021-2025 output during Development.

### 3. Candidate freeze

If the sole ATR-floor candidate is eligible, prepare the guarded selection object containing
...
registry-absence evidence required for independent review.
...
Registry mutation requires separate approval and a truthful contamination declaration. The guarded
transaction establishes one current-time `retrospective_selection_checkpoint`, registers missing
outcome-free identities at real current time, freezes the family, and appends one plan.
...
The `parameter-perturbation` gate requires valid exact evidence for the S001 reference and requires
the ATR-floor 1.10 and hold-18 variants each to preserve positive base/stress return, base profit
factor above 1.1, stress profit factor above 1.0, and stress drawdown at most 20%. The S001 reference
...
bound source, policy, manifest, calendar, registry event, volume feature, challenge artifact, or
approval. Never impute volume, change tercile boundaries, query another provider, or turn the
volume challenge into an entry rule.

After preregistration, never change the hypothesis, route, calendar, family, parameters, baseline,
volume formula, costs, metrics, thresholds, seeds, challenge targets, evidence classification, or
outcome mapping. Never use 2021-2025 outcomes to tune or replace the candidate, hide trials,
produce partial ranking, use mutable `latest`, call a broker, create orders, or claim live authority.
