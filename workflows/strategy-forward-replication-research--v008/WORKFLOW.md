...
canonical tracked `results/research-evidence/<sha256>.md`；檔名與 bytes 必須相符、不得覆寫、
不得有 mutable alias，且 fresh clone/Git GC 後仍須可驗證與永久保留。
...
18. Qualification snapshot publisher 必須證明實際 registry path 等於 repository root 加上
    ...
    `results/qualification-evidence/<sha256>.json`，Development/challenge artifacts 必須位於
    `results/study-evidence/**`，且 tracked bytes 必須等於 Git index bytes。
