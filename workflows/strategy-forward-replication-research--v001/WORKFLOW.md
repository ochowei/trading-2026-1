...
本流程用來判斷：一個只根據 Development 證據選出的日線策略，是否有足夠且未受污染的
Historical 與 prospective Shadow 證據，值得成為受控啟用的候選，並在啟用後繼續接受
fail-closed 監測。

流程依序回答候選能否成為 `shadow-eligible`、`activation-eligible` 與受控 `Active`
策略。每次通過只授予進入下一階段的資格，不保證未來獲利，也不單獨授權真實或自動下單。

## Scope and non-goals

本流程涵蓋一個 experiment family 的單輪研究，從預註冊、Development 選擇、Historical
Evaluation、穩健性挑戰、prospective Shadow、受控啟用資格，到啟用後的 drift pause 與
recovery。它規範資料角色、決策權限、必要證據、晉級 gate，以及失敗後開啟下一輪研究的
邊界。
...
4. 先完成並由人類研究負責人核准下列 preregistration inputs：
   ...
   - Historical、robustness、Shadow、pause 與 recovery 的 thresholds、checkpoints 和 outcome
     rules；
...
```text
planning -> development -> candidate-frozen -> historical-evaluation
         -> shadow-eligible -> shadow -> activation-eligible -> active-monitoring
```
...
### 3. Historical Evaluation and robustness challenge

Historical Evaluation 使用至少五個 annual non-overlapping folds。每筆 trade 歸屬於其
...
### 4. Prospective Shadow

只有 persisted passing Historical Screen 才能正式註冊 Shadow。Registration 必須以當下
...
3. Warmup 只提供先前 observation；不得把 warmup session、carry-in position 或 fold 外 exit
   計入 Evaluation performance。
4. Candidate freeze 後禁止調參、換資料、改 baseline、改 costs、改 success threshold，或用
   Evaluation/Shadow outcome 重新選冠軍。
5. 所有 outcome-relevant trials、failed observations、tombstones 與 decision rationale 都要
   保留；不得以刪檔、改名或只報 winner 隱藏 selection history。
6. 不得產生 partial ranking；任何應納入 candidate 無效、stale、legacy 或 unreproducible 時，
   selection 必須停止。
7. 門檻可在 outcome 前加嚴，不可低於本 workflow floors，也不可事後放寬或人工例外通過。
8. Historical failure 終止本輪；修改後的 candidate 不得重用已查看的 Historical evidence。
9. Outcome-relevant definition change 建立新 trial；Shadow 或 Active qualification 不得跨
   fingerprint carry over。
10. Missing、corrupt、stale、conflicting 或無法重算的 evidence 一律 fail closed，不得以 mutable
    `latest` pointer、人工 assertion 或 synthetic identity 補足。
11. 本流程及 Phase 6 evidence 不得宣稱 `authorized_for_live_orders=true`，也不得呼叫 broker。
12. Append-only evidence、human approvals、timestamps、snapshot IDs、complete commit SHAs 與
...
- `pass`：所有必要 identities、approvals 與 frozen gates 都通過。它只允許進入明定的下一
  stage；Historical pass 是 `shadow-eligible`，Shadow pass 是 `activation-eligible`。
...
本輪在以下情況終止：trial budget 用盡且無 candidate、Historical 或 Shadow `fail`、策略在
...
| `docs/result-validity-and-trial-history.md` | normative | Result validity、完整 candidate set、append-only trial history 與 formal ranking evidence。 |
...
| `docs/historical-qualification-and-shadow.md` | normative | Historical plan/folds/gates、benchmarks、selection adjustment、Shadow registration 與 activation eligibility。 |
...
| `docs/strategy-forward-replication-research-workflow.md` | reference | 本 `v001` 的 document-led source；保留作 authoring provenance，不與本 contract 共同成為雙重 authority。 |
