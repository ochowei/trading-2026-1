# Hypothesis: ACWI Turn-of-Month Replication — Corrected Availability
...
Stage 3 workflow-native family，並取得 `shadow-eligible` 資格。它以 exact `revisits` 關係
取代因 availability-start mismatch 而取消的 v002/S001；唯一設計修正是把純
availability/warmup start 由 `2008-03-26` 改成 provider first row `2008-03-28`。

`pass` 只允許日後另行註冊 prospective Shadow；不建立 broker fill、不送出訂單、不授權
Active promotion。若 Development 找不到 eligible candidate，或完整 Historical/robustness
evidence 否證 claim，本輪終止且不得調參後重用 2021-2025 validation evidence。
...
- selected candidate 在 Historical Evaluation 未達五個完整 folds、20 筆 completed trades、
  三個 traded folds或 60% positive traded folds；
...
結果是 `indeterminate` 而不是支持。`insufficient-evidence` 不適用於固定的
Development/Historical stages，只可能用於 study scope 外的 prospective Shadow。
