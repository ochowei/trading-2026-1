<!-- AI_CONTEXT_START - 此區塊供 AI Agent 快速讀取，人工更新
  ...
  phase9_note_2026_08_06: XLU-013 and XLU-014 now consume declared ^MOVE/^TNX historical auxiliary bundles through the provider-free execution boundary; migration runs remain migration-pending and do not advance latest or followup qualification.
  phase9_note_2026_08_06_legacy_wave: XLU-005, XLU-006, and XLU-007 now consume declared TLT/SPY historical auxiliary bundles through the same provider-free boundary; migration runs remain migration-pending and do not advance latest or followup qualification.
  phase9_note_2026_08_07_primary: XLU-001, XLU-003, XLU-004, XLU-008, XLU-009, and XLU-010 through XLU-012 now consume declared primary XLU bundles through the shared execution boundary; existing auxiliary declarations remain unchanged. Formal migration results remain pending requalification.
  phase9_evidence_2026_08_07: This remaining XLU primary-only wave is contract-covered; no new fixed-snapshot parity artifact has been promoted, so latest results and followup qualification remain unchanged.
...
- 回檔上限微調：7%/8% + ATR 過濾（XLU-011 Att3：8% 與 7% 結果完全相同）
...
**XLU-010 結果（波動率飆升均值回歸，3 次嘗試）：**
- Att1（20日寬回檔 5-10% + WR + ClosePos）：Part A Sharpe -0.22（13 訊號），Part B -0.10（8 訊號）。20日窗口即使加寬範圍仍捕捉 2022 漸進下跌
- **Att2（XLU-003 + ATR(5)/ATR(20) > 1.2）：Part A 5/5 全勝 WR 100%，cum +13.14%，Sharpe 0.00（std=0），Part B Sharpe 1.26（3 訊號，WR 66.7%，cum +4.85%）**。ATR 比率過濾完美消除漸進式下跌假訊號，但訊號數偏少（5+3=8）
...
## XLU-010: Volatility-Spike Mean Reversion（波動率飆升均值回歸）

> 實驗代碼：`xlu_010_20d_wide_pullback`
> 策略類型：均值回歸 + 波動率飆升過濾
> 建立日期：2026-04-08

### 假說

XLU-003 Part A（Sharpe 0.06）的核心弱點是 2022 年漸進式升息下跌產生假訊號。
這些假訊號的特徵是短期波動率（ATR(5)）未顯著高於長期波動率（ATR(20)），
因為價格是緩慢下跌而非急跌。加入 ATR 比率過濾器可精確區分兩者。

### 嘗試歷程

#### Att1：20日寬回檔（失敗）

嘗試 COPX-003 的 20 日回看 + 寬回檔範圍（5-10%）。
...
20 日窗口即使加寬範圍至 5-10%，仍捕捉 2022 漸進式下跌訊號。
多筆 day-1/day-2 停損顯示進場時仍在下跌中。
COPX-003 的成功無法遷移至 XLU（礦業股急跌 vs 公用事業慢跌本質不同）。

#### Att2：ATR(5)/ATR(20) > 1.2（最佳）

回到 XLU-003 框架（10日 PB 3.5-7% + WR ≤ -80 + ClosePos ≥ 40%），
加入 ATR 比率 > 1.2 波動率飆升過濾。

| 指標 | Part A | Part B |
|------|--------|--------|
| 訊號數 | 5 | 3 |
| 勝率 | **100.0%** | 66.7% |
| Sharpe | 0.00（std=0）| **1.26** |
| 累計報酬 | **+13.14%** | **+4.85%** |
| 平均持倉 | 4.8 天 | 9.7 天 |
| MDD | -0.90% | -3.34% |

ATR > 1.2 完美消除所有 2022 漸進式下跌假訊號。5/5 Part A 交易全部達標出場（100% WR）。
Part B 2/3 達標、1 筆到期（-0.20% 微虧）。
Part A Sharpe = 0.00 是 std=0 的數學異常（所有報酬均為 +2.50%），非績效問題。
...
重新引入 3 個 2021-2022 停損訊號（2021-09-20, 2022-01-18, 2022-06-10），
ATR > 1.1 門檻不足以過濾這些漸進式下跌。

### 結論

**ATR(5)/ATR(20) > 1.2 是 XLU 均值回歸的突破性發現：**

| 策略 | Part A Sharpe | Part B Sharpe | Part A WR | Part B WR |
|------|---------------|---------------|-----------|-----------|
| XLU-003（無 ATR 過濾） | 0.06 | 0.35 | 61.9% | 70.0% |
| XLU-010 Att2（ATR > 1.2）| 0.00*（100% WR）| 1.26 | **100.0%** | 66.7% |
| XLU-004（BB 突破最佳） | 0.18 | 0.26 | 53.8% | 75.0% |

*std=0 數學異常

**關鍵教訓：**
- ATR 比率是區分「急跌」（恐慌性，可均值回歸）與「慢跌」（利率驅動，不可回歸）的有效工具
- 此過濾器專門針對已知失敗模式（漸進式下跌），符合 lesson #6 的例外條件
- ATR > 1.2 vs > 1.1 的差異極端：1.2 時 Part A 100% WR，1.1 時降至 66.7%（過濾器極為敏感）
- 訊號數偏少（1.0/年 Part A，1.5/年 Part B）是主要限制
- **波動率飆升過濾可推廣至其他利率敏感/低波動資產（如 TLT、DIA）**

---

## XLU-011: Volatility-Adaptive Mean Reversion（波動率自適應均值回歸）

> 實驗代碼：`xlu_011_vol_adaptive_mr`
> 策略類型：均值回歸 + 波動率自適應過濾
> 建立日期：2026-04-09

### 假說

XLU-010 發現 ATR(5)/ATR(20) > 1.2 完美過濾漸進式下跌假訊號（Part A 100% WR），
但僅 8 個訊號（5+3）統計可靠性不足。ATR > 1.1 又過鬆（引入 3 個停損）。
探索未測試的中間值 ATR > 1.15，尋求品質與數量的最佳平衡。

### 嘗試歷程

#### Att1：ATR > 1.15（最終選擇）

XLU-003 框架（PB 3.5-7% + WR(10) ≤ -80 + ClosePos ≥ 40%）+ ATR(5)/ATR(20) > 1.15。
...
vs XLU-010 Att2（ATR > 1.2）：多 2 個 Part A + 1 個 Part B 好訊號。
Part A 新增 2022-12-19（+2.50% 達標）和 2023-09-06（+2.50% 達標），
Part B 新增 2025-03-05（+2.50% 達標）。三個新訊號全部達標出場。
唯一停損是 2021-09-20（-4.10%），與 XLU-010 Att3 相同——此訊號在 ATR 1.1-1.15 之間。
...
Part A 完全不變（無 ATR 1.12-1.15 範圍的 Part A 訊號）。
Part B 新增 2025-10-22 到期虧損（-3.05%），Sharpe 從 1.56 崩至 0.38。
確認 ATR > 1.15 是 Part B 的品質邊界。

#### Att3：回檔上限 8%（無差異）

ATR > 1.15 + 回檔上限從 7% 放寬至 8%。結果與 Att1 完全相同——
無訊號的回檔落在 7-8% 範圍內且同時通過 ATR > 1.15 過濾。

### 結論

**ATR > 1.15 是 XLU 全域最優 ATR 門檻：**

| 策略 | Part A Sharpe | Part B Sharpe | min(A,B) | 訊號數 A+B |
|------|---------------|---------------|----------|-----------|
| XLU-003（無 ATR） | 0.06 | 0.35 | 0.06 | 21+10=31 |
| XLU-010 Att3（ATR > 1.1） | 0.10 | 0.38 | 0.10 | 9+5=14 |
| **XLU-011 Att1（ATR > 1.15）** | **0.67** | **1.56** | **0.67** | 7+4=11 |
| XLU-010 Att2（ATR > 1.2） | 0.00* | 1.26 | 0.00* | 5+3=8 |
| XLU-004（BB 突破） | 0.18 | 0.26 | 0.18 | 13+4=17 |

*std=0 數學異常

**關鍵教訓：**
- ATR > 1.15 是品質與數量的最佳平衡：比 1.2 多 3 個好訊號，比 1.1 少 3 個壞訊號
- 年化訊號率 Part A 1.4/yr vs Part B 2.0/yr（比率 0.7:1），Part B 更頻繁（無過擬合）
