<!-- AI_CONTEXT_START - 此區塊供 AI Agent 快速讀取，人工更新
  ...
  phase9_note_2026_08_06: FCX-006, FCX-015, and FCX-016 now consume declared COPX/^VIX historical auxiliary bundles through the provider-free execution boundary; migration runs remain migration-pending and do not advance latest or followup qualification.
  phase9_note_2026_08_06_primary_wave: FCX-001 through FCX-005, FCX-007, and FCX-009 through FCX-014 now use the shared primary-only verified bundle boundary; migration runs remain pending and do not advance latest or followup qualification.
  note: FCX-016 added 2026-05-17 (Post-Parabolic 長窗 prior-Nd-return CEILING — cross-asset port of URA-014 Att1 ★ SUCCESS lesson #19 family v3 onto FCX-015 Att2 (FCX global optimum min(A,B)† = Part A Sharpe 1.43). **predict→confirm documented-failure, prediction CORRECT — track record now 3 SUCCESS (GLD-016/VOO-005/URA-014) + 10 documented-failure all correctly predicted**). Mandatory pre-analysis (done FIRST): single residual binding Part A SL = 2021-11-11 (-7.14%, only loser in FCX-015 Att2 10-trade Part A WR 90%). Prior-Nd return vs 9 winners: Ret60 SL **+20.97%** vs winners [+3.4%, +62.6%] → SL BELOW max by 41.59pp, 7/9 winners higher → **direction INVERTED**; fully INTERLEAVED on every window (Ret20 +10.15/Ret30 +25.99/Ret60 +20.97/Ret90 +13.43/Ret120 -1.20/Ret252 +110.77, all mid-pack). **Structural INVERSE of URA-014** (URA SL 2021-11-16 = highest Ret60 +51.39% fresh parabolic blow-off, +21.80pp above all winners, ≥20pp robust plateau; FCX SL = mid/low, FCX's biggest prior-60d run-ups +55-63% are ALL winners = 2020 COVID V-recovery breakouts). Ex-ante mechanism: for a BREAKOUT strategy on a copper-supercycle commodity miner, high prior-Nd run-up is the PRODUCTIVE momentum-continuation regime — the structural opposite of URA's MR framework where high prior-Nd run-up = parabolic-unwind dip trap. Predicted documented-failure. Three iterations (Ret60 ceiling sweep) CONFIRMED: Att1 (≤+0.21, mid) Part A 10→4/WR 90%→75%/Sharpe **1.43→0.64** (-55%)/cum +85.63→+16.98 — SL RETAINED (+20.97≤+0.21), 6 high-Ret60 winners KILLED (2020-07-06/08-05/11-03/12-30, 2021-04-26, 2023-01-06) → direction INVERTED, Part B 3→2, min(A,B) 0.64 **FAIL**; Att2 (≤+0.45, looser) Part A 10→6/83.3%/**0.97** (-32%)/cum +36.44 — SL RETAINED (gate non-binding on SL), 4 biggest-Ret60 winners killed, Part B 3/100% unchanged, min 0.97 **FAIL**; Att3 (≤+0.15, tighter cliff) Part A 10→**2** zero-var (only 2 lowest-Ret60 winners survive 2019-12-09/2020-06-02), SL removed only by also killing 7 winners, Part B 3→1, A/B cum gap |16.64-8.00|/16.64 = 52% >>30% **REJECT** (degenerate over-filter, EEM-016/CIBR-016/INDA-013 rigor precedent). **核心發現（lesson #19 family v3 邊界 — long-horizon prior-return CEILING 首個跨資產 documented-failure）**: (1) URA-014 post-parabolic long-horizon prior-Nd-return CEILING does NOT port to a copper-supercycle commodity-miner BREAKOUT strategy — URA-014's success precondition is "binding residual SL = fresh parabolic blow-off (Ret60 全場最高 + ≥20pp robust plateau)"; FCX-015 Att2's SL is mid/low Ret60, fully interleaved, direction INVERTED; (2) **framework×asset interaction**: for MR (URA capitulation) high prior-Nd run-up flags "false dip-buy in parabola unwind" (productive CEILING); for BREAKOUT (FCX BB Squeeze) high prior-Nd run-up flags "high-quality momentum-continuation breakout" (CEILING inverts, kills the best winners) — same lesson #19 v3 dimension, OPPOSITE direction across the two frameworks; (3) NO separable plateau on any prior-Nd window (contrast URA-014's ≥20pp robust plateau) — the only ceiling that removes the SL (<+20.97%) necessarily kills 7/9 winners; (4) URA-014 candidate cross-asset port list (FCX/COPX/SOXL/SIVR) converges: FCX tested = documented-failure (direction-inverted); COPX/SOXL/SIVR in avoid/remote-churn lane → the commodity/mining candidates of the URA-014 long-horizon CEILING port are effectively exhausted unless a future asset's binding SL is predict→confirm-verified as a separable post-parabolic outlier. FCX-015 Att2 remains FCX 突破策略 + 全域最優 (16 experiments, 54+ attempts); FCX's 13th failed strategy type added (long-horizon prior-return CEILING, cross-asset port of URA-014 lesson #19 family v3). FCX-015 added 2026-05-07 (VIX BANDS Filter on BB Squeeze Breakout, **repo 第 2 次 lesson #24 family BANDS 變體 + 首次 BANDS 變體於 BB Squeeze Breakout 框架 + repo 首次 ^VIX FLOOR 變體於任何資產**, cross-strategy port from XBI-017 Pullback MR + reverse-direction discovery). Three iterations: Att1 (mode=bands_exclude_mid, vix_low=17, vix_high=22, XBI-017 sweet spot direct port) Part A 13/69.2%/Sharpe **0.52** cum +50.33% / Part B 3/66.7%/0.41/+8.31% / min **0.41** (-36% vs FCX-014 0.64) — FAIL: 過濾 3 TPs (2021-04-26/2021-12-22/2023-01-06，皆 VIX 17-22 mid 帶) 而全 3 SLs 保留 (2019-07-24/2020-01-13/2021-11-11→shifted 11-12)；Att2 ★ SUCCESS (mode=floor, vix_low=14.0，反向假說) Part A 10/**90.0%**/Sharpe **1.43** cum +85.63% / Part B 3/100%/std=0 cum +25.97% / min(A,B)† **1.43** (+123% vs FCX-014 Att1 0.64); A/B 年化 13.18%/yr vs 12.24%/yr → gap **7.1% << 30% ✓✓✓** (vs FCX-014 52.5% ❌)；A/B 訊號比 1.33:1 (gap 25% < 50% ✓)；過濾 3 Part A SLs + 1 Part B SL (全部 SLs 集中於 VIX <= 14 calm regime) + 副作用過濾 2 calm-period TPs + 2 expiries；Att3 (mode=floor, vix_low=15.0，threshold robustness check) Part A 完全相同 Att2 (10/90%/1.43/+85.63%) / Part B 2/100%/std=0/+16.64% (-1 TP vs Att2，2024-04-29 VIX 介於 14-15 之間被過濾) / min(A,B)† 1.43 但 A/B gap 39.3% > 30% ❌ — 確認 14 為甜蜜點。**核心發現（lesson #24 family v2 + 反向 VIX 分布結構）**：(1) **lesson #24 BANDS 變體不可從 XBI 跨策略直接移植至 FCX BB Squeeze Breakout**(Att1 已證實)；(2) **FCX 殘餘 SLs 集中於 low-VIX calm regime**（與 XBI SLs 集中於 mid-VIX complacency creep 結構性相反）：FCX BB Squeeze breakout 在低 VIX 時為 false breakout（市場 calm 但 FCX 自身突破缺乏 broad-market vol 證實）；(3) **VIX FLOOR > 14 為 surgical filter** — 全部 3 Part A SLs + 1 Part B SL 均位於 VIX <= 14 帶；(4) **Repo 首次 lesson #24 family FLOOR 變體於任何資產**——擴展 lesson #24 family 為 5 種變體：LEVEL CAP（TLT-013 ^MOVE）/ DIRECTION（XLU-013、GLD-015、USO-025）/ BANDS exclude mid（XBI-017）/ **FLOOR（FCX-015，commodity/mining single stock 突破策略）**；(5) **A/B cum gap 突破** — FCX 商品超級週期結構性 A/B cum gap 邊界（FCX-014 52.5%、FCX-013 Att3 44.0%）首次降至 7.1%，VIX FLOOR 過濾 calm-period 邊際 trades（2 TPs + 2 expiries）系統性平衡 Part A 過度報酬至 Part B 健康水準。FCX-015 Att2 為新全域最優（15 次實驗、51+ 次嘗試），取代 FCX-014 Att1。FCX-014 added 2026-05-02 (Multi-Period Direction-Filter Regime Breakout, **repo 首次 lesson #19 family ceiling 維度移植至 BB Squeeze Breakout 框架**, cross-strategy port from TSM-011 Att3 RS Momentum framework). Three iterations: Att1 ★ (3d ceiling 12.0% on FCX-013 Att3 base) Part A 16/75.0%/Sharpe **0.69** cum +89.38% (vs FCX-013 Att3 17/70.6%/0.55/+75.86%, **+25%/+16%/+13.52pp**) / Part B 4 訊號完全不變 Sharpe 0.64 cum +16.98% / min(A,B) **0.64** (+16% vs 0.55 baseline). 過濾 2021-04-15 SL（3d=12.61% 唯一超出 12% 門檻訊號）+ cooldown chain shift 將 2021-05-05 TP 替換為 2021-04-26 TP（同 +8%）；Att2 (3d ceiling 11.0%，邊界測試) 訊號集 cum WR Sharpe 完全相同 Att1 — cooldown chain shift 將原 2023-01-06 TP 替換為 2023-01-09 TP（同 +8%），確認 [11.0%, 12.0%] 為 robust sweet spot 區間。中間 1d ceiling 0.08 與 3d 0.12 雙維度組合測試亦證明 1d ceiling 在 3d 已啟用時 redundant（chain shift 1-day 偏移後出場類型完全相同）；Att3 (3d ceiling 9.0%，下邊界 degradation 測試) Part A 15/66.7%/Sharpe **0.44** cum +50.77%（**-31% vs Att1**） / Part B 同 / min(A,B) **0.44** (degraded). 9.0% 過嚴過濾 3 TPs（2020-07-06、2021-05-05、2023-01-06）chain shift 無法補齊。**核心發現（lesson #19 family v12 cross-strategy 鏡像擴展，repo 第 4 個策略類型）**: (1) **Repo 首次 signal-day return CEILING 維度於 BB Squeeze Breakout 框架** — 既往 lesson #19 應用於 MR 框架（11+ 次 floor/cap 成功）+ RS Momentum 框架（TSM-011 Att3 ceiling）+ now BB Squeeze Breakout（FCX-014 Att1 ceiling），三大框架同源失敗模式對齊：MR「太淺 capitulation」（floor）/ RS「太深 rally」（ceiling）/ Breakout「rally exhaustion」（ceiling）；(2) **Cooldown chain shift 中性化效應於 robust threshold 區間**：[11%, 12%] 與 [12%, 12.61%] 區間 chain shift 維持同 outcome（次日訊號重新通過 ceiling），threshold 不需精準到 0.5% 步進；(3) **A/B 累計差結構性邊界**：Att1 0.64 sharpe 改善但 A/B cum gap 從 FCX-013 Att3 的 44.0% 增至 52.5%（>30% ❌），FCX 商品超級週期使 Part A 2020-2021 高報酬無法被 ceiling filter 改善的同時影響 Part B 弱回升期（max 3d 8.64% 遠低於 12% 閾值）；訊號比 1.6:1 (gap 37.5% < 50% ✓) 維持；(4) **跨資產假設（待驗證）**：BB Squeeze Breakout + signal-day ceiling 可能適用於其他高 vol mining/commodity 類型（COPX-011 銅礦 ETF、NVDA-012、TSLA-015 等既有 BB Squeeze 框架），閾值依 SLs 3d 分布調整。FCX-014 Att1 為新全域最優（14 次實驗、48+ 次嘗試），取代 FCX-013 Att3 為突破策略最佳。FCX-013 added 2026-04-27 (Multi-Week Regime-Aware BB Squeeze Breakout, **repo 第 3 次 lesson #22 試驗、首次商品/礦業單股驗證 buffered multi-week SMA trend regime**, cross-asset extension from TSLA-015 / NVDA-012). Three iterations, **Att3 SUCCESS**: Att1 (k=0.99 TSLA cross-asset port, 1% buffer) Part A 18/66.7%/Sharpe **0.44** cum +63.30% / Part B 5/80%/Sharpe **0.82** cum +26.34% / min(A,B) **0.44** (+7% vs FCX-004 0.41) — Part B 大幅改善 (0.41→0.82) 為主要貢獻，Part A 略退化 (0.51→0.44) 因 4 TP 過濾損失大於 2 SL 過濾受益. Att2 (k=0.97 NVDA cross-asset port, 3% buffer) Part A 19/68.4%/0.48 / Part B 6/66.7%/0.41 完全相同 baseline / min **0.41** — k=0.97 過寬，Part B 2025-08-26 SL (ratio 0.972 > 0.97) 保留，**證實 NVDA k 值不可直接移植 FCX**：NVDA 該區間有 transition winners 而 FCX 該區間僅有 SLs. **Att3 ★ SUCCESS (k=1.00 嚴格無緩衝)** Part A 17/70.6%/Sharpe **0.55** cum +75.86% / Part B 4/75%/Sharpe **0.64** cum +16.98% / min(A,B) **0.55** (+34% vs 0.41 baseline，+25% vs Att1 0.44). **跨資產發現（與 TSLA/NVDA 反向）**：FCX 上 k=1.00 嚴格優於 k<1 buffered，反轉 lesson #22 「k<1 緩衝」原則。失敗根因（TSLA k=1.00 / NVDA k=0.99 失敗）為 borderline transition winners 在 ratio 0.97-1.00 區間被誤殺；**FCX 該區間無 winners**（僅 1.013 略高於 1.00 自然保留），故 k=1.00 無代價。**lesson #22 跨資產精煉**：k 取決於資產 winners 在 transition zone (0.93-1.00) 分布——TSLA winners 集中 0.99-1.00 (k=0.99) / NVDA 跨 0.97-1.00 (k=0.97) / **FCX 集中 0.93-0.97 + bull-regime 主導 (k=1.00 嚴格)**. A/B 平衡 acceptance：訊號比 1.7:1 (gap 41.2% < 50%) ✓，cum gap 44.0% (FCX 商品超級週期結構性邊界) ✗，**從 baseline 89% / 3.83:1 大幅改善**. FCX-013 Att3 為 BB Squeeze 框架新最佳，與 FCX-001 MR 框架並列雙最優。13 experiments 39+ attempts. FCX-012 added 2026-04-23 (Donchian Lower Washout + Intraday Reversal MR, **repo first "Donchian Lower touch + intraday reversal" combo as MR primary entry**). Three iterations all failed vs FCX-004 min 0.41: Att1 (baseline: Close within 2.5% of Donchian_Low(20) + today or yesterday Low = 20d low + ClosePos>=40% + ATR(5)/ATR(20)>=1.10 + 60d DD in [-30%,-10%] + TP+9%/SL-11%/20d/cd15) Part A 8/50%/**-0.06** cum -8.16% / Part B 6/50%/**0.02** cum -0.93% / min **-0.06** — two Part A -11% SLs (2019-08-05 trade war, 2023-04-25 drift) are continuation-decline traps; Part B dominated by small expiries (2024-01/11/12) showing washout signal lacks trigger momentum in sideway regimes; Att2 (+ require today Low > yesterday Low = Day-After Capitulation) Part A 1/0%/Sharpe 0.00 zero-var cum -12.36% / Part B 2/100% Sharpe 1.07 cum +9.33% / min **-0.60** (single-trade approximation) — over-filters to 3 total signals, statistical insignificance; **validates lesson #20b Day-After Capitulation failure family extending to FCX 3% vol single commodity stock** (after URA 2.34% policy-driven / TLT 1% rate-driven, third data point); Att3 (revert Att2, add 2DD cap >= -7%, CIBR-012 direction) Part A 7/42.9% Sharpe **-0.20** cum -15.74% (**WORSE than Att1**) / Part B 6/50% Sharpe 0.02 unchanged / min **-0.20** — 2DD cap removed 1 TP winner (not SL) dropping WR 50→42.9%; Part B all signals' 2DD > -7% (structurally unbinding). **Core failure**: FCX winners' 2DD distribution spans -3%~-8% overlapping with losers (same pattern as FCX-011 Att2 found), no unidirectional selectivity opposite to CIBR 1.53% vol success. **Repo first Donchian Lower Washout + intraday reversal trial** — extends lesson #52 failed-breakdown-reversal family to "Donchian Lower binary price trigger" category: single-day Donchian Low touch + intraday reversal insufficient to distinguish "tail-of-washout" from "mid-acceleration" in multi-stage declining assets (parallels FXI-009 Failed Breakdown Reversal 3 iteration failure). FCX's 12th failed strategy type. FCX-001 remains global optimum; FCX-004 remains execution-model optimum (min 0.41). FCX-011 added 2026-04-22 (Post-Capitulation Vol-Transition MR, **repo first BB-lower + pullback-cap hybrid mode trial on high-vol single stock (FCX ~3% vol)** — cross-asset extension from VGK-008 / INDA-010 / EEM-014 / CIBR-012). Three iterations all failed vs FCX-004 min 0.41: Att1 (baseline hybrid: BB(20,2) + PB cap -15% + WR<=-80 + ClosePos>=40% + ATR>1.15 + TP+6%/SL-7%/20d/cd10) Part A 3 signals WR 66.7% cum +4.34% Sharpe **0.26** / Part B 5 signals WR 40.0% cum -7.86% Sharpe **-0.23** / min -0.23 — Part B 2024-07-19 + 2024-12-13 both 1-3d fast SLs indicate signal-day filter cannot distinguish accelerating-crashes from genuine capitulation; Att2 (+ 2DD cap >= -5%, CIBR-012 direction) Part A 2/50% Sharpe -0.09 / Part B 2/50% Sharpe -0.09 / min -0.09 — cap removes 2021-06-16 TP winner (deep 2DD) alongside 2024-07-19 SL; deep 2DD covers both winners and losers in FCX, no unidirectional selectivity; Att3 (+ 2DD floor <= -5%, VGK/INDA/EEM direction) Part A 1/100% zero-var / Part B 4/50% Sharpe ~0 / min 0.00 — floor filters 2/3 Part A signals (density 0.2/yr), keeps both winners and expiry losses in Part B. **Core failure**: FCX winners' 2DD distribution spans -3%~-8%, overlapping with losers (distinct from CIBR SLs concentrated deep 2DD or VGK SLs concentrated shallow 2DD). **Repo first BB-lower hybrid mode on high-vol single stock** — **extends XBI-010's 1.75% vol upper boundary to include single-stock category** (previously established only on ETFs). Integrated rule: BB-lower + pullback-cap hybrid mode applies only to low-mid vol broad/sector/single-country ETFs (1.12%~1.75%); fails on XBI (2.0% ETF), FXI (policy EM), GLD/TLT (commodity/rate), and now **FCX (~3% single stock)**. FCX's 9th failed strategy type (after pullback+WR, momentum pullback, RS, Donchian, trend pullback, RSI divergence, Gap-Down, BB-lower-hybrid). FCX-001 remains global optimum (11 experiments, 33+ attempts); FCX-004 remains execution-model optimum (min 0.41). FCX-010 added 2026-04-21 (Gap-Down Capitulation + Intraday Reversal MR, **repo first single-stock Gap-Down trial**, cross-asset test from IBIT-006 Att2). Three iterations all failed vs FCX-001 min 0.43: Att1 (Gap<=-2.0% + Close>Open + 10d PB [-6%,-18%] + WR<=-80 + TP+5%/SL-4%/15d) Part A n=9 WR 55.6% cum +7.77% Sharpe 0.21 / Part B n=2 WR 100% zero-variance Sharpe 0.00 — 4 Part A SLs concentrated in 2019 trade war + 2023 SVB first-wave shocks (gap-down continues not capitulates); Att2 (+ ClosePos>=50% strong intraday reversal filter) Part A n=8 WR 62.5% cum +12.42% Sharpe 0.36 / Part B unchanged n=2 zero-variance — removes 1 weak-reversal SL (2023-02-24) but 3 policy-shock SLs remain; Att3 (tighten Gap<=-2.5% + pullback floor -8%) Part A n=5 WR 80% cum +16.52% Sharpe **0.87** (repo-first single-stock gap-down high Part A Sharpe) / Part B n=1 zero-variance Sharpe 0.00, A/B signal annualized ratio 2.0:1 and A/B cum gap 69.7% violate balance goals. **Repo first Gap-Down Capitulation trial on single stock** — extends lesson #20a failure family: Gap-Down pattern requires NOT JUST 24/7 overnight price discovery but ALSO selling-pressure exhaustion uncorrelated with daytime policy/commodity continuity. FCX (copper-linked high-vol stock) has LME/SHFE/COMEX near-24h futures coverage but copper price shocks (trade policy, USD, global demand) persist into US session, paralleling FXI-010's Chinese policy continuity failure. Failure family extended: TQQQ-016 (leveraged index ETF, QQQ non-24/7) + FXI-010 (policy-driven EM ETF) + FCX-010 (commodity-linked single stock) — all fail; only IBIT (pure BTC 24/7 ETF) validates. Cross-asset hypothesis (pending validation): Gap-Down MR may apply ONLY to pure-crypto ETFs (BTC/ETH spot), not to any US equity with macroeconomic/commodity exposure.
-->
## AI Agent 快速索引

**當前最佳（均值回歸）：** FCX-001（三重條件極端超賣均值回歸：60日回撤≤-18% + RSI<28 + SMA50乖離≤-8%）
**當前最佳（突破策略）：** ★ **FCX-015 Att2**（VIX BANDS Filter on BB Squeeze Breakout：FCX-014 Att1 完整框架 + **^VIX > 14.0 FLOOR**，TP+8%/SL-7%/20d/cd10）★ **2026-05-07 新全域最優（15 次實驗、51+ 次嘗試）**
- Part A: 10 訊號 / WR **90.0%** / Sharpe **1.43** / cum +85.63% / MDD -10.09%
- Part B:  3 訊號 / WR **100%** / std=0 / Sharpe 0.00 / cum +25.97%（3 TP 全勝，與 FCX-014 baseline 4/75%/0.64/+16.98% 相比，過濾 1 SL，留 3 winners）
- min(A,B)† **= Part A Sharpe 1.43**（沿用 EWJ-003/SPY-009/DIA-012/IWM-013/CIBR-014 慣例：Part B std=0 結構性零方差，採 Part A Sharpe 為 min 約束），**+123% vs FCX-014 Att1 0.64**
- A/B 年化 cum 13.18%/yr vs 12.24%/yr → gap **7.1% << 30% ✓✓✓**（vs FCX-014 52.5% ❌，**首次突破 FCX 商品超級週期結構性 A/B gap 邊界**）
- A/B 訊號比 2.0/yr vs 1.5/yr → 1.33:1 (gap 25% < 50% ✓)
- **三項 acceptance criteria 全部達標**
- 跨策略貢獻：(1) repo 第 2 次 lesson #24 family BANDS 變體 + 首次於 BB Squeeze Breakout 框架；(2) **repo 首次 ^VIX FLOOR 變體於任何資產**；(3) 跨資產發現：FCX 與 XBI VIX 分布結構**反向**（FCX SLs 集中 low-VIX calm regime，XBI SLs 集中 mid-VIX complacency creep）；(4) Att1 (BANDS exclude mid) 直接移植 XBI-017 sweet spot 失敗確立 lesson #24 family 跨策略移植邊界

**前任最佳（突破）：** FCX-014 Att1（Multi-Period Direction-Filter Regime Breakout：FCX-013 Att3 完整框架 + **訊號日 3 日累計報酬 ceiling <= +12.0%** rally exhaustion filter，TP+8%/SL-7%/20d/cd10）— Part A 16/75%/Sharpe 0.69/+89.38%, Part B 4/75%/0.64/+16.98%, min(A,B) 0.64（已被 FCX-015 Att2 超越；3d ceiling 仍為 FCX-015 base 之必要組件）

**前前任最佳（突破）：** FCX-013 Att3（Multi-Week Regime-Aware BB Squeeze Breakout：FCX-004 框架 + **SMA(20) ≥ 1.00 × SMA(60)** 嚴格 multi-week trend regime gate，TP+8%/SL-7%/20d/cd10）— Part A Sharpe 0.55, Part B Sharpe 0.64, min(A,B) 0.55（+34% vs FCX-004 0.41）。**Repo 第 3 次 lesson #22 試驗、首次商品/礦業單股驗證**，跨資產反向發現：FCX k=1.00 嚴格優於 TSLA k=0.99 / NVDA k=0.97 buffered。

**前前前任最佳（突破）：** FCX-004（同進場無 regime gate，min 0.41）
...
- ~~Post-parabolic 長窗 prior-Nd-return CEILING（跨資產移植 URA-014 lesson #19 family v3）~~ → **FCX-016 三次迭代全部 FAIL/REJECT，predict→confirm documented-failure 正確預測**：FCX-015 Att2 殘餘 SL 2021-11-11 之 Ret60 +20.97% 與 winners [+3.4%,+62.6%] 完全 interleaved 且方向 INVERTED（URA SL 為全場 Ret60 最高 / FCX SL mid-low；對突破策略高 prior-Nd run-up 是 productive 而非 trap）。**禁止重複此維度於 FCX 突破策略**
- 已證明無效（禁止重複嘗試，FCX-016）：long-horizon prior-Nd-return CEILING 不可移植至 copper-supercycle commodity-miner 突破策略——同一 lesson #19 family v3 維度在 MR（URA productive CEILING）與 Breakout（FCX inverted）方向相反；URA-014 候選港 COPX/SOXL/SIVR 在 avoid/remote-churn lane，commodity/mining 候選實質耗盡
- FCX-015 Att2 已確認為突破策略 + 全域最優（16 次實驗、54+ 次嘗試，含均值回歸、突破（BB Squeeze + Donchian）、動量回檔、RSI(2)、相對強度、趨勢跟蹤、波動率自適應、RSI bullish hook divergence、Gap-Down Capitulation、BB-lower hybrid mode、Donchian Lower Washout、VIX FLOOR regime、**long-horizon prior-return CEILING** 十三大策略類型）
...
| FCX-006 | `fcx_006_relative_strength`   | FCX-COPX 相對強度（3次嘗試均失敗；Phase 9 auxiliary bundle，migration-pending） | 已完成 |
...
| FCX-013 | `fcx_013_regime_breakout`     | Multi-Week Regime-Aware BB Squeeze Breakout（FCX-004 + buffered SMA(20)≥k×SMA(60)，**Att3 SUCCESS k=1.00 嚴格 min(A,B) 0.55 +34%**，repo 第 3 次 lesson #22 試驗、首次商品/礦業單股、跨資產反向發現） | ✅ 突破策略前前任最佳 |
| FCX-014 | `fcx_014_breakout_ceiling`    | Multi-Period Direction-Filter Regime Breakout（FCX-013 Att3 + **訊號日 3d ceiling <= +12.0%** rally exhaustion filter，**Att1 SUCCESS min(A,B) 0.64 +16%**，repo 首次 lesson #19 family ceiling 維度移植至 BB Squeeze Breakout 框架） | ✅ 突破策略前任最佳 |
| FCX-015 | `fcx_015_vix_bands_breakout`  | VIX BANDS Filter on BB Squeeze Breakout（FCX-014 Att1 + **^VIX > 14.0 FLOOR**，**Att2 SUCCESS min(A,B)† 1.43 +123%**，repo 第 2 次 lesson #24 BANDS 變體 + 首次 BANDS 變體於 BB Squeeze Breakout + 首次 ^VIX FLOOR 變體於任何資產，A/B cum gap 52.5%→7.1% 突破 FCX 商品超級週期結構性邊界；Phase 9 auxiliary bundle，migration-pending） | ★ 突破策略當前最佳 |
| FCX-016 | `fcx_016_postparabola_regime_breakout` | Post-Parabolic 長窗 prior-Nd-return CEILING（FCX-015 Att2 + 跨資產移植 URA-014 lesson #19 family v3，3 次迭代全部 FAIL/REJECT，**predict→confirm documented-failure 正確預測**：FCX SL 2021-11-11 Ret60 與 winners 完全 interleaved 且方向 INVERTED vs URA-014；Phase 9 auxiliary bundle，migration-pending） | 已完成（documented-failure） |
...
**結論：FCX-013 Att3（k=1.00 嚴格）為 FCX 突破策略前任最佳，min(A,B) 0.55 vs FCX-004 的 0.41（+34%），同時大幅改善 A/B 平衡（訊號比 3.83:1 → 1.7:1）。** FCX-014 Att1 進一步將 min(A,B) 提升至 0.64（+16%）。

---

## FCX-014: Multi-Period Direction-Filter Regime Breakout（3 次嘗試，Att1 SUCCESS 3d ceiling 12.0%）★ 突破策略當前最佳

**實驗目的：** **Repo 首次將 lesson #19 family 的「signal-day return CEILING」維度跨策略移植至 BB Squeeze Breakout 框架**。既往 lesson #19 family 應用於 MR 框架（11+ 次 floor/cap 成功）、TSM-011 Att3 首次擴展至 RS Momentum 框架（5d ceiling +10.5% rally exhaustion），FCX-014 為第 3 大策略類型（BB Squeeze Breakout）的首次驗證。

**假設：** BB Squeeze Breakout 與 RS Momentum 同屬「動量延續」型策略，failure mode 結構性鏡像（MR 「太淺 capitulation」需 floor、RS/Breakout「rally exhaustion」需 ceiling）。FCX-013 Att3 trade-level 分析顯示 1 筆 SL（2021-04-15）3d return +12.61% 唯一超出 12% 門檻，過濾此單筆 SL 預期可改善 Sharpe。

**結果：** 三次迭代，**Att1 SUCCESS** min(A,B) **0.64**（+16% vs FCX-013 Att3 0.55）。

**迭代明細：**

| 迭代 | 3d ceiling | Part A (訊號 WR Sharpe cum) | Part B (訊號 WR Sharpe cum) | min(A,B) | 結論 |
|------|-----------|----------------------------|------------------------------|----------|------|
| Att1 ★ | 12.0% | 16 / 75.0% / **0.69** / +89.38% | 4 / 75.0% / 0.64 / +16.98% | **0.64** | **SUCCESS（+16%）** |
| Att2 | 11.0% | 16 / 75.0% / 0.69 / +89.38% | 4 / 75.0% / 0.64 / +16.98% | **0.64** | 持平（chain shift 中性化）|
| Att3 | 9.0% | 15 / 66.7% / 0.44 / +50.77% | 4 / 75.0% / 0.64 / +16.98% | **0.44** | FAILED（**-31%**）|
...
2. 收盤價 > Upper BB(20, 2.0)（突破上軌）
3. 收盤價 > SMA(50)（短期趨勢向上）
4. SMA(20) ≥ 1.00 × SMA(60)（嚴格 multi-week trend regime gate，同 FCX-013 Att3）
5. **訊號日 3 日累計報酬 <= +12.0%（lesson #19 family ceiling，rally exhaustion filter）**
6. 冷卻期 10 個交易日

**出場參數：** TP +8% / SL -7% / 持倉 20 天（同 FCX-013 Att3）

**成交模型：** 隔日開盤市價進場，0.15% 滑價（高 vol 個股），悲觀認定（同 FCX-013 Att3）

**核心 trade-level 發現（Att1 ★）：**

過濾事件：
- **2021-04-15 SL（3d=12.61%）唯一超出 12.0% ceiling 訊號**——過濾後消失 -7.14% 損失
- Cooldown chain shift 副作用：原 2021-05-05 TP 因受新訊號 2021-04-26 cooldown 抑制，但 **2021-04-26 為新 +8% TP**（同 outcome），淨效果 +7.14%（消除 SL）+ 0%（chain shift 等價）

Threshold sweep 結果：
- **[11%, 12%] robust sweet spot**（Att1 = Att2，cooldown chain shift 中性化）
- **9% 下邊界失敗**（Att3 過濾 3 TPs：2020-07-06、2021-05-05、2023-01-06）
- 上邊界 12.61%（最高 SL 3d 值）為理論上界，> 12.61% 則無 SL 過濾
- 1d ceiling 與 3d ceiling 組合測試（已棄用）：1d 0.08 + 3d 0.12 cooldown chain shift 導致 1-day 偏移（2020-07-06 → 07-07、2021-11-11 → 11-12）但出場類型完全不變，**1d ceiling 在 3d ceiling 已啟用時 redundant**

**A/B 平衡指標（Att1 ★ 最終）：**

- Part A 16 訊號（3.2/yr）vs Part B 4 訊號（2.0/yr），訊號比 1.6:1（gap 37.5% < 50% ✓）
- Part A WR 75.0% vs Part B 75.0%（**完全一致**）
- Part A annualized cum 17.88%/yr vs Part B 8.49%/yr（gap 52.5% > 30% ❌）
- A/B cum gap 為 FCX 結構性邊界——3d ceiling 在 Part B 完全非綁定（max 3d 8.64% 遠低於 12% 閾值），無法改善 Part B 同時擴大 Part A 與 Part B 的累計差距
- 結構性邊界較 FCX-013 Att3 略增（44% → 52.5%），但 Sharpe 改善 +16% 與 Sharpe 為主要 acceptance 標準

**跨資產貢獻：**

- **Repo 首次 lesson #19 family ceiling 維度移植至 BB Squeeze Breakout 框架**——擴展 lesson #19 適用範圍至 3 大策略類型（MR floor/cap → RS Momentum ceiling → BB Squeeze Breakout ceiling）
- **三大策略框架失敗模式鏡像對齊**：MR「太淺 capitulation」（floor）/ RS「太深 rally」（ceiling）/ Breakout「rally exhaustion」（ceiling）——同源 lesson #19 維度
- **Threshold robustness 區間特性**：[11%, 12%] cooldown chain shift 中性化使 threshold 不需精準調校，與既往 GLD-014 / SIVR-018 / TSM-011 同樣展現 lesson #19 family 的 robust threshold 區間特性
- **新跨資產假設（待驗證）**：BB Squeeze Breakout + signal-day ceiling 可能適用於其他高 vol mining/commodity 類型——COPX-011 銅礦 ETF、NVDA-012 AI 牛市個股、TSLA-015 EV 個股等既有 BB Squeeze 框架，閾值依該資產 SLs 3d 分布調整

**失敗分析：**

- **3 個 Part A SLs 未被 ceiling 過濾**（2019-07-24、2020-01-13、2021-11-11）——這些 SLs 之 signal-day 3d 範圍 2.66%-3.92%，遠低於任何合理 ceiling 閾值。失敗模式為「low conviction breakout」（非 rally exhaustion），須其他維度（regime 或外生因素）才能識別
- **Part B 唯一 SL（2024-05-14）為「weak breakout」失敗模式**（1d 2.77% / 3d 4.70% / 5d 4.07% 全為 Part B 最弱），與 Part A SL「extended rally」反向——floor filter 可捕捉但會誤殺 Part B TPs（2025-06-02 1d 4.34% 為 Part B 最弱 winner）
- **A/B cum gap 結構性問題未解**：FCX 商品超級週期使 Part A 2020-2021 高報酬無法被技術面 filter 改善

**結論：FCX-014 Att1 為 FCX 突破策略前任最佳，min(A,B) 0.64 vs FCX-013 Att3 的 0.55（+16%），repo 首次 lesson #19 family ceiling 跨策略至 BB Squeeze Breakout 框架；threshold sweep 確認 [11%, 12%] robust sweet spot 區間。** **FCX-014 Att1 已被 FCX-015 Att2 進一步超越（min(A,B)† 1.43，A/B cum gap 從 52.5% 收斂至 7.1%）**，但 FCX-014 的 3d ceiling 仍為 FCX-015 base 之必要組件。

---

## FCX-015: VIX BANDS Filter on BB Squeeze Breakout（3 次嘗試，Att2 SUCCESS VIX FLOOR > 14）★ 突破策略當前最佳

**實驗目的：** 將 XBI-017 Pullback MR 框架成功的 ^VIX BANDS regime gate 跨策略移植至 FCX-014 Att1 BB Squeeze Breakout 框架；目標突破 FCX-014 Att1 0.64 並改善 A/B 累計報酬差距 52.5%（FCX 商品超級週期結構性邊界）。**Repo 第 2 次 lesson #24 family BANDS 變體** + **首次 BANDS 變體於 BB Squeeze Breakout 框架** + 反向假說發現後 **repo 首次 ^VIX FLOOR 變體於任何資產**。

**基準：** FCX-014 Att1（BB Squeeze + 3d ceiling +12% + lesson #22 SMA regime；min(A,B) 0.64，A/B cum gap 52.5% > 30% ❌）。

### Att1（mode=bands_exclude_mid, vix_low=17, vix_high=22；XBI-017 sweet spot 直接移植，FAIL）

**配置：** 通過條件 = `VIX <= 17.0 OR VIX > 22.0`（排除中等 VIX 帶 17~22）。

**結果：**
- Part A: 13 訊號 / WR 69.2% / Sharpe **0.52** / cum +50.33% / MDD -10.09%
- Part B: 3 訊號 / WR 66.7% / Sharpe 0.41 / cum +8.31%
- min(A,B) **0.41**（-36% vs FCX-014 Att1 0.64）
- A/B 訊號比 4.33:1（gap 76.9% > 50% ❌），A/B 累計差 83.5%（>30% ❌）
- **三項 acceptance criteria 全部 FAIL**

**失敗分析（trade-level）：**
- 過濾 3 Part A TPs（VIX 18-21 mid 帶）：2021-04-26 / 2021-12-22 / 2023-01-06
- 全 3 Part A SLs **保留**（2019-07-24 / 2020-01-13 / 2021-11-11→shifted 11-12）
- 同樣過濾 1 Part B TP（2025-06-02 +8% TP）
- **核心發現**：FCX BB Squeeze 突破與 XBI MR 為**反向 VIX 分布結構**——FCX TPs 集中於 mid-VIX，SLs 在 low-VIX；XBI 為相反

### Att2 ★ SUCCESS（mode=floor, vix_low=14.0；反向假說）

**配置：** 通過條件 = `VIX > 14.0`（VIX FLOOR 過濾 low-VIX calm-period false breakouts）。

**結果：**
- Part A: 10 訊號 / WR **90.0%** / Sharpe **1.43** / cum +85.63% / MDD -10.09%
- Part B:  3 訊號 / WR **100%** / std=0 / Sharpe 0.00 / cum +25.97%（3 TPs 全勝）
- Part C (Live): 1 SL（2026-02-24，與 FCX-014 相同）
- min(A,B)† **= Part A Sharpe 1.43**（沿用 EWJ-003/SPY-009/DIA-012/IWM-013/CIBR-014 慣例：Part B std=0 結構性零方差，採 Part A Sharpe 為 min 約束），**+123% vs FCX-014 Att1 0.64**
- A/B 年化 cum 13.18%/yr vs 12.24%/yr → gap **7.1% << 30% ✓✓✓**（vs FCX-014 52.5% ❌）
- A/B 訊號比 2.0/yr vs 1.5/yr → 1.33:1 (gap 25% < 50% ✓)
- **三項 acceptance criteria 全部達標**

**過濾分析：**
- **過濾 3 Part A SLs（全部）+ 1 Part B SL**：
  * 2019-07-24 SL（VIX ~13，late summer trade-war calm）
  * 2020-01-13 SL（VIX ~12.6，pre-COVID complacency）
  * 2021-11-11 SL → shifted 11-12 也被過濾（VIX ~17 邊界區）
  * 2024-05-14 Part B SL（VIX ~13）
- **副作用**：過濾 2 calm-period TPs（2019-04-04 / 2019-11-07）+ 2 expiries（2023-07-13 -3.69%、2023-12-13 +5.32%，淨接近 0）
- 淨效果：Part A 16→10 訊號（-37.5%），WR 75%→90%、Sharpe 0.69→1.43；Part B 4→3 訊號，WR 75%→100%

**核心發現：**
- **FCX 殘餘 SLs 集中於 low-VIX calm regime**：FCX BB Squeeze breakout 在低 VIX 時為 false breakout——市場 calm 但 FCX 自身突破缺乏 broad-market vol 證實（無系統性風險擴散助推）
- **^VIX FLOOR 為 surgical filter**：全部 Part A/B SLs 均位於 VIX <= 14 帶，無「FLOOR 同時誤殺 winners」副作用
- **A/B cum gap 結構性突破**：FCX 商品超級週期結構性 A/B gap 邊界（FCX-014 52.5%、FCX-013 Att3 44.0%）首次降至 7.1%——VIX FLOOR 過濾 calm-period 邊際 trades（2 calm TPs + 2 expiries）系統性平衡 Part A 過度報酬至 Part B 健康水準

### Att3（mode=floor, vix_low=15.0；threshold robustness check）

**配置：** 通過條件 = `VIX > 15.0`（FLOOR 加嚴 1pt 測試甜蜜點邊界）。

**結果：**
- Part A: 10 訊號 / WR 90.0% / Sharpe 1.43 / cum +85.63% — **完全相同 Att2**
- Part B: 2 訊號 / WR 100% / std=0 / cum +16.64%（**-1 TP vs Att2 3 訊號**，過濾 2024-04-29 Part B TP，VIX 介於 14-15 之間）
- min(A,B)† **= Part A 1.43**（與 Att2 相同），但 A/B 年化 cum 13.18%/yr vs 8.00%/yr → gap **39.3% > 30% ❌**（A/B balance criterion 退化）

**結論：** FLOOR 15.0 對 Part A 非綁定（2019-04-04 / 2019-11-07 已在 14 處過濾，加嚴無新效果），但對 Part B 過嚴損失 1 winner，A/B cum gap 從 7.1% 退至 39.3%。**確認 Att2 FLOOR 14.0 為甜蜜點：稍嚴（15）損失邊際 winner，A/B cum gap 重回 >30%；Att1 BANDS 模式則系統性錯誤方向。**

**跨資產發現（lesson #24 family v2 + 反向 VIX 分布結構）：**
1. **lesson #24 BANDS 變體不可從 XBI 跨策略直接移植至 FCX BB Squeeze Breakout**（Att1 已證實）：跨策略移植需先做 trade-level signal-day VIX 分布檢驗
2. **FCX vs XBI VIX 分布結構性反向**：FCX BB Squeeze breakout SLs 集中 low-VIX calm regime；XBI Pullback MR SLs 集中 mid-VIX complacency creep
3. **Repo 首次 lesson #24 family FLOOR 變體於任何資產**——擴展 lesson #24 family 為 5 種變體：LEVEL CAP（TLT-013 ^MOVE）/ DIRECTION（XLU-013、GLD-015、USO-025）/ BANDS exclude mid（XBI-017）/ **FLOOR（FCX-015，commodity/mining single stock 突破策略）**
4. **新跨資產假設（待驗證）**：VIX FLOOR 可能適用於其他高 vol mining/commodity 突破策略（COPX-011 銅礦 ETF、NVDA-012 AI 個股、TSLA-015 EV 個股、SOXL-012 半導體 leveraged ETF），閾值依該資產 SL 之 VIX 分布調整

**結論：FCX-015 Att2 為 FCX 突破策略新全域最佳，min(A,B)† 1.43 vs FCX-014 Att1 的 0.64（+123%），repo 首次 ^VIX FLOOR 變體 + 首次 lesson #24 BANDS 變體於 BB Squeeze Breakout 框架；A/B cum gap 從 52.5% 收斂至 7.1% 為 FCX 商品超級週期結構性邊界首次突破。**

---

## FCX-016: Post-Parabolic 長窗 prior-Nd-return CEILING（3 次迭代全部 FAIL/REJECT，predict→confirm documented-failure 正確預測）

**實驗目的：** 跨資產移植 **URA-014 Att1 ★ SUCCESS** 的「post-parabolic 長窗 prior-Nd-return CEILING」regime gate（lesson #19 family v3，long-horizon 後拋物線排除）至 FCX-015 Att2（FCX 全域最優 min(A,B)† = Part A Sharpe **1.43**）框架之上，目標外科式移除其唯一殘餘 binding Part A SL 2021-11-11。memory/URA-014 明列 **FCX / COPX / SOXL / SIVR** 為候選跨資產移植標的（COPX/SOXL/SIVR 在 avoid/remote-churn lane，FCX 為唯一可測）——「但僅當 binding residual SL 的 prior-Nd-return 為相對 winners 的 separable plateau outlier；須先以 predict→confirm 驗證，若 interleaved → documented-failure」。

**基準：** FCX-015 Att2（VIX>14 FLOOR BB Squeeze Breakout；Part A 10/90%/Sharpe 1.43/+85.63%，Part B 3/100%/std=0/+25.97%，min(A,B)† = Part A 1.43，A/B cum gap 7.1%）。

### 強制 predict→confirm 前置分析（DECISIVE，先於建構執行）

重現 FCX-015 Att2 精確交易：Part A 10 筆（WR 90%，唯一輸家 = **2021-11-11 -7.14% SL**，single characterizable residual binding SL）；Part B 3 筆（WR 100%）。對 9 個 Part A winners 計算 signal-day prior-Nd cumulative return（FCX 自身 `Close.pct_change(N)`）：

| 維度 | SL 2021-11-11 | winners (n=9) 區間 | SL vs winners |
|------|---------------|--------------------|----------------|
| Ret20 | +10.15% | [+3.8%, +19.4%] | 低於 max 9.25pp，mid-pack（5低/4高）→ **INTERLEAVED** |
| Ret30 | +25.99% | [+2.4%, +43.3%] | 低於 max 17.26pp → **INTERLEAVED** |
| **Ret60** | **+20.97%** | **[+3.4%, +62.6%]** | **低於 max 41.59pp；9 winners 有 7 個 Ret60 更高 → 方向 INVERTED** |
| Ret90 | +13.43% | [-18.5%, +125.3%] | 第 3 低 → **INTERLEAVED** |
| Ret120 | -1.20% | [-19.1%, +126.4%] | mid-low → **INTERLEAVED** |
| Ret252 | +110.77% | [-0.3%, +361.5%] | mid-pack → **INTERLEAVED** |

**每一條 prior-Nd 窗 SL 均與 winners 完全 interleaved，無任何 separable plateau。** 在 URA-014 關鍵維度 Ret60，FCX 之 SL 為結構性 **INVERSE**：URA-014 之 SL 2021-11-16 為全場 Ret60 **最高**（+51.39%，高於全部 winners 21.80pp，>=20pp robust plateau，fresh parabolic blow-off）；FCX 之 SL 2021-11-11 反而 **mid/low**，而 FCX 最大 prior-60d run-ups（+55%~+63%）**全部是 winners**（2020-07-06/08-05/12-30 等 COVID V-recovery breakouts）。**結構性 ex-ante 機制：** FCX/銅 2021-05 已見頂（~$46），2021-10/11 為盤整/下行；2021-11-11 prior-60d +20.97% 是對 2021-08/09 回檔的 recovery bounce，非新鮮拋物線 blow-off。對 **突破策略 + copper-supercycle commodity miner**，高 prior-Nd run-up 是 **productive 動能延續 regime**，與 URA **MR 框架**「高 prior-Nd run-up = 抄底拋物線崩解陷阱」結構性相反。**predict→confirm 結論：DOCUMENTED-FAILURE（方向 INVERTED，無 separable plateau）。**

### Att1（runup_ceiling = +0.21，mid；剛低於 SL Ret60 +20.97% 的最緊值，FAIL）

**配置：** 通過條件 = base + `prior-60d cumulative return <= +0.21`。

**結果：**
- Part A: **4 訊號** / WR **75.0%** / Sharpe **0.64** / cum +16.98%（vs FCX-015 Att2 10/90%/1.43/+85.63%）
- Part B: 2 訊號 / WR 100% / std=0 / cum +16.64%
- min(A,B)† **= Part A 0.64**（**-55% vs 1.43**）
- **SL 2021-11-11 RETAINED**（Ret60 +20.97% ≤ +0.21 通過）；6 個高 Ret60 winners KILLED（2020-07-06/08-05/11-03/12-30、2021-04-26、2023-01-06）→ 方向完全 INVERTED（SIVR-019/DIA-013-class inversion）
- **FAIL**（predict→confirm 預測 CONFIRMED）

### Att2（runup_ceiling = +0.45，looser robustness；FAIL）

**配置：** 通過條件 = base + `prior-60d cumulative return <= +0.45`。

**結果：**
- Part A: 6 訊號 / WR 83.3% / Sharpe **0.97** / cum +36.44%（**-32% vs 1.43**）
- Part B: 3 訊號 / WR 100% / std=0 / cum +25.97%（與 FCX-015 Att2 相同）
- min(A,B)† **= Part A 0.97**
- **SL 2021-11-11 RETAINED**（Ret60 +20.97% ≤ +0.45，gate 對 SL **non-binding**）；僅誤殺 4 個最大 Ret60 winners（2020-07-06 +58.78 / 2020-08-05 +55.57 / 2020-12-30 +62.56 / 2023-01-06 +45.65）
- **FAIL**：單調 Att1 0.64 → Att2 0.97，放寬愈不糟但 SL 永不被移除（一旦門檻寬到留住 winners）→ **無任何門檻能同時移除 SL 並保留 winners → 無 separable plateau**（對照 URA-014 之 >=20pp robust plateau）

### Att3（runup_ceiling = +0.15，tighter cliff；REJECT）

**配置：** 通過條件 = base + `prior-60d cumulative return <= +0.15`。

**結果：**
- Part A: **2 訊號** / WR 100% / std=0（僅 2 個最低 Ret60 winners 存活：2019-12-09 Ret60 +13.08、2020-06-02 +3.39）/ cum +16.64%
- Part B: 1 訊號 / WR 100% / std=0 / cum +8.00%
- SL 2021-11-11（Ret60 +20.97% > +0.15）此時被移除，但同時移除 7 個 winners（degenerate over-filter）
- A/B cum gap |16.64-8.00|/16.64 = **52% >> 30% ❌**
- **REJECT**：統計上無意義（n=2 Part A / n=1 Part B），degenerate over-filter（EEM-016/CIBR-016/INDA-013 over-filter rigor precedent）；唯一能移除 SL 的方式（ceiling < +20.97%）必然同時殺死 7/9 winners

**跨資產發現（lesson #19 family v3 邊界 — long-horizon prior-return CEILING 的首個跨資產 documented-failure）：**
1. **URA-014 的 post-parabolic 長窗 prior-Nd-return CEILING 不可移植至 copper-supercycle commodity miner 之 BREAKOUT 策略**：URA-014 之成功前提是「binding residual SL 為 fresh parabolic blow-off（Ret60 全場最高 + >=20pp robust plateau）」；FCX-015 Att2 之 SL 2021-11-11 為 mid/low Ret60、與 winners 完全 interleaved，方向 INVERTED
2. **框架×資產交互**：對 **MR 框架**（URA capitulation），高 prior-Nd run-up 標記「拋物線崩解中的假抄底」（productive CEILING）；對 **突破策略**（FCX BB Squeeze），高 prior-Nd run-up 標記「動能延續的優質突破」（CEILING 反而誤殺最佳 winners）——同一 lesson #19 family v3 維度在兩種框架方向相反
3. **predict→confirm 軌跡延伸**：3 SUCCESS（GLD-016/VOO-005/URA-014）+ **10** documented-failure 全部正確預測；memory 之「FCX 為 URA-014 候選港，須先驗 prior-Nd separability，若 interleaved → documented-failure」caveat 直接命中
4. **URA-014 候選跨資產移植清單收斂**：FCX 已測（documented-failure，方向 INVERTED）；COPX/SOXL/SIVR 在 avoid/remote-churn lane → URA-014 long-horizon CEILING 港的 commodity/mining 候選實質耗盡（除非未來資產之 binding SL 經 predict→confirm 確認為 separable post-parabolic outlier）

**結論：FCX-016 三次迭代全部 FAIL/REJECT，未超越 FCX-015 Att2 min(A,B)† 1.43。predict→confirm 前置分析正確預測 documented-failure（方向 INVERTED，無 separable plateau）。FCX-015 Att2 維持 FCX 突破策略全域最優。FCX 之 12 大失敗策略類型新增第 13 類「long-horizon prior-return CEILING（跨資產移植 URA-014 lesson #19 family v3）」。**
