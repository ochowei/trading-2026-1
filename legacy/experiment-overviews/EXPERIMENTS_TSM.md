<!-- AI_CONTEXT_START - 此區塊供 AI Agent 快速讀取，人工更新
  ...
  phase9_note: TSM-007, TSM-008, TSM-011, TSM-012, TSM-016, TSM-017, TSM-018, TSM-019, and TSM-022 now declare TSM primary plus their SMH/volatility auxiliary data through the verified snapshot bundle; migration runs remain migration-pending and do not advance latest or followup qualification.
  phase9_note_2026_08_06_tsm: TSM-009, TSM-013, TSM-014, TSM-015, TSM-020, and TSM-021 now consume declared NVDA/SMH/QQQ/AAPL/SOXX auxiliary bundles through the provider-free execution boundary; migration runs remain migration-pending and do not advance latest or followup qualification.
  phase9_note_2026_08_07_primary: TSM-001 through TSM-005 and TSM-010 now consume declared primary TSM bundles through the shared execution boundary; TSM-006 was already migrated and auxiliary declarations remain unchanged. Formal migration results remain pending requalification.
  phase9_evidence_2026_08_07: This TSM primary-only wave is contract-covered; no new fixed-snapshot parity artifact has been promoted, so latest results and followup qualification remain unchanged.
  note_2026_05_10_tsm021: TSM-021 added 2026-05-10 (QQQ Macro-Health Gate on RS Momentum Pullback, **★ NEW GLOBAL OPTIMUM — repo 第 2 次 lesson #25 broad-market context confirmation gate 跨資產驗證（繼 IWM-015 後）+ repo 首次跨策略框架延伸 (capitulation MR → RS Momentum Pullback) + repo 首次半導體 ADR 個股 + cross-asset macro context filter 成功 + TSM 結構性 0.83 ceiling 首次突破**, cross-strategy mirror extension from IWM-015 — 經 TSM-013/014/015/016/017/018/019/020 共 8 次 cross-asset macro filter 試驗 REJECT/PARTIAL 後首次成功). Three iterations: **Att1 (FLOOR direction，QQQ 5d >= -3%) REJECT** min(A,B) **0.78** (-6% vs baseline 0.83) — Part A 11/81.8%/Sharpe 0.78 cum +61.20% / Part B 10 訊號完全不變 0.83；過濾 2022-12-07 SL ✓ 但 cooldown chain shift 將 2022-01-05 winner 替換為 2022-01-13 SL（淨退化），同時 Part B 全部訊號 QQQ_5d >= -3% 結構性非綁定（包含 2024-07-16/2024-10-30 兩 SLs）；證實 5d 短時框 + FLOOR 方向不適用 RS Momentum framework；**Att2 ★ (CEILING direction，QQQ 10d <= +2%) SUCCESS** Part A **9/88.9% WR/Sharpe 1.14** cum +61.73% MDD -8.42% PF **8.12** max consec losses **1** (vs baseline 12/83.3%/0.86，+33% Sharpe / +5.6pp WR) / Part B **8/87.5% WR/Sharpe 1.23** cum +59.23% MDD -7.77% PF **7.90** max consec losses **1** (vs baseline 10/80%/0.83，**+48% Sharpe / +7.5pp WR**) / min(A,B) **1.14 ★ NEW GLOBAL OPTIMUM** (**+37% vs baseline 0.83，TSM 結構性 0.83 ceiling 首次突破於 12 次實驗、43+ 次嘗試後**) / **A/B 累計報酬差距 |61.73-59.23|/61.73 = 4.0%** ✓ (<<30% 大幅優於 baseline 19.3%) / **A/B 訊號數差距 (9-8)/9 = 11.1%** ✓ (<<50% 與 baseline 16.7% 同等). 過濾結果：✓ 2022-12-07 Part A SL（QQQ_10d < +2% 在 2022 Q4 bear 環境）✓ 2022-11-21 Part A SL → cooldown chain shift 至 2022-11-28 SL（同質替換）✓ **2024-07-16 Part B SL 過濾**（關鍵改善，QQQ_10d > +2% 在 Trump-Taiwan 政治風險前 broad-market 已過熱）✗ 2025-05-08 Part B winner / 2021-01-26 / 2023-01-19 Part A winners 誤殺（acceptable trade-off）；**Att3a (CEILING +1% 加嚴 robustness) REJECT min 0.83 = baseline TIE**（過嚴 over-filter Part A 12→7 大量 winners）；**Att3b (CEILING +3% 放鬆 robustness) REJECT min 0.74**（放回 2024-07-16 SL，confirms +2% 為甜蜜點下界）。**核心成功發現（lesson #25 family v2 cross-strategy 鏡像擴展）**：(1) **Repo 第 2 次 lesson #25 broad-market context confirmation gate 跨資產驗證**（繼 IWM-015 後）；(2) **首次跨策略框架延伸**：lesson #25 從 capitulation MR (IWM-015 CEILING：要求 broad-market 已 confirmed risk-off → 利用 systemic V-bounce) 鏡像至 RS Momentum Pullback (TSM-021 CEILING：要求 broad-market 未過熱 → 排除 broad-market rally exhaustion 環境)，**同方向 (CEILING) 但機制相反**——MR 利用 systemic mean-reversion，Momentum 排除 systemic exhaustion；(3) **首次半導體 ADR 個股 + cross-asset macro filter 成功**——TSM-013/014/015/016/017/018/019/020 共 8 次 cross-asset macro filter 試驗（QQQ CEILING / QQQ BAND / AAPL anchor / BB-Width / earnings exclusion / ATR BAND / VIX term structure / SOXX sector anchor）全部 REJECT/PARTIAL，TSM-021 為首次達 min(A,B) > 0.83 突破；(4) **TSM 結構性 0.83 ceiling 首次突破**（TSM-008/011/013-020 經 12 次實驗、43+ 次嘗試達 ceiling）；(5) **新跨資產假設 (lesson #25 family v2 適用候選)**：broad-market context CEILING gate (QQQ N-day return <= +X%) 可能適用其他 RS / MBPC momentum 框架（NVDA-013、SOXL-010、VOO-004、CIBR-015）— TSM-021 為首例，閾值需依資產對 broad-market 耦合程度調整。TSM-021 Att2 為新全域最優（21 次實驗、46+ 次嘗試），取代 TSM-011 Att3。
...
**當前最佳：** ★ **TSM-021 Att2**（QQQ Macro-Health Gate on RS Momentum Pullback：TSM-011 Att3 完整框架 + **QQQ 10 日報酬 <= +2%** broad-market context CEILING 確認閘門，TP+8%/SL-7%/25天/cd 10）★ **2026-05-10 新全域最優（21 次實驗、46+ 次嘗試），TSM 結構性 0.83 ceiling 首次突破**
- Part A: Sharpe **1.14**, 累計 +61.73%, 9 訊號 (1.8/年), WR **88.9%**, MDD -8.42%, PF 8.12, max consec losses **1**
- Part B: Sharpe **1.23**, 累計 +59.23%, 8 訊號 (4.0/年), WR **87.5%**, MDD -7.77%, PF 7.90, max consec losses **1**
- min(A,B) **1.14**（**+37% vs TSM-011 Att3 baseline 0.83，TSM 結構性 0.83 ceiling 首次突破於 12 次實驗、43+ 次嘗試後**）
- **A/B 累計差 4.0%**（<<30% ✓ 大幅優於 baseline 19.3%），**A/B 訊號數差距 11.1%**（<<50% ✓ 與 baseline 16.7% 同等）
- 關鍵改善：QQQ_10d CEILING +2% 過濾 2024-07-16 Part B SL（broad-market QQQ 已過熱 >+2%）+ 2022-12-07 / 2022-11-21 Part A SLs；cooldown chain shift 將 2022-11-21 替換為 2022-11-28 SL（同質替換）；副作用為 2025-05-08 / 2021-01-26 / 2023-01-19 winners 誤殺（acceptable trade-off）
- **跨資產貢獻**：(1) repo 第 2 次 lesson #25 broad-market context confirmation gate 跨資產驗證（繼 IWM-015 後）；(2) **首次跨策略框架延伸**：lesson #25 從 capitulation MR (IWM-015 CEILING：要求 broad-market 已 confirmed risk-off → 利用 systemic V-bounce) 鏡像至 RS Momentum Pullback (TSM-021 CEILING：要求 broad-market 未過熱 → 排除 broad-market rally exhaustion 環境)，**同方向但機制相反**；(3) **首次半導體 ADR 個股 + cross-asset macro filter 成功**——TSM-013/014/015/016/017/018/019/020 共 8 次 cross-asset macro filter 試驗 REJECT/PARTIAL 後，TSM-021 為首次達 min(A,B) > 0.83 突破；(4) **新跨資產假設（lesson #25 family v2）**：broad-market context CEILING gate 可能適用其他 RS / MBPC momentum 框架（NVDA-013、SOXL-010、VOO-004、CIBR-015）

**前任最佳：** TSM-011 Att3（Signal-Day Direction Filter on RS Momentum Pullback：TSM-008 完整框架 + **5 日報酬 ceiling <= +10.5%** rally exhaustion filter，TP+8%/SL-7%/25天/cd 10）— Part A Sharpe 0.86 cum +74.10%, Part B Sharpe 0.83 cum +59.78%, min(A,B) 0.83（+5% vs TSM-008 baseline 0.79），A/B 累計差 19.3% / 訊號比 1.2:1（gap 16.7%）

**前前任最佳：** TSM-008（RS 出場優化：同 TSM-007 進場，TP+8%/SL-7%/25天）— Part A Sharpe 0.79/Part B 0.83，min(A,B) 0.79，A/B 訊號數 12/10，A/B gap 0.04（極佳平衡）
...
| TSM-007 | `tsm_007_relative_strength`    | 相對強度動量回調（TSM vs SMH+回撤+SMA50，Phase 9 auxiliary bundle）| 前最佳 |
| TSM-008 | `tsm_008_rs_exit_optimization` | RS 出場優化（同 TSM-007 進場，TP+8%/SL-7%/25天，Phase 9 auxiliary bundle）| 前最佳 |
| TSM-009 | `tsm_009_pairs_trading`        | 配對交易 TSM/NVDA z-score 均值回歸（3次嘗試均失敗） | 失敗；Phase 9 auxiliary bundle，migration-pending |
...
| TSM-013 | `tsm_013_qqq_divergence_rs`     | TSM-QQQ 20d Cross-Asset Divergence CEILING Regime-Gated RS Momentum Pullback（**repo 第 6 次 cross-asset divergence regime gate 跨資產移植、首次半導體 ADR 個股 + RS Momentum Pullback 框架**，Att1 PARTIAL：Part A 9/100% zero-var all TPs / Part B unchanged → min TIE baseline 0.83，Att2/Att3 REJECT） | ⚠️ 部分成功；Phase 9 auxiliary bundle，migration-pending |
| TSM-014 | `tsm_014_qqq_divergence_band`   | TSM-QQQ 20d Cross-Asset Divergence **BAND** Regime-Gated RS Momentum Pullback（**repo 首次 cross-asset divergence BAND 變體於任何資產**，FLOOR + CEILING 雙向，3 次迭代全部 REJECT/TIE，min(A,B) 最佳 0.83 TIE / 失敗根因 Part B SLs Rel_QQQ 落於 winners 分布中段非極端） | ❌ 失敗；Phase 9 auxiliary bundle，migration-pending |
| TSM-015 | `tsm_015_aapl_divergence_rs`    | TSM-AAPL 20d Cross-Asset Divergence Regime-Gated RS Momentum Pullback（**repo 首次 AAPL 主要客戶 anchor 試驗於任何資產 + repo 首次同一 target 雙 anchor stack 結構（QQQ + AAPL）**，3 次迭代全部 REJECT/TIE，min(A,B) 最佳 0.83 TIE / 失敗根因 AAPL FLOOR -7% non-binding + AAPL CEILING +5% over-filter Part A winners） | ❌ 失敗；Phase 9 auxiliary bundle，migration-pending |
...
| TSM-020 | `tsm_020_soxx_divergence_rs`    | TSM-SOXX 20d Cross-Asset Divergence CEILING Regime-Gated RS Momentum Pullback（**repo 首次「sector-internal anchor」變體於任何資產**，3 次迭代全部 REJECT/TIE，min(A,B) 最佳 0.83 TIE / 失敗根因 Part B SLs 在 TSM-SOXX 維度落於 winners 分布中段） | ❌ 失敗；Phase 9 auxiliary bundle，migration-pending |
| TSM-021 | `tsm_021_qqq_macro_health_gate` | QQQ Macro-Health Gate on RS Momentum Pullback（**repo 第 2 次 lesson #25 broad-market context confirmation gate 跨資產驗證 + 首次跨策略框架延伸 (capitulation MR → RS Momentum) + TSM 結構性 0.83 ceiling 首次突破**：QQQ 10d <= +2% CEILING，Att2 SUCCESS min(A,B) **1.14 (+37% vs 0.83)**, A/B 平衡達標） | **★ 最佳**；Phase 9 auxiliary bundle，migration-pending |
| TSM-022 | `tsm_022_vxn_implied_vol_rs`    | ^VXN forward-looking implied-vol DIRECTION gate on RS（lesson #24 family，3次迭代均失敗 0.64/0.20/-0.09） | 失敗 |
...
## TSM-022：^VXN Forward-Looking Implied-Vol DIRECTION Regime Gate（3 次迭代均失敗）
...
**結論**：TSM-011 Att3（min 0.83）仍為全域最優（13 次實驗、36+ 次嘗試）。TSM 純技術面/regime-classifier 路徑已窮盡（八大策略類型）；若要突破 0.83 需 event-driven 結構建模（地緣政治事件日曆、客戶 earnings 事件窗口），非單一 signal-day / regime-gate 維度可達。
