"""
FCX-004 訊號偵測器：Bollinger Band Squeeze Breakout
FCX-004 Signal Detector: BB Squeeze Breakout
...
from trading.experiments.fcx_004_bb_squeeze_breakout.config import FCXBBSqueezeConfig

logger = logging.getLogger(__name__)


class FCXBBSqueezeDetector(BaseSignalDetector):
    """FCX BB Squeeze Breakout 訊號偵測器"""

    def __init__(self, config: FCXBBSqueezeConfig):
        self.config = config
    ...
    def detect_signals(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        logger.info("FCX: Detected %d BB squeeze breakout signals", signal_count)
        return df
