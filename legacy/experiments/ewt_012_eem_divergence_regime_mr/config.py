"""
EWT-012: EWT–EEM Cross-Asset Divergence Regime-Gated MR
...
@dataclass
class EWT012Config(ExperimentConfig):
    """EWT-012 EWT–EEM Cross-Asset Divergence Regime-Gated MR 參數"""
    ...
    # ===== EWT-012 核心新增：EWT–EEM 跨資產 divergence regime gate =====
    ...
    # 預設 = Att1（documented-failure，最乾淨 separator 嘗試；
    # 沿用 SIVR-019/GLD-016 latest.json = Att1 慣例）
    divergence_threshold: float = 0.0  # Att1 預設：2d CEILING ≤ 0.0%


def create_default_config() -> EWT012Config:
    return EWT012Config(
        name="ewt_012_eem_divergence_regime_mr",
        experiment_id="EWT-012",
