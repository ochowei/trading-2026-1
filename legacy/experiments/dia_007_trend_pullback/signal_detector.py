"""
DIA-007 訊號偵測器：Trend Pullback to SMA(50)
DIA-007 Signal Detector: Trend Pullback to SMA(50)

進場條件（全部滿足）：
1. SMA(50) > SMA(200)（黃金交叉，確認上升趨勢）
2. SMA(50) > SMA(50)[10日前]（SMA(50) 仍在上升，趨勢健康）
3. Low <= SMA(50)（當日最低價觸及 SMA(50)，回檔測試支撐）
4. Close > SMA(50)（收盤反彈站回 SMA(50) 上方）
5. 冷卻期 15 個交易日
"""
...
from trading.experiments.dia_007_trend_pullback.config import DIA007TrendPullbackConfig

logger = logging.getLogger(__name__)


class DIA007TrendPullbackDetector(BaseSignalDetector):
    """DIA Trend Pullback 訊號偵測器"""

    def __init__(self, config: DIA007TrendPullbackConfig):
        self.config = config
    ...
    def detect_signals(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        # 2. SMA(50) is rising (healthy trend, not flattening/reversing)
        cond_rising = df["SMA_Fast"] > df["SMA_Fast_Prev"]

        # 3. Pullback to SMA(50): Low dipped to within proximity of SMA(50)
        ...
        logger.info("DIA-007: Detected %d trend pullback signals", signal_count)
        return df
