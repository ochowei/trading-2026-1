"""
IWM-008 訊號偵測器：Bollinger Band Squeeze Breakout 優化版
IWM-008 Signal Detector: BB Squeeze Breakout Optimized

進場條件（全部滿足）：
1. 過去 5 日內 BB Width 曾低於 60 日 30th 百分位（近期波動收縮，vs IWM-006 的 25th）
2. 收盤價 > Upper BB(20,2)（突破上軌）
3. 收盤價 > SMA(50)（趨勢向上）
4. 冷卻期 10 個交易日（vs IWM-006 的 15 天）
"""
...
class IWM008BBSqueezeDetector(BaseSignalDetector):
    """IWM BB Squeeze Breakout 優化訊號偵測器"""

    def __init__(self, config: IWM008BBSqueezeConfig):
        self.config = config
    ...
    def detect_signals(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        logger.info("IWM-008: Detected %d BB squeeze breakout signals", signal_count)
        return df
