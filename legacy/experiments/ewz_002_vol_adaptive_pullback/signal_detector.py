"""
EWZ-002 訊號偵測器：波動率自適應回檔均值回歸

進場條件（全部滿足）：
1. 10 日高點回檔 >= 7%（深度過濾）
2. 10 日高點回檔 <= 10%（隔離極端崩盤）
...
from trading.experiments.ewz_002_vol_adaptive_pullback.config import EWZ002Config

logger = logging.getLogger(__name__)


class EWZ002SignalDetector(BaseSignalDetector):
    def __init__(self, config: EWZ002Config):
        self.config = config
    ...
    def detect_signals(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        logger.info("EWZ-002: Detected %d vol-adaptive pullback signals", signal_count)
        return df
