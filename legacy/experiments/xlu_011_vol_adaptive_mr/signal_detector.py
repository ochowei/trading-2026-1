"""
XLU-011 訊號偵測器：波動率自適應均值回歸

進場條件（全部滿足）：
1. 10 日回檔 >= 3.5% 且 <= 7%（同 XLU-003）
2. Williams %R(10) <= -80（超賣確認）
3. 收盤位置 >= 40%（日內反轉確認）
4. ATR(5) / ATR(20) > 1.15（波動率飆升過濾，1.1 與 1.2 之間的中間值）
5. 冷卻期 7 個交易日
"""
...
class XLUVolAdaptiveMRDetector(BaseSignalDetector):
    def __init__(self, config: XLUVolAdaptiveMRConfig):
        self.config = config
    ...
    def detect_signals(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        logger.info("XLU-011: Detected %d vol-adaptive MR signals", signal_count)
        return df
