"""
IWM-011 訊號偵測器：波動率自適應 RSI(2) 均值回歸

進場條件（全部滿足）：
1. RSI(2) < 10（極端超賣）
2. 2 日累計跌幅 >= 2.5%（幅度過濾）
3. 收盤位置 >= 40%（日內反轉確認）
4. ATR(5) / ATR(20) > 1.2（波動率飆升過濾）
5. 冷卻期 5 個交易日
"""
...
from trading.experiments.iwm_011_vol_adaptive_rsi2.config import IWM011Config

logger = logging.getLogger(__name__)


class IWM011SignalDetector(BaseSignalDetector):
    def __init__(self, config: IWM011Config):
        self.config = config
    ...
    def detect_signals(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        logger.info("IWM-011: Detected %d vol-adaptive RSI(2) signals", signal_count)
        return df
