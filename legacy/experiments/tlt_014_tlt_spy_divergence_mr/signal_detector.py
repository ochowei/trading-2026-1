"""
TLT TLT-SPY Cross-Asset Divergence Regime-Gated MR 訊號偵測器 (TLT-014)

進場條件（全部滿足，訊號日為 T，執行模型於 T+1 開盤進場）：
1. 10 日高點回檔 ≥ 3% 且 ≤ 7%（同 TLT-013 Att1）
2. Williams %R(10) ≤ -80（同 TLT-013 Att1）
3. 收盤位置 ≥ 40%（日內反轉，同 TLT-013 Att1）
4. BB(20, 2) 寬度 / Close < 5%（沿用 TLT-007 / TLT-013 backward-looking realized vol gate）
5. ^MOVE 收盤值 <= 130（沿用 TLT-013 Att1 forward-looking implied vol gate）
6. TLT N 日報酬 - SPY N 日報酬 >= min_relative_return（TLT-014 核心新增 cross-asset
   divergence gate，過濾 reflation/risk-on regime 中 TLT 嚴重落後 SPY 的訊號）
7. 冷卻期 7 天
"""

import logging

import pandas as pd

from trading.core.base_signal_detector import BaseSignalDetector
from trading.core.followup_data import DeclaredAuxiliaryData
from trading.experiments.tlt_014_tlt_spy_divergence_mr.config import TLT014Config

logger = logging.getLogger(__name__)


class TLT014SignalDetector(DeclaredAuxiliaryData, BaseSignalDetector):
    """TLT-014：BB-width + ^MOVE + TLT-SPY divergence regime gate MR"""

    def __init__(self, config: TLT014Config):
        self.config = config

    def auxiliary_symbols(self) -> tuple[str, ...]:
        return (self.config.move_ticker, self.config.benchmark_ticker)

    def compute_indicators(self, df: pd.DataFrame) -> pd.DataFrame:
        df = df.copy()

        # 回檔幅度
        n = self.config.pullback_lookback
        df["High_N"] = df["High"].rolling(n).max()
        df["Pullback"] = (df["Close"] - df["High_N"]) / df["High_N"]

        # Williams %R
        wr_n = self.config.wr_period
        highest = df["High"].rolling(wr_n).max()
        lowest = df["Low"].rolling(wr_n).min()
        hl_range = highest - lowest
        df["WR"] = (highest - df["Close"]) / hl_range.where(hl_range > 0, float("nan")) * -100
        df["WR"] = df["WR"].fillna(-50.0)

        # 收盤位置
        day_range = df["High"] - df["Low"]
        df["ClosePos"] = (df["Close"] - df["Low"]) / day_range
        df.loc[day_range == 0, "ClosePos"] = 0.5

        # BB-width regime gate
        bb_n = self.config.bb_period
        bb_k = self.config.bb_std
        sma = df["Close"].rolling(bb_n).mean()
        std = df["Close"].rolling(bb_n).std()
        df["BB_Upper"] = sma + bb_k * std
        df["BB_Lower"] = sma - bb_k * std
        df["BB_Width_Ratio"] = (df["BB_Upper"] - df["BB_Lower"]) / df["Close"]

        # TLT 自身 N 日報酬（給 cross-asset divergence 用）
        div_n = self.config.divergence_lookback
        df["TLT_Ret_N"] = df["Close"].pct_change(div_n)

        # ^MOVE forward-looking implied vol gate
        move_df = self.require_auxiliary(self.config.move_ticker, df.index)
        df["MOVE_Close"] = move_df["Close"]

        # SPY benchmark cross-asset divergence gate（TLT-014 核心）
        bench_df = self.require_auxiliary(self.config.benchmark_ticker, df.index)
        bench_close = bench_df["Close"]
        df["Bench_Close"] = bench_close
        df["Bench_Ret_N"] = bench_close.pct_change(div_n)
        df["Rel_Return_N"] = df["TLT_Ret_N"] - df["Bench_Ret_N"]

        return df

    def detect_signals(self, df: pd.DataFrame) -> pd.DataFrame:
        df = df.copy()

        cond_pullback_min = df["Pullback"] <= self.config.pullback_threshold
        cond_pullback_max = df["Pullback"] >= self.config.pullback_upper
        cond_wr = df["WR"] <= self.config.wr_threshold
        cond_reversal = df["ClosePos"] >= self.config.close_position_threshold
        cond_bb_regime = df["BB_Width_Ratio"] < self.config.max_bb_width_ratio
        cond_move_level = df["MOVE_Close"] <= self.config.max_move_level
        cond_divergence = df["Rel_Return_N"] >= self.config.min_relative_return

        df["Signal"] = (
            cond_pullback_min
            & cond_pullback_max
            & cond_wr
            & cond_reversal
            & cond_bb_regime
            & cond_move_level
            & cond_divergence
        )

        # 冷卻機制
        signal_indices = df.index[df["Signal"]].tolist()
        suppressed: list[pd.Timestamp] = []
        last_signal = None

        for idx in signal_indices:
            if last_signal is not None:
                gap = len(df.loc[last_signal:idx]) - 1
                if gap <= self.config.cooldown_days:
                    suppressed.append(idx)
                    continue
            last_signal = idx

        if suppressed:
            df.loc[suppressed, "Signal"] = False
            logger.info("TLT-014: %d signals suppressed by cooldown", len(suppressed))

        signal_count = df["Signal"].sum()
        logger.info("TLT-014: Detected %d cross-asset divergence-gated MR signals", signal_count)
        return df
