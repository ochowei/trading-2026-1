"""
SPY-008 訊號偵測器：Bollinger Band Squeeze Breakout
SPY-008 Signal Detector: BB Squeeze Breakout
...
from trading.experiments.spy_008_bb_squeeze_breakout.config import SPYBBSqueezeConfig

logger = logging.getLogger(__name__)


class SPYBBSqueezeDetector(BaseSignalDetector):
    """SPY BB Squeeze Breakout 訊號偵測器"""

    def __init__(self, config: SPYBBSqueezeConfig):
        self.config = config
    ...
    def detect_signals(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        logger.info("SPY: Detected %d BB squeeze breakout signals", signal_count)
        return df
