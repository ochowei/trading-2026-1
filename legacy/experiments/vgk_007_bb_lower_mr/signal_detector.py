"""
VGK-007 BB 下軌均值回歸訊號偵測器

在 BB(20,2.0) 下軌觸及時買入，搭配 -7% 回檔上限（崩盤隔離）+ WR + ClosePos + ATR 品質過濾。
相較於 VGK-004 的固定 3-7% 回檔範圍，BB 下軌是統計自適應門檻，
低波動期（Part B 2024-2025）淺門檻捕捉更多有效訊號。
"""
...
from trading.experiments.vgk_007_bb_lower_mr.config import VGK007Config

logger = logging.getLogger(__name__)


class VGK007SignalDetector(BaseSignalDetector):
    """
    VGK BB 下軌均值回歸訊號偵測器
    ...
    def __init__(self, config: VGK007Config):
        self.config = config
    ...
    def detect_signals(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        if suppressed:
            ...
            logger.info("VGK-007: %d signals suppressed by cooldown", len(suppressed))

        signal_count = df["Signal"].sum()
        logger.info("VGK-007: Detected %d BB Lower Band MR signals", signal_count)
        return df
