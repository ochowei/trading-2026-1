"""
EWT-004 訊號偵測器：2日急跌過濾 + 非對稱出場均值回歸
...
from trading.experiments.ewt_004_crash_filter_asymmetric.config import EWT004Config

logger = logging.getLogger(__name__)


class EWT004SignalDetector(BaseSignalDetector):
    def __init__(self, config: EWT004Config):
        self.config = config
    ...
    def detect_signals(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        logger.info("EWT-004: Detected %d crash-filtered pullback signals", signal_count)
        return df
