"""
COPX-005 訊號偵測器：Bollinger Band Squeeze Breakout
COPX-005 Signal Detector: BB Squeeze Breakout
...
from trading.experiments.copx_005_bb_squeeze_breakout.config import COPXBBSqueezeConfig

logger = logging.getLogger(__name__)


class COPXBBSqueezeDetector(BaseSignalDetector):
    """COPX BB Squeeze Breakout 訊號偵測器"""

    def __init__(self, config: COPXBBSqueezeConfig):
        self.config = config
    ...
    def detect_signals(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        if suppressed:
            ...
            logger.info("COPX: %d signals suppressed by cooldown", len(suppressed))

        signal_count = df["Signal"].sum()
        logger.info("COPX: Detected %d BB squeeze breakout signals", signal_count)
        return df
