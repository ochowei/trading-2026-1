"""
VOO Signal-Day Capitulation-Strength Filter MR 配置 (VOO-006)
...
(2) Part B 1/1 SL：3 日累計跌幅 -10.67%（regime-shift 級別深度急跌）
    ...
    - 注意：2025-04-07 1d -0.42%（淺於 -0.5%）已先被 1d floor 過濾，3d cap 在
      VOO-006 訊號集為 regime-shift 安全層（同 SPY-009 Att2 結論）
...
- VOO-006：1d FLOOR <= -0.5% AND 3d cap >= -8%（**同 SPY-009，驗證同指數
...
@dataclass
class VOO006Config(ExperimentConfig):
    """VOO-006 Signal-Day Capitulation-Strength Filter MR 參數"""
...
def create_default_config() -> VOO006Config:
    return VOO006Config(
        name="voo_006_signal_day_capitulation_mr",
        experiment_id="VOO-006",
