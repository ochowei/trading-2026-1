...
class TSMPairsTradingDetector(DeclaredAuxiliaryData, BaseSignalDetector):
    ...
    def compute_indicators(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        # The verified bundle supplies NVDA already aligned as-of to every
        # primary decision session.
        nvda_df = self.require_auxiliary(self.config.pair_ticker, df.index)

        # 計算對數價格比值（log ratio 更穩定）
        df["Price_Ratio"] = np.log(df["Close"] / nvda_df["Close"])
