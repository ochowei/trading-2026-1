"""
URA-014: Post-Parabolic Long-Horizon Regime-Gated Capitulation MR
Uses ExecutionModelBacktester (next-open + 0.1% slippage + pessimistic intrabar).
"""

from trading.core.base_config import ExperimentConfig
from trading.core.base_signal_detector import BaseSignalDetector
from trading.core.bundle_strategy import PrimaryBundleStrategyMixin
from trading.core.execution_strategy import ExecutionModelStrategy
from trading.experiments.ura_014_postparabola_regime_mr.config import (
    URA014Config,
    create_default_config,
)
from trading.experiments.ura_014_postparabola_regime_mr.signal_detector import (
    URA014SignalDetector,
)


class URA014Strategy(PrimaryBundleStrategyMixin, ExecutionModelStrategy):
    """URA Post-Parabolic Long-Horizon Regime-Gated Capitulation MR (URA-014)"""

    slippage_pct: float = 0.001

    bundle_trial_family = "URA:postparabola-regime-mr"
    bundle_trial_hypothesis = (
        "URA post-parabolic regime filtering can be reproduced from a verified primary bundle."
    )

    def create_config(self) -> ExperimentConfig:
        return create_default_config()

    def create_detector(self) -> BaseSignalDetector:
        return URA014SignalDetector(create_default_config())

    def _print_strategy_params(self, config: ExperimentConfig) -> None:
        if isinstance(config, URA014Config):
            print(
                f"  Pullback: {config.pullback_lookback}d high"
                f" >= {abs(config.pullback_threshold):.0%}"
                f", cap <= {abs(config.pullback_upper):.0%}"
            )
            print(f"  RSI({config.rsi_period}) < {config.rsi_threshold}")
            print(f"  2-Day Decline: <= {config.two_day_decline:.0%}")
            print(
                f"  ATR Band: {config.atr_ratio_floor} <= ATR({config.atr_short_period})"
                f"/ATR({config.atr_long_period})"
                f" <= {config.atr_ratio_ceiling}"
            )
            print(f"  {config.multi_period_lookback}d Return Cap: >= {config.multi_period_cap:.1%}")
            print(
                f"  Post-Parabola Gate: {config.runup_lookback}d return"
                f" <= {config.runup_ceiling:+.0%}"
            )
            print(f"  Cooldown: {config.cooldown_days} days")
        super()._print_strategy_params(config)
