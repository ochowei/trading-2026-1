"""
TSLA-003 訊號偵測器：緊密回撤範圍均值回歸
TSLA-003 Signal Detector: Tight Retracement Mean Reversion

進場條件（vs TSLA-002 差異）：
1. 收盤價低於 60 日高點 22-40%（收窄回撤範圍）
2. RSI(2) < 15（極端短期超賣）
3. 2日跌幅 >= 7%（提高恐慌門檻）
4. 冷卻期 10 個交易日
"""
...
class TSLATightRetracementDetector(BaseSignalDetector):
    """TSLA 緊密回撤訊號偵測器"""

    def __init__(self, config: TSLATightRetracementConfig):
        self.config = config
    ...
    def detect_signals(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        logger.info("TSLA: Detected %d tight retracement signals", signal_count)
        return df
