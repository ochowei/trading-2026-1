"""
TSM-010 訊號偵測器：Multi-Week Regime-Aware Momentum Breakout Pullback Continuation
...
設計依據：lesson #22（TSLA-015 / NVDA-012 / FCX-013 BB Squeeze 三次驗證、
NVDA-013 MBPC 第一次 cross-strategy 驗證）首次跨資產移植至 TSM。
"""
...
from trading.experiments.tsm_010_regime_mbpc.config import TSM010Config

logger = logging.getLogger(__name__)


class TSM010RegimeMBPCDetector(BaseSignalDetector):
    """TSM-010 訊號偵測器"""

    def __init__(self, config: TSM010Config):
        self.config = config
    ...
    def compute_indicators(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        # === 2-day return（lesson #19 family）===
        df["DD2"] = df["Close"].pct_change(2)
    ...
    def detect_signals(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        if self.config.use_dd2_cap:
            cond_dd2_cap = df["DD2"] >= self.config.dd2_cap_threshold
        else:
            cond_dd2_cap = pd.Series(True, index=df.index)
        ...
        if suppressed:
            ...
            logger.info(
                "TSM-010: %d signals suppressed by cooldown",
                len(suppressed),
            )

        signal_count = df["Signal"].sum()
        logger.info(
            "TSM-010: Detected %d regime-aware MBPC signals",
            signal_count,
        )
        return df
