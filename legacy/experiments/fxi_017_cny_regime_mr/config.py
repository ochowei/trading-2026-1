"""FXI–CNY (Yuan) Currency-Regime-Gated Mean Reversion (FXI-017)
...
- 空遠端 artifact `fxi_016_usdcnh_direction_mr` 獨立指向「FXI vs 離岸
  ...
  regime 代理），並以**獨立 module 名稱** `fxi_017_cny_regime_mr`
...
@dataclass
class FXI017Config(ExperimentConfig):
    """FXI-017 FXI–CNY 貨幣 regime gate 參數
    ...
    max_cny_return: float = 0.01  # Att1（最終 default）：CNY 20d ≤ +1.0%
    use_cny_divergence: bool = False
    min_relative_return: float = -0.06  # Att3 值（停用時保留供參考）


def create_default_config() -> FXI017Config:
    return FXI017Config(
        name="fxi_017_cny_regime_mr",
        experiment_id="FXI-017",
