"""
XBI-006 訊號偵測器：Bollinger Band Squeeze Breakout
(XBI-006 Signal Detector: BB Squeeze Breakout)
...
from trading.experiments.xbi_006_bb_squeeze_breakout.config import XBI006Config

logger = logging.getLogger(__name__)


class XBI006BBSqueezeDetector(BaseSignalDetector):
    """XBI BB Squeeze Breakout 訊號偵測器"""

    def __init__(self, config: XBI006Config):
        self.config = config
    ...
    def detect_signals(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        logger.info("XBI-006: Detected %d BB squeeze breakout signals", signal_count)
        return df
