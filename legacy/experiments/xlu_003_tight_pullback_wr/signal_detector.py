"""
XLU-003 訊號偵測器：緊縮回檔門檻 + Williams %R + 反轉K線

進場條件（全部滿足）：
1. 10 日回檔 >= 3.5% 且 <= 7%（緊縮回檔範圍）
...
class XLUTightPullbackSignalDetector(BaseSignalDetector):
    def __init__(self, config: XLUTightPullbackConfig):
        self.config = config
    ...
    def detect_signals(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        logger.info("XLU-003: Detected %d tight pullback + WR signals", signal_count)
        return df
