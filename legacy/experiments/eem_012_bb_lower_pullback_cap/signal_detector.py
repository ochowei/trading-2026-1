"""
EEM-012 訊號偵測器：BB 下軌 + 回檔上限混合進場均值回歸

進場條件（全部滿足）：
1. Close <= BB(20, 2.0) 下軌（自適應深度門檻）
2. 10 日高點回檔 >= -7%（EM 結構性崩盤隔離）
3. Williams %R(10) <= -80（超賣確認）
4. ClosePos >= 40%（EEM 驗證有效的日內反轉確認）
5. ATR(5) / ATR(20) > 1.1（波動率飆升，過濾 EM 慢磨下跌）
6. 冷卻期 10 個交易日
"""
...
from trading.experiments.eem_012_bb_lower_pullback_cap.config import EEM012Config

logger = logging.getLogger(__name__)


class EEM012SignalDetector(BaseSignalDetector):
    def __init__(self, config: EEM012Config):
        self.config = config
    ...
    def detect_signals(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        if suppressed:
            ...
            logger.info("EEM-012: %d signals suppressed by cooldown", len(suppressed))

        signal_count = df["Signal"].sum()
        logger.info("EEM-012: Detected %d BB lower + cap hybrid signals", signal_count)
        return df
