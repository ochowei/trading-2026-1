"""
TLT-004 訊號偵測器：Bollinger Band Squeeze Breakout
TLT-004 Signal Detector: BB Squeeze Breakout
...
from trading.experiments.tlt_004_bb_squeeze_breakout.config import TLTBBSqueezeConfig

logger = logging.getLogger(__name__)


class TLTBBSqueezeDetector(BaseSignalDetector):
    """TLT BB Squeeze Breakout 訊號偵測器"""

    def __init__(self, config: TLTBBSqueezeConfig):
        self.config = config
    ...
    def detect_signals(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        logger.info("TLT: Detected %d BB squeeze breakout signals", signal_count)
        return df
