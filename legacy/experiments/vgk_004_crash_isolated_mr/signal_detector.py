"""
VGK-004 訊號偵測器：崩盤隔離回檔+WR+ATR 均值回歸

進場條件（全部滿足）：
1. 10 日高點回檔 >= 3% 且 <= 7%（深度過濾 + 崩盤隔離）
...
from trading.experiments.vgk_004_crash_isolated_mr.config import VGK004Config
...
class VGK004SignalDetector(BaseSignalDetector):
    ...
    def detect_signals(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        logger.info("VGK-004: Detected %d crash-isolated pullback signals", signal_count)
        return df
