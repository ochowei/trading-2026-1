"""
IWM-006 訊號偵測器：Bollinger Band Squeeze Breakout
IWM-006 Signal Detector: BB Squeeze Breakout
...
from trading.experiments.iwm_006_bb_squeeze_breakout.config import IWM006BBSqueezeConfig

logger = logging.getLogger(__name__)


class IWM006BBSqueezeDetector(BaseSignalDetector):
    """IWM BB Squeeze Breakout 訊號偵測器"""

    def __init__(self, config: IWM006BBSqueezeConfig):
        self.config = config
    ...
    def detect_signals(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        logger.info("IWM-006: Detected %d BB squeeze breakout signals", signal_count)
        return df
