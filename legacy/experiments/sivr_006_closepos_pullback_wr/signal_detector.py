"""
SIVR 非對稱出場 + 回檔範圍 + Williams %R 訊號偵測器
基於 SIVR-005，進場邏輯完全相同，僅調整出場參數。

Based on SIVR-005, same entry logic, only exit parameters changed.
"""
...
class SIVRClosePosSignalDetector(BaseSignalDetector):
    """
    SIVR 非對稱出場 + 回檔範圍 + Williams %R 訊號偵測器

    三條件同時成立時觸發訊號（同 SIVR-005）：
    1. 收盤價相對 10 日最高價回檔 ≥ 7%
    2. 收盤價相對 10 日最高價回檔 ≤ 15%（過濾極端崩盤）
    3. Williams %R(10) ≤ -80（超賣）
    """

    def __init__(self, config: SIVRClosePosConfig):
        self.config = config
    ...
    def detect_signals(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        logger.info("SIVR: Detected %d AsymExit+Pullback+WR reversion signals", signal_count)
        return df
