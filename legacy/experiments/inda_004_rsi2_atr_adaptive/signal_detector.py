"""
INDA-004 訊號偵測器：短期動量崩潰均值回歸

進場條件（全部滿足）：
1. Williams %R(10) ≤ -80（超賣確認）
2. 2日累計跌幅 ≥ 2.0%（短期急跌動量）
3. 收盤位置 ≥ 40%（日內反轉確認）
...
class INDAMomentumCrashDetector(BaseSignalDetector):
    def __init__(self, config: INDAMomentumCrashConfig):
        self.config = config
    ...
    def detect_signals(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        df["Signal"] = cond_wr & cond_decline & cond_reversal & cond_vol
        ...
        logger.info("INDA-004: Detected %d momentum crash MR signals", signal_count)
        return df
