...
## 6. 確認指標的邊際效益遞減
<!-- freshness:
  derived_from: [TQQQ-007,TQQQ-012,TQQQ-013,USO-006,USO-010~020,TSM-004,FCX-003,SIVR-006,SIVR-007,IWM-005,XBI-005,FXI-011,INDA-009,EEM-013]
...
| FCX | FCX-001/FCX-004 | 三重極端超賣/BB Squeeze | 0.43/0.41 | 10 次實驗 ✓ |
...
| EEM | EEM-012 Att3 | BB 下軌+回檔上限+WR+ClosePos+ATR（混合進場）| 0.34 | 13 次實驗 ✓ |
...
| FXI | FXI-005 Att3 | 出場優化均值回歸（TP5.5%/SL5%/20d）| 0.38 | 12 次實驗 ✓ |
...
## 19. 2日急跌過濾（雙向性發現 2026-04-21）
<!-- freshness:
  derived_from: [FCX-008,USO-013,EWT-004,VGK-005,CIBR-012]
  validated: 2026-04-21
  data_through: 2025-12-31
  confidence: medium
-->
...
**跨資產延伸假設**：2DD cap 過濾可能適用其他 BB 下軌+回檔上限混合進場資產（XBI、XLU、IWM、COPX、VGK、EEM），特別是 Part A 仍有 crash-day SLs 的資產。閾值應隨日波動度縮放（CIBR 1.53% → -4.0%；高波動資產預計需更深閾值）。

**Meta-lesson**：當一個過濾器「方向」驗證無效（如 CIBR-004 的 2DD floor），其**反方向**可能作為 timing/regime 過濾器有效。此觀察為新過濾器設計途徑，超越「oscillator hook / range expansion / capitulation depth」家族（lesson #20b 失敗家族）。
...
## 20b. RSI Bullish Hook Divergence 對高波動均值回歸有效（SIVR 驗證，COPX/FCX/URA/XBI/USO/WVF/MACD/Range Expansion 失敗擴展邊界）
<!-- freshness:
  derived_from: [SIVR-015,COPX-009,FCX-009,URA-008,URA-009,URA-010,TLT-006,XBI-011,USO-022,EEM-013,CIBR-011,IBIT-008]
  validated: 2026-04-20
  data_through: 2025-12-31
  confidence: medium
