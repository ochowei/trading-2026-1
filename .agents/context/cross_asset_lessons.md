...
| XLU | XLU-009（突破）/ XLU-003（均值回歸） | KC Squeeze Breakout / 緊縮回檔範圍 + WR + 反轉K線 | ~1.2/1.0 / ~4.2/5.0 | 83.3%/100% / 61.9%/70.0% | **XLU-009**：Keltner Channel Squeeze（BB in KC(20,1.5*ATR10)，5日內）+ Close > KC upper + SMA(50)，TP+3.0%/SL-4.0%/20天/冷卻7天。Part A Sharpe **0.69**/Part B **2.26**（min 0.69，vs XLU-004 0.18 **+283%**）。KC upper 門檻高於 BB upper，精準過濾弱突破。年化訊號率 A/B 比 1.2:1（極佳平衡），但 Part B 僅 2 訊號（統計樣本有限）。XLU-009 Att2 驗證 BB upper 突破+8日窗口（Part A -0.08，門檻太低引入 4 個停損），Att3 驗證 KC upper+8日窗口（Part A 0.10，新增 2 壞訊號），5日窗口 + KC upper 是甜蜜點。XLU-003：回檔 3.5-7% + WR(10)≤-80 + ClosePos≥40%，TP+2.5%/SL-4.0%/20天。Part A Sharpe 0.06/Part B 0.35。SL -4.0% 是 XLU 全域甜蜜點。XLU-005~008 分別驗證趨勢回檔、TLT 配對/過濾、RSI(2)、動量回檔、XLU/SPY 配對交易、BB 寬帶/成交量確認均失敗（9 次實驗、37 次嘗試，含均值回歸、BB/KC 突破、趨勢回檔、跨資產配對/過濾、動量回檔、XLU/SPY 配對交易七種策略類型） |
