...
## 15. ATR 波動率自適應過濾有效邊界
<!-- freshness:
  derived_from: [IWM-011,COPX-007,XLU-011,SIVR-012,XBI-009,IBIT-004,FCX-008,EEM-010,EWZ-002,CIBR-014]
  validated: 2026-04-30
  data_through: 2025-12-31
  confidence: high
-->
...
**新發現（CIBR-014 Att2，2026-04-30，repo 首次）**：**ATR ratio CEILING（>1.40）= in-crash 加速階段**，與既有 FLOOR（>1.15）= panic-vs-slow-grind 分離形成 BAND 結構。CIBR 1.53% vol trade-level 分析發現 2021-02-26 SL 之 ATR(5)/ATR(20) = 1.5065 為 outlier（其他 7 訊號 1.16-1.31），單純 FLOOR 無法區分；加 CEILING <= 1.40 形成 BAND (1.15, 1.40] 後精準過濾此 SL 而保留所有 TP。**ATR ratio 維度為唯一可區分維度**（return-based 1d/2d/3d/5d 維度全部重疊）。**對稱失敗模式發現**：lesson #15 原為 FLOOR-only，新 BAND 概念待跨資產驗證於 XBI/COPX/FXI/URA 等已有 ATR FLOOR 應用的資產（trade-level outlier 分析識別 SL 的 ATR ratio）。
