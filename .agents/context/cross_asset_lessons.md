...
## 15. ATR 波動率自適應過濾有效邊界
<!-- freshness:
  derived_from: [IWM-011,COPX-007,XLU-011,SIVR-012,XBI-009,IBIT-004,FCX-008,EEM-010,EWZ-002,CIBR-014,FXI-014]
  validated: 2026-04-30
  data_through: 2025-12-31
  confidence: high
-->
...
**跨資產 BAND 規則精煉（FXI-014 Att2，2026-04-30，repo 第 2 次成功 + 首次政策驅動 EM 單國 ETF 驗證）**：FXI 2.0% vol trade-level 分析揭示 Part A 8 SLs 中 5 筆 ATR(5)/ATR(20) ≥ 1.41-1.54（in-crash acceleration），TP/expiries 與 Part B winners 全在 ATR ≤ 1.32（除 2 筆 expiries 1.71/2.00）。BAND (1.05, 1.35] 過濾 5 筆 acceleration SLs + 2 weak expiries，Sharpe 0.38→1.01（+166%）。**vol 縮放規則猜想**：vol 略高（2.0% vs CIBR 1.53%）的政策驅動 ETF 上 acceleration 訊號 ATR 範圍略寬，需略緊 CEILING（1.35 vs 1.40）。**重要邊界突破**：先前 lesson #6 在 FXI 上多次驗證「signal-day secondary filters 結構性失敗於政策驅動 EM」，FXI-014 為**首例例外**——當 winners/losers 在 filter 維度上有清晰邊界（如 ATR ratio CEILING 對 in-crash acceleration），signal-day filter 仍可成功；前提為**結構性 distinguishable feature** 存在。**待驗證候選**：URA（2.34% vol，URA-007 ATR FLOOR 失敗，BAND 結構可重試）、XBI（2.0%，XBI-009 失敗但 XBI-005 framework 可疊加）、EEM（已 BAND 候選，FLOOR 1.10 + CEILING 待測）。
