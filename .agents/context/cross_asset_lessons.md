...
103. **ATR 波動率自適應過濾在日波動 >1.5% 的資產上跨期不穩定** — XLU-011（日波動 ~1.0-1.5%）ATR(5)/ATR(20)>1.15 極為成功（min Sharpe 0.18→0.67，+272%），但移植到 SIVR（日波動 2-3%）後 Part A Sharpe 0.22→0.41（+86%）而 Part B 0.26→0.12（-54%），min(A,B) 從 0.22 降至 0.12。ATR>1.10 引入壞訊號（Part A WR 70%→56.5%），SL -4.0% 未挽救 Part B 停損。**原因：高波動資產的 ATR 比率在正常交易中波動更大，1.15 門檻的區分力不如在低波動資產上穩定。過濾器在 Part A（含 COVID、2022 熊市等極端事件）有效移除慢速回檔假訊號，但在 Part B（2024-2025 不同市場環境）同時移除好訊號。ATR 過濾器有效邊界約為日波動 ≤ 1.5%，與 ClosePos 有效邊界（≤ 2.0%）類似**（SIVR-012 驗證）
