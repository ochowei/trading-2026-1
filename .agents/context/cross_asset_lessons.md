...
25. **動量回調在多成分等權重板塊 ETF** — 板塊級 ROC 反映個股事件加總非板塊趨勢。**RS 動量（板塊 vs SPY/QQQ）對市值加權板塊 ETF 同��無��**：CIBR 三次嘗試（QQQ/SPY 基準、鬆/緊/品質過濾）均負 Sharpe，網路安全無獨立板塊動量週期（CIBR-006 驗證）。RS 動量有效條件���(a) 強週期性板塊如半��體（SOXL-010）或 (b) 地理/資產類別差異大的比較對（EWT vs EEM）
...
<!-- freshness:
  derived_from: [XBI-009,XBI-002,COPX-006,SPY,DIA,IWM,EEM-005,VGK-002,EWZ-002]
  validated: 2026-04-12
  data_through: 2025-12-31
  confidence: high
-->
...
- **非美國已開發市場 ETF（VGK/EWJ）**：pullback+WR+ATR 框架最佳，RSI(2) 無效（歐洲/日本市場慢磨特性）
