...
**Cross-Asset Divergence Regime Gate family v4 DRIVER-PURE 前置條件（SIVR-019 + EWT-010 共同界定 2026-05-17）**：cross-asset divergence regime gate（以 X−Y N日報酬差作為 MR base 的品質 regime 閘門，**非進場觸發**）family 現 **3-for-2**：
- **SUCCESS（皆 driver-pure 單因子反向）**：TSLA-017 TSLA−QQQ（個股 vs 自身指數，+81%）/ TLT-014 TLT−SPY（純利率 vs 股票，+393%）/ GLD-016 GLD−USD（純貨幣 vs 美元，+104%）
- **FAIL（皆 driver-impure）**：SIVR-019 SIVR−USD（白銀 ~50% 工業需求 → 與美元 DECOUPLED，best winners 發生於 strong-USD regime）/ EWT-010 EWT−EEM（**EWT 是 EEM 成分，ρ≈0.85 正相關**，binding loser 中美貿易戰為同步廣域 China/EM co-move，divergence≈0 於衝擊日，與 winners 任一 lookback 完全交錯）
