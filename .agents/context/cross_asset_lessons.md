...
100. **2日跌幅過濾不適用低波動利率敏感 ETF** — XLU（日波動 1.08%）2日跌幅 ≤ -1.0% 加在 PB 3.5-7% + WR≤-80 + ClosePos 進場上，Part B Sharpe 0.35→0.01，訊號 10→6，WR 70%→50%。**原因：低波動 ETF 的均值回歸機會來自漸進式回檔（利率驅動），非急跌。2日跌幅過濾移除的正是最有回歸動能的訊號**。USO-013 的 2日跌幅成功因 USO 日波動 ~2% 且回檔為急跌性質。2日跌幅過濾的有效邊界約為日波動 ≥ 2%（XLU-010 Att1 驗證）
101. **利率敏感 ETF 均值回歸受升息結構性限制無法透過技術面過濾突破** — XLU-010 三次嘗試：2日跌幅過濾（Part A -0.02/Part B 0.01）、提高回檔下限至 4%（-0.02/0.11）、收緊 WR 至 -85（0.04/-0.11），均失敗。**三種不同過濾方向一致失敗的根本原因：2022 升息期假訊號的共同特徵不是「不夠超賣」或「回檔太淺」或「跌得不急」，而是「利率政策驅動的持續性下跌」——純技術面無法區分**。與 TLT（lesson #75）情況一致，利率敏感資產的均值回歸策略存在不可修復的 Part A 結構性弱點（XLU-010 驗證）
