...
## 6. 確認指標的邊際效益遞減
<!-- freshness:
  derived_from: [TQQQ-007,TQQQ-012,TQQQ-013,USO-006,USO-010~020,TSM-004,FCX-003,SIVR-006,SIVR-007,IWM-005,XBI-005,FXI-011,INDA-009,EEM-013,TLT-010,TLT-011,TLT-012]
...
**DIA-012 擴展（2026-04-24）**：低波動寬基 ETF（DIA ~1.0% vol）SLs 在 1 日與 3 日報酬維度有清晰分離（與 2 日維度正交）：(a) 1d cap >= -2.0% 過濾 news/policy-driven 單日深度急跌（如 Omicron 黑色星期五、COVID 二波）；(b) 3d cap >= -7% 過濾 regime-shift 級別 3 日延續下跌（如 Trump 關稅週末延續）。**1d+3d 雙維度過濾**為新策略類別（repo 首次），擴展 lesson #19 family 至第三類失敗結構：與 2DD cap CIBR-direction（深 2DD SL 集中）和 2DD floor EEM/INDA/USO-direction（淺 2DD SL 集中）並列為三大失敗模式。**規則**：低波動寬基 ETF 在 RSI(2)+2DD+ClosePos 框架基線上，若 SL 在 2DD 維度與贏家分布重疊但在 1d/3d 維度有偏移，採 1d cap + 3d cap 雙維度過濾。
