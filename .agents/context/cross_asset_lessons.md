...
## 20b. RSI Bullish Hook Divergence 對高波動均值回歸有效（SIVR 驗證，COPX 失敗擴展邊界）
<!-- freshness:
  derived_from: [SIVR-015,COPX-009]
  validated: 2026-04-18
  data_through: 2025-12-31
  confidence: medium
  note: SIVR-015 Att1 first repo validation (10-day pullback framework). COPX-009 (2026-04-18) three iterations all failed on 20-day pullback framework (min 0.45→0.15), extending pattern boundary: requires pullback lookback ≤10 days.
-->
...
- 其他子條件：RSI 週期需為 14（RSI(7) 過噪，SIVR 翻負）；hook lookback 5 日最佳（7 日納入過舊 RSI 低點）；hook delta 3 點最佳（2 點太鬆納入噪音）
...
**跨資產泛化假設（待進一步驗證）**：可能適用於其他**日波動 2-3% + 回檔回看窗口 ≤10 日**且已驗證 pullback+WR 框架的資產（FCX、USO、TSLA 待測）。**低波動資產**（GLD、SPY、EWJ、VGK）divergence 訊號可能過於稀少；**政策/事件驅動資產**（FXI、TLT）可能因 RSI 特徵受宏觀事件影響而失效；**長回檔窗口資產**（COPX 20 日）已確認失效。
