...
**跨資產泛化假設（待進一步驗證）**：可能適用於其他**日波動 2-3% + 回檔回看窗口 ≤10 日 + 兩段 Part A/B 皆活躍 MR regime**且已驗證 pullback+WR 框架的資產（USO、TSLA 待測）。**低波動資產**（GLD、SPY、EWJ、VGK）divergence 訊號可能過於稀少；**政策/事件驅動資產**（FXI、TLT）可能因 RSI 特徵受宏觀事件影響而失效；**長回檔窗口資產**（COPX 20 日）已確認失效；**post-peak 持續下跌資產**（FCX 2024-2025 銅價）因 Part B regime 失效而失敗。
