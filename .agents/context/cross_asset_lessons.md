...
## 6. 確認指標的邊際效益遞減
<!-- freshness:
  derived_from: [TQQQ-007,TQQQ-012,TQQQ-013,USO-006,USO-010~020,TSM-004,FCX-003,SIVR-006,SIVR-007,IWM-005,XBI-005,FXI-011,INDA-009,EEM-013,TLT-010,TLT-011,TLT-012,TLT-015,NVDA-010,CIBR-013,SIVR-017,TSM-012]
...
**反例 9（Volume ratio filter 在 RS momentum pullback 框架雙向皆失敗 / lesson #6 volume-based filter 邊界第三次擴展）**：**TSM-012 驗證 signal-day Volume / SMA(Volume,20) ratio filter** 雙向（floor / ceiling）皆失敗於 TSM-008 RS momentum pullback 框架（**repo 第 3 次 volume filter 主訊號試驗、首次於 RS momentum 框架**，2026-05-08）。三次迭代 min(A,B) **0.65 / 0.54 / 0.65** 均未勝過 TSM-011 Att3 的 0.83。Att1（vol_ratio_min >= 1.30，H1 capitulation buy hypothesis）Part A 5/80%/Sharpe **1.56** cum +33.71%（4/5 TP，0 SL）/ Part B 4/75%/Sharpe **0.65** cum +17.04%（2024-07-08 SL 仍存活）/ min **0.65** — vol >= 1.30 過於嚴苛雙 Part 縮減約 60%，保留之 Part A 訊號品質極佳但 **2024-07-08 Part B SL 結構性穿透高 vol 過濾**（此 SL vol 高屬 capitulation 範疇但 5d 跌幅深，後續 1 週繼續下跌），volume confirmation 無法區分「真假 capitulation」；Att2（vol_ratio_max <= 1.20，H2 orderly continuation hypothesis）Part A 9/66.7%/Sharpe **0.54** cum +33.94%（2 SLs：2022-11-21、2022-12-07 baseline 已知失敗訊號通過 vol <= 1.20 缺口進入）/ Part B 7/71.4%/Sharpe **0.54** cum +26.84%（2 SLs）/ min **0.54** — orderly continuation 假設失敗，低 vol 包含「無動量參與的弱訊號」而非「健康 profit-taking」；Att3（vol_ratio_min >= 1.10，moderate floor）Part A 6/83.3%/Sharpe **1.76** cum **+44.41%** / Part B 4 同 Att1 / min **0.65** / **A/B 累計差 62%**（>> 30% target）— 結構性 A/B 失衡：Part A 高 vol 訊號天然密集於 2020-2023 高波動期，Part B 2024-2025 較低波動期 vol 整體偏低，floor filter 系統性傾向 Part A。**核心發現**：(1) Volume filter 在 RS momentum pullback 框架**雙向皆失敗**——floor 過嚴削減訊號但無法區分 SL/TP、ceiling 引入弱訊號；(2) **2024-07-08 Part B SL 結構性穿透 volume confirmation**——與 lesson #20b oscillator hook 失敗家族（RSI/CCI/Stoch/MACD turn-up 無法區分真假 V-bounce）平行，volume 維度無法區分真假 capitulation；(3) **Volume filter A/B regime asymmetry 為新發現失敗模式**：絕對 vol_ratio threshold 在「Part A 高波動 / Part B 低波動」結構下**系統性 A/B 失衡**——需 vol normalization（rolling z-score 而非絕對 ratio）以避免結構性偏差，但 z-score normalization 本身會引入額外複雜度。**新跨資產規則（lesson #6 volume-based filter 邊界第三次擴展）**：**volume-based filters 作為 supplementary not substitutive 維度規則從 MR 框架（URA-011 Volume spike、SIVR-017 MFI）擴展至 RS momentum 框架**——三類框架皆驗證，volume filters 作為 primary screening dimension 在 active price-momentum-driven 框架（MR + RS momentum）皆無效，建議僅作為 secondary confirmation 維度與其他主要 price-action filter（5d ceiling、2DD cap、ClosePos）combination 使用。TSM-011 Att3 仍為全域最優（12 次實驗、36+ 次嘗試）。
...
<!-- freshness:
  ...
  data_through: 2025-12-31
  confidence: high
  note_2026_05_08_tlt015: TLT-015 added (HYG Credit Cross-Asset Divergence Regime-Gated MR, **repo 首次 HYG 高收益債作為跨資產 divergence 過濾器於任何資產 + 第二 cross-asset anchor 疊加於已存在 anchor 試驗**, three iterations all FAIL/TIE vs TLT-014 Att3 baseline min(A,B)† 0.69). Att1 (lookback=20, max_credit=+5%) min(A,B)† **0.69 TIE**: HYG-TLT 20d divergence < +5% 對全部 5+4 訊號皆滿足，filter 完全 non-binding。Att2 (lookback=20, max_credit=+2%) min(A,B)† **0.00 REJECT**: threshold 過嚴 Part A 訊號 5→2（zero-var）+ Part B 4→3 誤刪 2024-11-15 TP HYG-TLT=+3.50%，雙 zero-var 樣本崩潰。Att3 (lookback=10, max_credit=+3%) min(A,B)† **0.50 REJECT (-28%)**: 過濾 2020-11-09 TP HYG-TLT 10d=+4.57% 但保留 2021-01-06 SL HYG-TLT 10d=+3.13%，**filter 系統性反向選擇——移除 winner 而非 loser**。**核心發現（cross-asset divergence regime gate orthogonality 失敗）**：(1) **HYG-TLT 與 SPY-TLT 高度相關（非正交）**——假設 HYG（duration + credit 雙因子）為 TLT-SPY 的 credit-orthogonal 互補維度，實際資料顯示 reflation regime 中 HYG-TLT 訊號方向與 SPY-TLT 同向（皆 risk-on），但**幅度不足以區分 winner vs loser**——SPY-TLT divergence 已榨取 TLT cross-asset 維度的 selectivity 上限；(2) **新跨資產規則（lesson #6 + lesson #20 v3 邊界，repo 首次第二 cross-asset anchor 疊加失敗）**：cross-asset divergence regime gate 應限定為**單一最強 anchor**，疊加多個 cross-asset anchors（SPY + HYG + LQD）為**結構性冗餘**——多個 risk-on/off benchmarks 在 reflation regime 中同步反映同一 macro factor，無新增 selectivity，反引入過擬合 + 訊號樣本崩潰風險；(3) **與 TLT-008（IEF pair）失敗區隔**：TLT-008 失敗於「same-asset-class 機械性 pair」（duration ratio 固定，無 regime 區分力）；TLT-015 失敗於「cross-asset divergence stacking」（與已存在 anchor 相關性過高）。**所有單一 cross-asset anchor 成功案例（TLT-014/NVDA-021/TSLA-017/INDA-012/EWZ-009）皆為「單一 anchor」**——repo 首次「在已成功 anchor 上疊加第二 anchor」試驗結構性失敗；(4) **TLT 0.69 ceiling 突破方向（pending）**：需引入**真正正交維度**——(a) 殖利率曲線陡峭化指標（^TYX 30Y - ^TNX 10Y slope，forward-looking 通膨預期）、(b) DXY 美元指數（FX 維度，與 equity/credit benchmarks 不同 macro factor）、(c) TIPS-Treasury breakeven inflation rate（直接捕捉通膨預期）。2021-01-06 reflation regime 主要由「fiscal stimulus 預期」驅動而非單純 credit cycle，需 inflation-anchored 指標方能 cleanly 區分 SL 與 winners。**TLT-014 Att3 仍為全域最優（15 次實驗、44+ 次嘗試）**。
  note_2026_05_08_tsm013: TSM-013 added (TSM-QQQ 20d Cross-Asset Divergence CEILING Regime-Gated RS Momentum Pullback, **repo 第 6 次 cross-asset divergence regime gate 跨資產移植、首次半導體 ADR 個股 + RS Momentum Pullback 框架**, cross-asset port from NVDA-021 (NVDA-QQQ CEILING + MBPC) / mirror INDA-012 / EWZ-009). Three iterations all PARTIAL/REJECT vs TSM-011 Att3 min 0.83. **Att1 ★ PARTIAL** (lookback=20, max_relative_return=+0.15 loose ceiling) Part A **9 訊號 / WR 100% / Sharpe 0.00 zero-var (9/9 TPs) / cum +99.90% / MDD -6.06%**（vs baseline 12/83.3%/0.86/+74.10%/-7.89%, **cum +35% / WR +17pp / MDD -23%**, 過濾 baseline 全部 2 SLs + 1 winner）/ Part B **10 訊號完全 unchanged / WR 80% / Sharpe 0.83 / cum +59.78%**（filter 對 Part B 完全非綁定，2/2 Part B SLs 之 Rel_QQQ_20d 皆 < +15%）/ min(A,B)† **0.83 TIE baseline**（依 EWJ-003/SPY-009/DIA-012/IWM-013/CIBR-014 慣例，Part A zero-var 結構性最優、Part B 變異 Sharpe 為 binding constraint）；Att2 (lookback=20, max_relative_return=+0.10) FAILED min(A,B) **0.65** REJECT — Part B 8/75%/Sharpe 0.65（**Part B 退化 -22% Sharpe**，過濾 2 winners 但 2 SLs 仍存活）；Att3 (lookback=10, max_relative_return=+0.10) FAILED min(A,B) **0.31** REJECT — 短週期 Rel_QQQ noisy + cooldown chain shift 引入額外 SLs。**核心發現（lesson #19/#26 family v3 邊界精煉，repo 首次跨 Part SLs divergence 結構反向發現）**: (1) **TSM Part A 與 Part B SLs 在 Rel_QQQ_20d 維度結構性反向**：Part A SLs 高 Rel_QQQ（>+15%, rally exhaustion 結構，CEILING 方向有效）vs Part B SLs 低 Rel_QQQ（<+10%, earnings drift / sector-specific drop 但 QQQ 同步上漲 → TSM 相對沒有過度跑贏，CEILING 方向結構性失敗）。**單一 CEILING threshold 結構性無法雙 Part 同步改善**，Part B 為 binding constraint，CEILING 方向結構性無法突破 0.83；(2) **與 NVDA-021 結構對比**：NVDA Part A/B SLs 皆集中於高 Rel_QQQ（CEILING 方向結構性一致 → min 0.55→1.43, +160%）；TSM 因 multi-driver 結構（中國地緣政治 + 半導體景氣 + 客戶集中度 + 新興市場 ADR earnings cycle）使 Part B SLs 機制不同於 Part A，CEILING 方向結構性失敗於 Part B；(3) **新跨資產規則（lesson #19/#26 family v3 邊界）**：cross-asset divergence regime gate 適用邊界擴展為「Part A/B SLs 在 divergence 維度單向對齊」雙條件。NVDA-021 ✓（雙 Part 對齊高 Rel_QQQ → CEILING 成功 +160%）；TLT-014 ✓（雙 Part 對齊低 Rel_SPY → FLOOR 成功 +393%）；TSLA-017 ✓（雙 Part 對齊低 Rel_QQQ → FLOOR 成功 +81%）；INDA-012 / EWZ-009 ✓（雙 Part 對齊 → CEILING 成功）；**TSM-013 ✗**（雙 Part SLs 結構性反向，CEILING 僅解 Part A）；(4) **Multi-driver 個股 vs single-driver 個股 driver 結構為 cross-asset divergence regime gate 適用邊界 precondition**：multi-driver 結構（TSM 半導體 ADR）使 Part A/B SLs 機制反向，single-driver（NVDA AI / TSLA EV）使 Part A/B SLs 機制一致——driver 結構為 lesson #19/#26 family v3 新規則維度；(5) **Att1 為 partial-success 重要實質改善**：repo 首次將 cross-asset divergence regime gate（CEILING 方向）移植至半導體 ADR 個股 + RS Momentum Pullback 框架，Part A 結構性突破（zero-var all TPs，cum +35%、WR +17pp、MDD -23%）為實質改善但 Part B sample size + 結構不對齊使 min(A,B) TIE baseline；(6) **lesson #6 邊界第四次擴展候選**：cross-asset divergence regime gate 在「multi-driver 個股 + RS framework」上 Part A 有效但 Part B 結構性失敗，與既有「single-driver 個股 + MBPC/BB Squeeze」全部成功案例（NVDA-021/TSLA-017）形成失敗邊界。TSM-011 Att3 仍為 min(A,B) 全域最優（13 次實驗、39+ 次嘗試）。
  note_2026_05_08_nvda021: NVDA-021 Att2 added (NVDA-QQQ 20d Cross-Asset Divergence CEILING Regime-Gated MBPC, **repo 首次 cross-asset divergence regime gate（CEILING 方向）成功移植至高波動 AI mega-cap 個股 + MBPC 框架，雙重邊界擴展首次成功**, cross-strategy port from INDA-012/EWZ-009 (CEILING in MR framework) + cross-asset port from TSLA-017 (FLOOR direction, BB Squeeze framework)). Three iterations: Att1 (max_relative_return=+0.05) Part A 15/80.0%/Sharpe **0.82** cum +101.64% / Part B 5/80.0%/Sharpe **1.99** cum +35.99% / min(A,B) **0.82** (+49% vs NVDA-013 Att3 0.55) / A/B 年化 cum 差 **9.3% < 30% ✓** (大幅優於 baseline 26.4%) / 年化訊號比 1.2:1 (gap 16.7% < 50% ✓); **Att2 ★ (max_relative_return=+0.03 sweet spot)** Part A 10/**90.0%**/Sharpe **1.43** cum +85.63% / Part B 5/80.0%/Sharpe 1.99 cum +35.99% (與 Att1 相同) / min(A,B) **1.43** (**+74% vs Att1 0.82, +160% vs NVDA-013 baseline 0.55，repo 首次突破 NVDA 結構性 Sharpe 0.55 ceiling**) / 額外過濾 5 Part A 訊號（含 2 SLs：2020-10-20 + 2023-08-28），Part A SLs 3→1（唯一殘留 2019-02-20 為 NVDA-QQQ 邊緣 < +3%）/ A/B 年化 cum 差 20.5% < 30% ✓、年化訊號比 0.8:1 < 50% ✓ / NVDA Part A WR 歷史新高 90.0%；Att3 (max_relative_return=+0.01 過嚴) Part A 9/88.9%/Sharpe **1.33** cum +71.88% / Part B 3/100% std=0 zero-var Sharpe 0.00 cum +25.97% / min(A,B)† **1.33** (-7% vs Att2)—— +1% 過濾 1 Part A winner + 2 Part B 訊號，Part B 訊號密度 1.5/yr (vs baseline 3.5/yr) 統計顯著性不足. **核心發現（lesson #19/#26 family v2 邊界精煉）**: (1) **Repo 首次 cross-asset divergence regime gate（CEILING 方向）成功移植至高波動 AI mega-cap 個股 + MBPC 框架（雙重邊界擴展）**——既有成功案例：TLT-014 (TLT-SPY 20d FLOOR, 利率 vs 股票 MR) / TSLA-017 (TSLA-QQQ 20d FLOOR, 高波動 AI 個股 vs 大盤 BB Squeeze) / INDA-012 (INDA-EEM 60d CEILING, 單一國家 vs 大盤 EM MR) / EWZ-009 (EWZ-EEM 10d CEILING, 商品國 EM vs 大盤 EM MR); NVDA-021 為**雙重邊界擴展首次成功**：(a) CEILING 方向首次於 MBPC（動量延續）框架，(b) CEILING 方向首次於高波動 AI 個股類別; (2) **NVDA 結構性 Sharpe 0.55 ceiling 首次突破（21 次實驗 / 53+ 次嘗試後）**——突破來源：cross-asset relative performance 維度為 NVDA-013 雙重 SMA/ATR regime gate 飽和後的下一個獨立選擇維度; (3) **NVDA-QQQ vs NVDA-SMH anchor 選擇**：QQQ（NVDA ~5-12% 權重）較 SMH（NVDA ~20% 權重）為更獨立 anchor，提供更乾淨的 NVDA-specific rally exhaustion 訊號; (4) **CEILING vs FLOOR 方向選擇規則（lesson #19/#26 family v2）**：CEILING 適用「individual asset rally exhaustion vs broader benchmark」結構（個股過度跑贏大盤、單一國家 ETF 過度跑贏區域 ETF）；FLOOR 適用「individual asset weakness vs broader benchmark」結構（利率資產 reflation regime SL、單一個股 event-driven sell-off）；方向選擇依資產 SLs 在 divergence 維度的單向結構決定. **新跨資產假設（待驗證）**：CEILING 方向可能適用其他 mega-cap 個股 + 動量延續框架（TSLA / GOOGL / META / AMZN vs QQQ），閾值需依資產 vs benchmark 的 vol/leadership 結構調整. NVDA-021 Att2 為新全域最優（21 次實驗、52+ 次嘗試），取代 NVDA-013 Att3 為當前最佳.
  note_2026_05_08_inda: INDA-012 Att1 added (INDA-EEM 60d Relative Strength Divergence Filter on Multi-Period Capitulation MR, **repo 首次 60d lookback RS divergence 維度於任何資產 + repo 第 7 次 lesson #20 v3 cross-asset divergence regime gate 應用 + 第 2 次 EM single-country + EEM anchor 配對（繼 EWZ-009 後）**, extends INDA-011 Att3 framework with INDA-EEM 60d RS cap +5% gate). Three iterations all SUCCESS / 完全相同結果（robustness 跨 lookback 與 threshold 確認）：Att1 ★ (rs_lookback=60, max_rs_excess=+0.05) Part A 4 訊號 WR **100%** Sharpe **4.07** cum **+12.82%** MaxDD -3.70% / Part B 2 訊號 WR **100%** std=0 cum +7.12% (與 INDA-011 Att3 baseline 完全相同) / min(A,B)† **4.07** (**+640% vs INDA-011 Att3 baseline 0.55，repo 首次突破 INDA 結構性 0.55 ceiling**) / A/B 年化 cum 差 30.0% / 訊號比 0.8:1.0 = gap 20%; Att2 (rs_lookback=60, max_rs_excess=+0.10) 與 Att1 完全相同結果 — 確認 sweet spot 範圍 [+5%, +10%] 對 60d lookback 高度穩健; Att3 (rs_lookback=20, max_rs_excess=+0.03) 與 Att1 完全相同結果 — 確認 SL 在 20d 維度（RS_20d=+4.62%）亦為 outlier，跨 lookback 維度皆有效. **核心發現（lesson #20 v3 family v2 邊界擴展）**: (1) **Repo 首次 60d lookback RS divergence 維度於任何資產**——既有 lesson #20 v3 應用皆 ≤ 20d lookback（TLT-014/TSLA-017/COPX-014/USO-026/NVDA-016 20d、EWZ-009 10d），INDA-012 為首次中長期 lookback 試驗；(2) **INDA 失敗模式為「INDA 過去 60 日領漲後 country-specific 走弱」更長 horizon regime divergence**——與 EWZ「短期 country-specific 急跌」（EWZ-009 10d）structurally 不同，故 lookback 跨資產差異化為 lesson #20 v3 family v2 新規則；(3) **Trade-level 維度分析**：2022-09-16 SL（Fed CPI shock，前 60 日 INDA 領 EEM +15.28% 為唯一 outlier）為 country-specific weakness 訊號，全部 4 Part A TPs RS_60d ∈ [-5.92%, +2.07%] + 1 small profit exp +1.72% + 全部 2 Part B TPs ∈ [-5.18%, -3.58%] 皆深度負或微正，single dimension surgical sweet spot 異常乾淨；(4) **Robustness 跨 dimension 三重確認**：60d/+5%、60d/+10%、20d/+3% 三組合皆產生相同訊號集（4 wins + 2 wins）——SL 為「mid-long term INDA-EEM divergence outlier」結構性訊號，非 dimension-fragile artifact. **新跨資產假設（lesson #20 v3 family v2 lookback 維度規則）**：(a) 短期急跌 country-specific 反彈 → 10d（EWZ）; (b) 中期 sector/macro regime divergence → 20d（TLT/TSLA/COPX/USO/NVDA, mixed success）; (c) **中長期累積相對強度後 country-specific 走弱 → 60d（INDA, success）**——適用候選：EM single-country ETFs（EWT 半導體驅動 / FXI 政策驅動可能不適合）、sector ETFs（XBI/CIBR 板塊內 RS divergence 可能 mirror）。INDA-012 Att1 為新全域最優（12 次實驗、37+ 次嘗試），擴展 lesson #20 v3 family。
...
| INDA | **INDA-012 Att1** | **INDA-EEM 60d RS Divergence Filter on Multi-Period Capitulation MR (INDA-011 Att3 + INDA-EEM 60d return diff <= +5%)** | **4.07†** | **12 次實驗 ★（INDA-012 repo 首次 60d lookback RS divergence 維度於任何資產，repo 第 7 次 lesson #20 v3 cross-asset divergence regime gate 應用，第 2 次 EM single-country + EEM anchor 配對，+640% vs INDA-011 Att3 0.55）**|
...
- **個股（TSLA/NVDA/FCX）**：BB 擠壓突破或極端超賣（取決於波動度）。**FCX-013 驗證 buffered multi-week SMA regime 第 3 次跨資產有效（首次商品/礦業單股，反向 k=1.00 嚴格甜蜜點）** min 0.41→0.55（+34%），擴展 lesson #22 適用至商品/週期性個股（k 值資產相依，需 trade-level ratio 分布分析）。**NVDA-021 驗證 cross-asset divergence regime gate（CEILING 方向）對高波動 AI mega-cap 個股 + MBPC 框架有效**（NVDA 20d - QQQ 20d ≤ +3% rally exhaustion filter）：min(A,B) 0.55→**1.43**（+160%，repo 首次突破 NVDA 0.55 結構性 ceiling）；驗證「individual asset rally exhaustion vs broader benchmark」CEILING 方向適用結構（mirror INDA-012/EWZ-009 EM 單一國家 ETF + MR 框架的 CEILING）；QQQ（NVDA ~5-12% 權重）較 SMH（~20%）為更獨立 anchor。**新跨資產假設待驗證**：CEILING 方向可能延伸至其他 mega-cap AI 個股 + 動量延續框架（TSLA / GOOGL / META / AMZN vs QQQ）
...
- **FAILED**：NVDA-018 ^VXN（high-vol AI single stock + MBPC 框架）、**TLT-016 multi-window IV DIRECTION combo 跨策略移植**（rate-driven asset + MR 框架，2026-05-09）、**TQQQ-019 ^VIX 5d cumulative DIRECTION**（leveraged ETF + extreme capitulation framework，2026-05-09）、**TQQQ-020 ^VIX 1d momentum reversal/peak-passing**（leveraged ETF + extreme capitulation framework，2026-05-09，repo 首次「VIX 1d peak-passing」維度試驗）

**TQQQ-019 + TQQQ-020 整合跨資產發現（v9 適用邊界精煉，repo 首次發現 VIX DIRECTION 維度全層級結構性失效於 extreme capitulation framework，2026-05-09）**：
- **TQQQ-019**（^VIX 5d cumulative DIRECTION）三次嘗試 +5/+10/+15 全部 REJECT/TIE vs TQQQ-018 Att3 baseline 0.80。AI_CONTEXT 結論：「capitulation 與 VIX spike 結構性共線」，新假設「可能需 ^VIX 1d change（signal-day deceleration）或 peak passing 確認方能切分」
- **TQQQ-020**（^VIX 1d momentum reversal / peak-passing，repo 首次「^VIX 1d」維度試驗於任何資產）三次嘗試 ≤0/+3/+7 全部 REJECT/TIE：(a) Att1 1d ≤ 0 結構性失敗（capitulation 訊號日 VIX 必上升）；(b) Att2 1d ≤ +3 TIE baseline 但 Part A 退化（驗證 Part B SL 與 Part A winners 在 VIX 1d 維度結構性反向）；(c) Att3 1d ≤ +7 cooldown chain shift 引入新 SL
- **整合結論**：VIX **DIRECTION** 維度（不論時間尺度 1d ~ 5d、不論 cumulative vs momentum reversal）皆與 TQQQ extreme capitulation framework 結構性 100% 共線——**整體拒絕 lesson #24 family DIRECTION 維度於 -15% drawdown extreme capitulation framework**
- **新跨資產規則（v9 適用邊界精煉）**：lesson #24 family DIRECTION 維度（任何時間尺度 + 任何形式 cumulative/momentum）適用邊界 = 「進場條件不含極端 capitulation 觸發」——TQQQ extreme capitulation（DD ≤ -15% + RSI(5) < 25 + Volume > 1.5x）為首例 framework-level 失敗確認（TQQQ-019 + TQQQ-020 連續六次嘗試證實）
- **新跨資產假設（待驗證）**：可能突破 TQQQ Part B 0.80 ceiling 之三類方向：(a) ^VIX **LEVEL** 維度（如 VIX < 50 absolute cap 防 mid-panic 進場）；(b) **VIX-VXN cross-index divergence**（VIX 為 SPX implied vol、VXN 為 NDX implied vol，兩者差值可能識別 NASDAQ-specific 過度恐慌）；(c) **完全替代 framework**（拋棄 -15% drawdown 極端 capitulation 結構，採 vol-transition MR / BB Squeeze Breakout / Cross-asset divergence regime gate 等）
- **lesson #24 family 全層級綜整（v9）**：適用邊界 = 「進場 framework 之核心 trigger 與 IV index 維度結構性正交」——non-extreme capitulation + RSI hook MR + Pullback MR + BB Squeeze Breakout 等「非極端 capitulation 觸發」frameworks 適用 ✓ ；TQQQ extreme capitulation -15% drawdown framework 為首例 framework-level 失敗 ✗
