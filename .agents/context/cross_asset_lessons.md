...
## 19. 2日急跌過濾（雙向性發現 2026-04-21，EEM-014 再確認方向資產相依性）
<!-- freshness:
  derived_from: [FCX-008,USO-013,EWT-004,VGK-005,CIBR-012,EEM-014]
  validated: 2026-04-21
  data_through: 2025-12-31
  confidence: medium
-->
...
**跨資產延伸假設**：2DD 方向精煉可能適用其他混合進場資產（XBI、XLU、IWM、COPX、VGK、EWJ、EWT、EWZ）。方向選擇遵循上述 SL 結構檢查法。閾值隨日波動度縮放：
- CIBR 1.53% vol → cap -4.0%（2.6σ）
- EEM 1.17% vol → floor -0.5%（~0.4σ，淺漂移過濾）

**Meta-lesson**：當一個過濾器「方向」驗證無效（如 CIBR-004 的 2DD floor、EEM-014 Att1 的 2DD cap），其**反方向**可能作為 timing/regime 過濾器有效。此觀察為新過濾器設計途徑，超越「oscillator hook / range expansion / capitulation depth」家族（lesson #20b 失敗家族）。**Att1 失敗轉 Att2 成功為「反向驗證」教科書案例**：遇到跨資產移植失敗時，先檢查資產結構（殘餘 SL 特徵）而非放棄方向。
