...
本版本另建立 worktree-independent shared qualification authority。Frozen study 仍以
`state/qualification-registry.json` 作 repository-relative logical identity，但 mutation、head
checkpoint、catalog、transaction journal 與 locks 必須由 verified Git common repository identity
解析到同一 private shared state。既有分岔 registry 只能 byte-for-byte 匯入 immutable shards，
不得挑選、串接、重算 hash 或用較新的 worktree 取代較舊 authority。Family open-plan projection
必須同時重播全部 registered shards 與 active shared chain。
...
本流程亦涵蓋 shared qualification state 的一次性 complete-inventory migration、immutable shard
catalog、global single-open-plan projection，以及 superseded workflow 下已有 accepted
`close-invalidated` impact decision 的 paused study plan 之 cross-chain administrative closure。
Migration 或 closure 都不是 study stage，不產生 research outcome 或 trading authority。
...
- 從 worktree path、branch、commit recency 或 filesystem timestamp 選擇 canonical registry；
- 合併 registry event arrays、重算 historical event hashes，或忽略未註冊的 source chain；
...
6. 在任何 plan registration、screen、challenge、replay 或 terminal work 前，shared qualification
   authority 必須已完成 verified migration 並報告完整 source inventory。Repository identity、
   catalog、任一 shard/checkpoint、active chain/head、shared lock 或 pending journal 缺失、衝突或
   drift 時不得執行 outcome-relevant work。新 worktree-local registry 不具 authority。
...
| Human shared-state migration approver | 核准 exact complete source-registry/checkpoint inventory 與 stable repository identity；不得提供 derived hashes、忽略已發現的 chain，或把 migration 當作 plan closure。 |
| Human cross-chain plan-closure approver | 以新的當下穩定 identifier 核准 exact imported plan 的 terminal administration；paused study 另須 accepted `close-invalidated` impact evidence，且不得把 closure 當作 study outcome。 |
...
authority 的治理事件；它不代表任何 strategy activation。本版本 study 沒有 strategy activation
...
Shared-state migration 與 cross-chain closure 亦位於正常 study path 之外。Migration 只建立完整
authority catalog；closure 只終止 exact plan 的 administrative lock。兩者都不得推進 study。
...
trading qualification plan abandon \
  ...
  --workflow <exact-active-workflow-path> \
...
或 owning study cancellation 都不能授權 mutation。本版本可處理同一 workflow family 之較舊
...
### 3b. Shared qualification-state migration and cross-chain administration

Default private qualification authority 必須由 verified Git common directory 與 stable repository
identity 解析；所有 worktrees 共享相同 catalog、active chain、head、mutation lock、
study-registration lock 與 transaction journal。`state/qualification-registry.json` 仍是 frozen
logical identity，但不得在每個 worktree 建立獨立 mutable authority。Explicit `--path` 只可作
read-only diagnosis、完整 migration source 或 exact committed legacy recovery。

Migration request 必須列出 complete source registry/checkpoint pairs 與 stable human approver。
Writer 必須自己計算每個 source digest、event count、head hash、families、plans、study bindings 與
repository identity；caller 不得提供 derived authority。Apply 前須取得 repository-wide lock，
拒絕 held source lock、pending journal、missing/mismatched checkpoint、malformed chain、duplicate
event identity、incomplete inventory 或 unsafe path，再 byte-identical stage content-addressed
immutable shards、authority catalog、active chain/checkpoint 與 recoverable journal。Commit decision
必須綁定 exact inventory bytes；retry 只能完成同一 decision。

Global reader 必須 replay 每個 registered shard 與 active chain。任一 `historical_plan` 在 exact
`historical_screen`、`historical_plan_abandoned` 或 authorized
`historical_plan_closed_invalidated` 前保持 open。任何 omitted/unregistered shard、duplicate plan、
conflicting terminal fact、catalog drift 或 cross-chain binding mismatch 都 fail closed；workflow
version、branch、commit、mtime 或 source path 不可選出 winner。

Cross-chain terminal event 必須 append 到 active chain，並綁定 source shard digest/head、plan event
hash、plan/family、完整 frozen study identity、canonical current lifecycle、exact authorizing workflow
release/capabilities、accepted impact evidence digest、current approver/time/reason，以及 catalog 與
active-chain prior head。Cancelled study 可依既有 abandonment semantics 關閉；paused study 只在
其 workflow 已 superseded 且 accepted impact 明列 exact `close-invalidated` 時，使用
`historical_plan_closed_invalidated` 關閉。後者不會 cancel、complete、move、resume、review 或
判定 study。Screened、completed、active-version paused、unbound、already-terminal 或 caller-selected
study 一律不具資格。

Migration、plan closure、workflow release/activation、study operation、outcome review 與 trading
authority 彼此獨立。Migration 完成後仍須每次 closure 的 separate current approval；closure 不會
建立 Evaluation、outcome、Shadow、promotion、broker、order、position 或 live authority。
...
24. Release readiness 必須 provider-free 證明本 v011 的獨立
...
27. `qualification-plan-abandonment-v1` 只授權 exact active/effective workflow release 下的 guarded
    administrative writer。它不得由舊版本、draft/prepared version、study lifecycle 或 caller
...
31. `shared-qualification-state-v1` 只授權 complete-inventory、byte-preserving、human-approved
    migration 與 worktree-independent read/write coordination。缺少任一 discovered chain、source
    checkpoint、repository identity、catalog shard、shared lock 或 journal 時全部 qualification
    mutation fail closed。
32. Historical registry shards 與原始 checkpoints 永不可重寫、拼接、刪除或重新 hash。Authority
    catalog 必須 content-addressed binding 每個 shard；new event 只可 append 到 shared active chain。
33. Global single-open-plan projection 必須涵蓋所有 registered shards 與 active chain。Explicit
    `--path`、current worktree、branch、commit recency、mtime 或 caller assertion 不得縮小 projection。
34. `cross-chain-plan-administration-v1` 只授權 exact imported plan 的 administrative terminal
    writer。Cancelled plan 沿用 abandonment eligibility；paused plan 另需 superseded owner workflow
    與 accepted exact `close-invalidated` impact evidence。兩者都不建立 outcome 或 trading authority。
35. Shared catalog migration、cross-chain closure、new study creation、plan registration、Evaluation、
    replay、review、release 與 activation 各需自己的 exact gate 與 human authority，不得合併推定。
...
| Shared qualification-state migration | Stable repository identity、complete explicit source registry/checkpoint inventory、每個 source/shard digest/event-count/head、open-plan conflict projection、current human approver/time、commit decision、authority catalog、active chain/checkpoint、shared locks/journal、idempotent recovery 與 complete tracked snapshot replay。 |
| Qualification-plan abandonment | Exact original plan/shard/event identity、complete frozen study identity、canonical terminal `cancelled` owning-study state、active/effective authorizing release/capability、current stable human approver/time/concrete reason、catalog 與 active-chain prior heads、canonical abandonment event/hash 與 matching catalog/shard/checkpoint snapshot replay。 |
| Cross-chain invalidated-plan closure | Exact imported plan/shard/event identity、superseded paused owning-study lifecycle、accepted exact `close-invalidated` impact evidence/digest、active authorizing release/capability、current human approval/reason、prior shared heads、canonical `historical_plan_closed_invalidated` event 與 global replay。 |
...
Private shared qualification state 位於 verified Git-common runtime root，永不 commit。Frozen
logical identity 與 operational absolute path 必須分開保存；tracked qualification snapshot 必須
自包含 catalog、全部 referenced shards/checkpoints 與 active chain/head，使 fresh clone 可
provider-free replay，但不得由 tracked snapshot 反向授權 private mutation。
...
Shared qualification authority 由 maintained runtime-state resolver、catalog/shard validator、
repository-wide lock 與 guarded migration/cross-chain administration writers 實作。所有 study
compiler、plan/screen/challenge/replay、terminal evidence 與 completion locks 必須解析同一 shared
authority；任何仍直接建立 worktree-local default registry 的 public mutation path 都是 release
blocker。
...
Shared repository identity、migration inventory、catalog、任一 shard/checkpoint、active chain/head、
cross-chain terminal binding 或 shared lock/journal failure 同樣立即 pause 全部 qualification
mutation。發現未註冊的 legacy registry 時不得只把它當作備份或忽略。
...
Shared-state recovery 只能完成已 commit 且 exact inventory/digests/approver/prior heads 全部相同的
migration 或 terminal decision；任何新增 source、不同 bytes 或 catalog drift 都需要新 preview
與新 approval。
...
Superseded-version paused study 的 open plan 只有在 accepted impact 明列 exact
`close-invalidated`、shared migration 完成且另取當下核准後，才可 append cross-chain invalidated
closure。缺少任一條件時 study 與 plan 都保持原狀。
## Version boundary
...
- shared runtime repository identity、authority catalog/shard schema、global plan projection、
  cross-chain terminal vocabulary、migration inventory 或 worktree-local compatibility boundary。
...
本 v011 draft 整合 accepted v010/C001 shared qualification state authority，並完整保留 v010 的
fixed-calendar retrospective、qualification-plan abandonment、workflow release safety、explicit
activation 與 terminal legacy archive boundaries。Authoring 不改變任何既有 registry、plan 或
study bytes。

Completed v008/v009 studies 保持 terminal。Cancelled v009/S002 保持 `cancelled`；其 exact open plan
只有在 v011 effective、complete shared migration 與 separate current cross-chain closure approval
後才可行政關閉。Paused v008/S003 保持 pinned to v008；accepted impact decision 為
`close-invalidated`，但只有 v011 guarded event 可關閉其 plan，且不改寫 study lifecycle。Paused
v009/S003 保持 pinned to v009；若 v011 activation 前尚未建立 v010 successor study，其
version-boundary decision 為 `restart-on-v011`。任何 successor 必須使用 target version 的 next
CLI-allocated local study、exact `revisits` 與 truthful `known-contaminated` disclosure。v010 目前無
study；本 draft 不建立任何 study。

對 Active v010 與 Draft v011 的 release-safety assessment，writer 必須配置下一個永不重用的
`SAxxx`，從 repository 取得兩個 exact version paths、v010 `RELEASE.json` digest、v011
`WORKFLOW.md` digest 與 blocking v010 studies 的當下狀態，再以 current UTC 與 stable actor
add-only 寫入 `ASSESSMENT.json`。當下 v010 為 N04 且沒有 unfinished study，所以不得製造空的
assessment；若 release 前狀態改變，guarded release check 必須重新判定。

關閉 assessment 前，每個 blocking study 必須已是 `paused`、`completed` 或 `cancelled`。
Paused predecessor study 必須明確選擇 `continue-on-v010`、`restart-on-v011` 或
...
本 v011 若成為 Active，`workflow-release-safety-v1` 以相同 add-only boundary 治理 v012 與後續
...
本 v011 的 workflow release lifecycle 為 `draft -> prepared -> active`。Release preparation
...
時已有效，不回填歷史 activation 時間。v010 與後續版本只能使用上述 explicit Workflow
...
| `docs/historical-qualification-and-shadow-v008.md` | normative | Exact-study readiness、frozen selection boundaries、screen replay 與 evidence mechanics；其 caller-selected calendar、prospective Shadow 與 promotion semantics 不適用於本版本。 |
...
| `docs/shared-qualification-state-v011.md` | normative | Git-common shared private authority、complete-inventory immutable shard migration、global plan projection、cross-chain administration、evidence replay 與 failure boundary。 |
| `docs/controlled-followup-cutover.md` | reference | 只供 historical ledger/parity mechanics 參考；Active promotion、position ownership 與 order authority 不適用於本版本 study。 |
