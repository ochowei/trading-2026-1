# Result validity and trial history (v005)
...
Qualification plans may additionally carry an `evidence_role` and frozen `evidence_audit`.
`retrospective-confirmatory` requires one of the three explicit provenance classifications and
an exact `retrospective_selection_checkpoint`, and projects only `retrospectively-supported` or
`retrospective-screen-failed`; result validity rejects
any attempt to attach Shadow evidence to that role. Historical plans with an explicit audit require
`verified-clean` and complete trial history. Older Historical events omit these additive fields and
remain byte-stable and verifiable.
The qualification lifecycle is documented in
[historical-qualification-and-shadow-v005.md](historical-qualification-and-shadow-v005.md).
