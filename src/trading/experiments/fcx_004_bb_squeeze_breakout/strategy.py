"""
FCX-004: Bollinger Band Squeeze Breakout 策略
FCX BB Squeeze Breakout Strategy

以波動收縮後的突破取代均值回歸，捕捉 FCX 銅礦趨勢驅動的爆發性上漲。
基於 TSLA-005/NVDA-003 成功經驗移植。
"""
...
from trading.experiments.fcx_004_bb_squeeze_breakout.signal_detector import (
    FCXBBSqueezeDetector,
)
...
class FCXBBSqueezeBreakoutStrategy(ExecutionModelStrategy):
    ...
    def create_detector(self) -> BaseSignalDetector:
        return FCXBBSqueezeDetector(create_default_config())

    def _print_strategy_params(self, config: ExperimentConfig) -> None:
        if isinstance(config, FCXBBSqueezeConfig):
