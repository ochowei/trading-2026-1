...
使用 DIA-003 的 RSI(2) 進場架構，測試 TP +3.0%（原 +2.5%）。
"""
...
from trading.experiments.dia_004_wider_tp.signal_detector import (
    DIARsi2WiderTPSignalDetector,
)
...
class DIARsi2WiderTPStrategy(ExecutionModelStrategy):
    ...
    def create_detector(self) -> BaseSignalDetector:
        return DIARsi2WiderTPSignalDetector(create_default_config())

    def _print_strategy_params(self, config: ExperimentConfig) -> None:
        if isinstance(config, DIARsi2WiderTPConfig):
