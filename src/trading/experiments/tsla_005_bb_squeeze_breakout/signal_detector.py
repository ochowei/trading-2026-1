"""
TSLA-005 訊號偵測器：Bollinger Band Squeeze Breakout
TSLA-005 Signal Detector: BB Squeeze Breakout
...
Att2: 放寬擠壓條件（從「當日最低」改為「5日內曾低於25th百分位」）。
"""
...
from trading.experiments.tsla_005_bb_squeeze_breakout.config import TSLABBSqueezeConfig

logger = logging.getLogger(__name__)


class TSLABBSqueezeDetector(BaseSignalDetector):
    """TSLA BB Squeeze Breakout 訊號偵測器"""

    def __init__(self, config: TSLABBSqueezeConfig):
        self.config = config
    ...
    def detect_signals(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        logger.info("TSLA: Detected %d BB squeeze breakout signals", signal_count)
        return df
