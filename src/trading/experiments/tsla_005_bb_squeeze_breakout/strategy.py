"""
TSLA-005: Bollinger Band Squeeze Breakout 策略
TSLA BB Squeeze Breakout Strategy

以波動收縮後的突破取代均值回歸，捕捉 TSLA 動量驅動的爆發性上漲。
"""
...
from trading.experiments.tsla_005_bb_squeeze_breakout.signal_detector import (
    TSLABBSqueezeDetector,
)
...
class TSLABBSqueezeBreakoutStrategy(ExecutionModelStrategy):
    ...
    def create_detector(self) -> BaseSignalDetector:
        return TSLABBSqueezeDetector(create_default_config())

    def _print_strategy_params(self, config: ExperimentConfig) -> None:
        if isinstance(config, TSLABBSqueezeConfig):
