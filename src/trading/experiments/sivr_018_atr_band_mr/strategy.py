"""
SIVR-018: Capitulation-Strength Filter MR
Uses ExecutionModelBacktester (next-open + 0.15% slippage + pessimistic intrabar).
"""

from trading.core.base_config import ExperimentConfig
from trading.core.base_signal_detector import BaseSignalDetector
from trading.core.execution_strategy import ExecutionModelStrategy
from trading.experiments.sivr_018_atr_band_mr.config import (
    SIVR018Config,
    create_default_config,
)
from trading.experiments.sivr_018_atr_band_mr.signal_detector import (
    SIVR018SignalDetector,
)


class SIVR018Strategy(ExecutionModelStrategy):
    """SIVR Capitulation-Strength Filter MR (SIVR-018)"""

    slippage_pct: float = 0.0015

    def create_config(self) -> ExperimentConfig:
        return create_default_config()

    def create_detector(self) -> BaseSignalDetector:
        return SIVR018SignalDetector(create_default_config())

    def _print_strategy_params(self, config: ExperimentConfig) -> None:
        if isinstance(config, SIVR018Config):
            print(
                f"  Pullback: {config.pullback_lookback}d high"
                f" >= {abs(config.pullback_threshold):.0%}"
                f", cap <= {abs(config.pullback_cap):.0%}"
            )
            print(f"  Williams %R({config.wr_period}) <= {config.wr_threshold}")
            if config.use_rsi_hook:
                print(
                    f"  RSI({config.rsi_period}) Bullish Hook: "
                    f"lookback {config.rsi_hook_lookback}d"
                    f" / delta >= {config.rsi_hook_delta}"
                    f" / near-low <= {config.rsi_hook_max_min}"
                )
            if config.use_atr_band:
                print(
                    f"  ATR Band: {config.atr_ratio_floor} <= ATR("
                    f"{config.atr_short_period})/ATR({config.atr_long_period})"
                    f" <= {config.atr_ratio_ceiling}"
                )
            if config.use_1d_cap:
                print(f"  1d return cap: ret_1d >= {config.one_day_cap:.1%}")
            if config.use_3d_cap:
                print(f"  3d return cap: ret_3d >= {config.three_day_cap:.1%}")
            if config.use_3d_floor:
                print(f"  3d return floor: ret_3d <= {config.three_day_floor:.1%}")
            print(f"  Cooldown: {config.cooldown_days} days")
        super()._print_strategy_params(config)
