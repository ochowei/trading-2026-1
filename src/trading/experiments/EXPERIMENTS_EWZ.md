<!-- AI_CONTEXT_START - 此區塊供 AI Agent 快速讀取，人工更新
  ...
  phase9_note_2026_08_06: EWZ-005, EWZ-008, EWZ-009, and EWZ-010 now consume declared EEM/^VIX or EEM/BRL historical auxiliary bundles through the provider-free execution boundary; migration runs remain migration-pending and do not advance latest or followup qualification.
  phase9_note_2026_08_06_primary_wave: EWZ-001, EWZ-002, EWZ-003, EWZ-004, and EWZ-007 now use the shared primary-only verified bundle boundary; migration runs remain pending and do not advance latest or followup qualification.
  note_2026_05_17: EWZ-010 added 2026-05-17 (EWZ–BRL Currency-Regime-Gated Vol-Transition MR, **documented-failure；cross-asset divergence/currency regime gate family v4 driver-purity 第 5 個 driver-impure 失敗 subclass = COMMODITY-DRIVEN SINGLE-COUNTRY EM EQUITY vs OWN-COUNTRY CURRENCY (BRL)；延伸禁忌 #36e**, predict→confirm 預分析完全命中；FXI-015 精確 isomorph). base = EWZ-009 Att1 全域最優（min(A,B) 1.50，七條件 MR + EWZ–EEM 10d rel CEILING ≤ +2.5%；唯一殘餘 binding Part A SL = 2019-03-25 -4.1% stop_loss，sustained Brazil-specific weakness / up-day rebound after big drop，Brazil 退休金改革政局事件）+ 第八條件 EWZ–BRL 貨幣 regime gate（GLD-016 UUP / FXI-015 CNY 形式；BRL=X = USD/BRL，+ve = BRL 貶值）。**強制 predict→confirm 預分析（先做）決定性命中**：重現 EWZ-009 Att1 全 10+6 筆交易，唯一 binding Part A SL 2019-03-25 BRL_20d **+4.26** 落於 9 Part A winners [-0.26,+6.98] 正中央（介於 +4.02/+4.50），4 winners BRL 更弱（+4.50/+5.84/+6.98/+4.02），BRL_10d/EWZ−BRL rel_20d/rel_10d 全部維度皆完全交錯、無 ≥15pp robust plateau → 預測 documented-failure。三次迭代全 CONFIRMED vs baseline min 1.50：Att1（BRL 20d CEILING ≤ +4.0%，弱-BRL risk-off 直覺）Part A 10→4(+1 zero-var=5)/90%→100%/Sharpe 1.50→**0.00** zero-var/cum +48.77%→**+27.63%**（-43%）/ Part B 6/83.3%/1.82 ≈不變（cooldown chain-shift neutral，2024-11-29→2024-12-02）→ degenerate over-filter REJECT — 非外科式同殺 4 winners（BRL_20d>+4.0），FCX-016 Att3 / TLT-014 Att2 判例（TSLA-018 Att1 / SIVR-019 Att1 / FXI-015 Att1 同構）；Att2（CEILING 收緊 ≤ +2.0%）Part A 10→3/100% zero-var/cum **+15.76%**（-68%）/ Part B 6→**1**/83.3%→**0.0%**/cum +25.52%→**-0.84%**（Part B 崩潰，唯一存活 2024-01-18 time_expiry -0.84% 為虧損）/ min **≈0.00** catastrophic — 不隔離 interleaved SL 反連殺所有持平/弱-BRL winners（EWT-010 Att2 / FXI-015 Att2 inverted 同構）；Att3（EWZ−BRL 20d relative-divergence FLOOR ≥ -12.0%，TLT-014/GLD-016 Att2/FXI-015 Att3 精確類比）Part A 10→8/SL 2019-03-25 **RETAINED**（rel20 -11.05 ≥ -12.0 通過，gate 對 SL 非綁定）/90%→87.5%/Sharpe 1.50→**1.28**（-15%）/cum +48.77%→+34.94% / Part B 6→5/Sharpe 1.82→1.64/cum +25.52%→+20.53% / min **1.28**（-15%）FAIL — 反殺最深背離 winners 2021-10-21(-17.07)/2022-09-28(-13.10)，EWZ vol(2.18%) ≫ BRL=X vol(1.06%) 使 rel ≈ EWZ 動量、零獨立 BRL 區分力（SIVR-019 Att2 / FXI-015 Att3 同構）。**核心跨資產規則（延伸禁忌 #36e / family v4 driver-purity 前置條件）**：country-specific 貨幣（BRL）regime gate 亦不可移植至 commodity-driven 單一國家 EM 股票 ETF——成功前提 = 資產主驅動須「即」該貨幣（gold↔USD，GLD-016 SUCCESS）；EWZ 主驅動為大宗商品（Vale 鐵礦砂 / Petrobras 原油）+ 巴西財政/政局，非 BRL 匯率本身，driver-impure，殘餘 binding SL 2019-03-25 為巴西退休金改革政局/country-specific 持續性疲弱事件、與在地貨幣 regime 結構性解耦且**反向**（最強 winners 2021-10-21/2022-09-28 發生於最弱-BRL regime，因 BRL 貶值常伴隨 commodity-EM risk-on 反彈 EWZ MR 正獲利於此）。EWZ-010 加入 EWJ-006/EEM-016/INDA-013/TSM-012/TSLA-018/FXI-015「idiosyncratic non-separable single residual SL」family，為 family v4 第 5 個 driver-impure subclass（區別於 SIVR-019 industrial-metal-vs-USD / EWT-010 component-vs-parent / TSLA-018 high-vol-growth-stock-vs-USD / FXI-015 policy-driven-EM-equity-vs-CNY）。predict→confirm 軌跡 3 SUCCESS（GLD-016/VOO-005/URA-014）+ 13 documented-failure 全部命中。EWZ-009 Att1 仍為 EWZ 全域最優（10 次實驗、35+ 次嘗試）。
...
**當前最佳：** ★ **EWZ-009 Att1**（EWZ-EEM Relative Strength Divergence Filter on Vol-Transition MR：EWZ-007 Att3 完整框架 + **EWZ-EEM 10 日報酬差 ≤ +2.5pp** cross-asset pair-divergence regime gate，TP+5%/SL-4%/18天）★ **2026-05-08 新全域最優（10 次實驗、35+ 次嘗試；EWZ-010 EWZ–BRL currency-regime documented-failure 三次迭代全 CONFIRMED 未撼動，見禁忌 #36e）**
...
- **EWZ–BRL 20d CEILING <= +4.0%（EWZ-010 Att1）**：min(A,B) **REJECT**（vs EWZ-009 Att1 1.50）。Part A 10→5 zero-var WR 100% Sharpe 0.00 cum +48.77%→+27.63%（-43%）/ Part B 6/83.3%/1.82 ≈不變。predict→confirm 預測命中：SL 2019-03-25 BRL_20d +4.26 與 9 winners [-0.26,+6.98] 完全交錯，CEILING 移除 SL 但非外科式同殺 4 winners（BRL_20d>+4.0：+4.50/+5.84/+6.98/+4.02），degenerate over-filter（FCX-016 Att3 / TLT-014 Att2 判例；TSLA-018 Att1 / SIVR-019 Att1 / FXI-015 Att1 同構）
- **EWZ–BRL 20d CEILING <= +2.0%（EWZ-010 Att2）**：min(A,B) **≈0.00 catastrophic**（vs 1.50）。Part A 10→3 zero-var cum +15.76%（-68%）/ Part B 6→**1**/WR 0.0%/cum -0.84%（Part B 崩潰，唯一存活 2024-01-18 time_expiry -0.84% 為虧損）。收緊 CEILING 不隔離 interleaved SL 反連殺所有持平/弱-BRL winners + Part B 5/6 winners（BRL_20d>+2.0）（EWT-010 Att2 / FXI-015 Att2 inverted 同構）
- **EWZ–BRL 20d relative-divergence FLOOR >= -12.0%（EWZ-010 Att3）**：min(A,B) **1.28**（vs 1.50，-15%）FAIL。Part A 10→8 / SL 2019-03-25 **RETAINED**（rel20 -11.05 ≥ -12.0 通過 gate 非綁定）/ WR 90%→87.5% / Sharpe 1.50→1.28 / Part B 6→5 / Sharpe 1.82→1.64。FLOOR 反殺最深背離 winners 2021-10-21(rel20 -17.07)/2022-09-28(-13.10) + Part B 2024-06-10(-12.01)；EWZ vol(2.18%) ≫ BRL=X vol(1.06%) 使 rel ≈ EWZ 動量、零獨立 BRL 區分力（SIVR-019 Att2 / FXI-015 Att3 同構）。**三次迭代全 documented-failure 全部命中 → family v4 driver-purity 第 5 個 driver-impure subclass：commodity-driven single-country EM equity vs own currency (BRL)，禁忌 #36e**
...
- **EWZ–BRL 貨幣 regime gate（EWZ-010 三次迭代，全部 documented-failure 預測命中）**：BRL=X 20d CEILING Att1 ≤ +4.0%（degenerate over-filter REJECT，Part A 10→5 zero-var）/ Att2 ≤ +2.0%（catastrophic，Part B 6→1 崩潰）/ EWZ−BRL 20d divergence FLOOR Att3 ≥ -12.0%（min 1.28，-15% FAIL，SL RETAINED）。唯一殘餘 binding Part A SL 2019-03-25 於 BRL_20d/BRL_10d/EWZ−BRL rel_20d/rel_10d 全部維度與 9 winners 完全交錯，無 ≥15pp robust plateau。**family v4 driver-purity 第 5 個 driver-impure subclass：commodity-driven single-country EM equity vs own currency (BRL)，FXI-015 精確 isomorph，禁忌 #36e**
...
- ~~EWZ–BRL 本國貨幣 regime gate（CEILING / divergence FLOOR）~~ → EWZ-010 三次迭代全 documented-failure（預測命中）：commodity-driven 單一國家 EM 股票 ETF driver-impure（主驅動為大宗商品+巴西政局非 BRL），殘餘 binding SL 2019-03-25 與在地貨幣 regime 解耦且反向，family v4 driver-purity 第 5 個 driver-impure subclass，禁忌 #36e（FXI-015 CNY isomorph）
...
| EWZ-005 | `ewz_005_rs_momentum`            | RS 動量回調 EWZ vs EEM (Att1-3；Phase 9 auxiliary bundle，migration-pending)          | ❌ 未超越 EWZ-002 |
...
| EWZ-008 | `ewz_008_vix_implied_vol_mr`     | ^VIX Forward-Looking Implied-Vol Regime-Gated MR（lesson #24 family v2 變體；Phase 9 auxiliary bundle，migration-pending）| ❌ 三次迭代均失敗（lesson #24 family 在 EM single-country ETF 結構性失敗） |
| EWZ-009 | `ewz_009_ewz_eem_divergence_mr`  | EWZ-EEM Relative Strength Divergence Filter on Vol-Transition MR（lesson #20 v3 cross-asset divergence regime gate，repo 第 6 次跨資產 + 首次 EM single-country ETF + 首次 broad benchmark anchor 變體；Phase 9 auxiliary bundle，migration-pending）★ | ✅ 當前最佳 |
| EWZ-010 | `ewz_010_brl_regime_mr`          | EWZ–BRL Currency-Regime-Gated Vol-Transition MR（cross-asset divergence/currency regime gate family v4 driver-purity 第 5 個 driver-impure subclass：commodity-driven single-country EM equity vs own currency BRL，FXI-015 精確 isomorph，禁忌 #36e；Phase 9 auxiliary bundle，migration-pending）| ❌ 三次迭代全 documented-failure（predict→confirm 命中） |
...
## EWZ-010: EWZ–BRL Currency-Regime-Gated Vol-Transition MR ❌ 三次迭代全 documented-failure

**目標**：延伸 EWZ-009 Att1 全域最優（min(A,B) Sharpe **1.50**，七條件 MR + EWZ–EEM 10d rel CEILING ≤ +2.5%），新增第八條件 **EWZ–BRL 本國貨幣 regime gate**（GLD-016 UUP USD-regime / FXI-015 CNY-regime 形式），目標移除 EWZ-009 Att1 唯一殘餘 binding Part A SL **2019-03-25**（-4.1% stop_loss，sustained Brazil-specific weakness / up-day rebound after big drop，巴西退休金改革政局事件）。

**結論：三次迭代全 documented-failure，predict→confirm 預分析完全命中。EWZ-009 Att1 仍為 EWZ 全域最優。** 本實驗為 **cross-asset divergence/currency regime gate family v4 driver-purity 第 5 個 driver-impure 失敗 subclass：COMMODITY-DRIVEN SINGLE-COUNTRY EM EQUITY vs OWN-COUNTRY CURRENCY (BRL)**，延伸禁忌 #36e（FXI-015 policy-driven-EM-equity-vs-CNY 之精確 isomorph）。

跨資產脈絡（family v4 driver-purity 前置條件）：
- **DRIVER-PURE 單因子反向 = SUCCESS**：TSLA-017（TSLA−QQQ）/ TLT-014（TLT−SPY）/ GLD-016（GLD−USD，純貨幣 USD-inverse）
- **DRIVER-IMPURE = documented-failure**：SIVR-019 industrial-metal-vs-USD / EWT-010 component-vs-parent / TSLA-018 high-vol-growth-stock-vs-USD / FXI-015 policy-driven-EM-equity-vs-own-currency(CNY) / **EWZ-010 commodity-driven-EM-equity-vs-own-currency(BRL)（本實驗，第 5 個）**

### 強制 predict→confirm 預分析（先於建構，重現 EWZ-009 Att1 全 10+6 筆交易）

EWZ-009 Att1 Part A：10 筆，唯一 SL = 2019-03-25（-4.1%），其餘 9 筆全 +5% TP。BRL=X = USD/BRL（+ve N 日報酬 = BRL 貶值 = 巴西 risk-off 直覺）。

| 日期 | 類型 | EWZ_20d | BRL_20d | EWZ−BRL_rel20 |
|------|------|---------|---------|---------------|
| **2019-03-25** | **SL** | **-6.79%** | **+4.26%** | **-11.05pp ← 唯一殘餘 binding SL** |
| 2019-05-14 | TP | -4.06% | +2.84% | -6.90pp |
| 2019-08-02 | TP | -2.92% | +0.96% | -3.88pp |
| 2019-11-13 | TP | +0.09% | -0.26% | +0.35pp |
| 2020-11-02 | TP | -4.26% | +1.05% | -5.32pp |
| 2021-01-22 | TP | -4.26% | +4.50% | -8.77pp |
| 2021-07-07 | TP | -5.54% | +2.98% | -8.51pp |
| 2021-10-21 | TP | -11.24% | +5.84% | -17.07pp |
| 2022-09-28 | TP | -6.12% | +6.98% | -13.10pp |
| 2023-10-04 | TP | -5.04% | +4.02% | -9.06pp |

Part B（6 筆全勝）BRL_20d ∈ [-0.26%, +4.37%]、EWZ−BRL_rel20 ∈ [-12.50pp, -3.45pp]。

**預分析判定（NOT separable → 預測 documented-failure）**：
- **BRL_20d**：SL +4.26 落於 9 winners [-0.26, +6.98] 正中央（介於 +4.02 與 +4.50），**4 winners BRL 更弱**（+4.50/+5.84/+6.98/+4.02）——完全交錯、無 ≥15pp robust plateau；**方向結構性反轉鐵證 = 最強 winners 2021-10-21/2022-09-28 發生於最弱-BRL regime**（BRL 貶值常伴隨 commodity-EM risk-on 反彈，EWZ MR 正獲利於此）
- **BRL_10d**：SL +2.43 落於 winners [+0.69, +6.18] 正中央，5 winners 更弱 → 同樣交錯
- **EWZ−BRL_rel20**：SL -11.05 落於 winners [-17.07, +0.35] 中段 → CEILING/FLOOR 兩向皆不可分；EWZ 日波動 2.18% ≫ BRL=X 1.06%（~2x）使 rel ≈ EWZ 動量、零獨立 BRL 區分力
- Part B 全 6 winners 與 SL/winners 完全重疊；任何足以咬住 SL 的 CEILING 必同殺 Part B winners → Part B 崩潰

### 三次迭代結果（FXI-015 精確 isomorph，全 CONFIRMED vs EWZ-009 Att1 min 1.50）

| 迭代 | gate | Part A | Part B | min(A,B) | 結果 |
|------|------|--------|--------|----------|------|
| Att1 | BRL 20d CEILING ≤ +4.0% | 10→5 / WR 100% / Sharpe **0.00** zero-var / cum +27.63%（-43%） | 6/83.3%/1.82 ≈不變 | **REJECT** | degenerate over-filter — 非外科式同殺 4 winners（BRL_20d>+4.0），TSLA-018 Att1 / FXI-015 Att1 同構 |
| Att2 | BRL 20d CEILING ≤ +2.0% | 10→3 / WR 100% zero-var / cum +15.76%（-68%） | 6→**1** / WR **0.0%** / cum **-0.84%**（崩潰） | **≈0.00 catastrophic** | 不隔離 interleaved SL 反連殺所有持平/弱-BRL winners + Part B 5/6 winners，EWT-010 Att2 / FXI-015 Att2 inverted 同構 |
| Att3 | EWZ−BRL 20d divergence FLOOR ≥ -12.0% | 10→8 / SL 2019-03-25 **RETAINED** / WR 87.5% / Sharpe **1.28**（-15%）/ cum +34.94% | 6→5 / Sharpe 1.64 / cum +20.53% | **1.28（-15%）FAIL** | FLOOR 不隔離 interleaved SL（rel20 -11.05 通過）反殺最深背離 winners 2021-10-21(-17.07)/2022-09-28(-13.10) + Part B 2024-06-10(-12.01)；vol-mismatch 零區分力，SIVR-019 Att2 / FXI-015 Att3 同構 |

### 核心跨資產規則（延伸禁忌 #36e / family v4 driver-purity 前置條件）

1. **country-specific 貨幣（BRL）regime gate 不可移植至 commodity-driven 單一國家 EM 股票 ETF**——成功前提 = 資產主驅動須「即」該貨幣（gold↔USD，GLD-016 ★ SUCCESS）。EWZ 主驅動為大宗商品（Vale 鐵礦砂 / Petrobras 原油）+ 巴西財政/政局，**非 BRL 匯率本身**，driver-impure。
2. **殘餘 binding SL 2019-03-25 為巴西退休金改革政局/country-specific 持續性疲弱事件**，與在地貨幣 regime 結構性解耦且**反向**（最強 winners 2021-10-21/2022-09-28 發生於最弱-BRL regime）。
3. **family v4 driver-impure 失敗 subclass 現 5 類**：SIVR-019 industrial-metal-vs-USD / EWT-010 component-vs-parent / TSLA-018 high-vol-growth-stock-vs-USD / FXI-015 policy-driven-EM-equity-vs-CNY / **EWZ-010 commodity-driven-EM-equity-vs-BRL**。EWZ 加入 EWJ-006/EEM-016/INDA-013/TSM-012/TSLA-018/FXI-015「idiosyncratic non-separable single residual SL」family。
4. **predict→confirm 軌跡完整**：3 SUCCESS（GLD-016/VOO-005/URA-014）+ 13 documented-failure（含 EWZ-010 三次迭代）全部命中。EWZ-009 Att1 仍為 EWZ 全域最優（10 次實驗、35+ 次嘗試）。
