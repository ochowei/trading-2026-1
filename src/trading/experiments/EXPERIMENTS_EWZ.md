...
**當前最佳：** EWZ-007 Att3（Post-Capitulation Vol-Transition MR：EWZ-006 Att3 框架 + **1d cap >= -5.0%（surgical Petrobras filter）**，TP+5%/SL-4%/18天）★ **2026-04-28 新全域最優**
- Part A: Sharpe **0.95**, 累計 +42.67%, 11 訊號 (2.2/年), WR 81.8%
- Part B: Sharpe **1.82**, 累計 +25.52%, 6 訊號 (3.0/年), WR 83.3%（**與 baseline EWZ-006 完全相同**）
- min(A,B): **0.95**（+38% vs EWZ-006 的 0.69）
- A/B 年化訊號比 1.36:1（36% gap < 50% ✓），A/B 累計差 40.2%（>30% 目標，結構性）
- 1d cap 為 surgical filter：僅過濾 2021-02-22 Petrobras 暴跌（1d -6.19%，repo 中極少見的單日 outlier 事件），Part A 訊號 12→11，無 cooldown shift，Part B 全保留
- 驗證 lesson #19 family 對「商品/政治雙驅動 EM ETF」需切換至「outlier-event surgical filter」而非 regime-level threshold filter

**前任最佳：** EWZ-006 Att3（BB 下軌+回檔上限混合進場：BB(20,1.5) 下軌觸及 + 10日高點回檔上限 10% + WR(10)≤-80 + ClosePos≥40% + ATR(5)/ATR(20)>1.1 + 非對稱出場 TP+5%/SL-4%/18天）
...
**前前任最佳：** EWZ-002 Att3（波動率自適應回檔+WR，min(A,B) Sharpe 0.34）
...
| 參數 | EWZ-001 | EWZ-002 Att3 | EWZ-005 Att1 | EWZ-006 Att3 | **EWZ-007 Att3 ★** |
|------|---------|---------------|--------------|--------------|---------------------|
| 進場框架 | 10日回檔≥7% + WR(10)≤-80 | 10日回檔 7-10% + WR(10)≤-80 | RS(20d)≥3% + 5日回調3-7% + SMA(50) | BB(20,1.5) 下軌 + 10日回檔上限 10% | **BB(20,1.5) 下軌 + 10日回檔上限 10%** |
| ClosePos | 無 | ≥40% | 無 | ≥40% | **≥40%** |
| ATR 過濾 | 無 | ATR(5)/ATR(20)>1.1 | 無 | ATR(5)/ATR(20)>1.1 | **ATR(5)/ATR(20)>1.1** |
| Capitulation filter | 無 | 無 | 無 | 無 | **1d cap >= -5.0%** |
| TP | +4.0% | +5.0% | +5.0% | +5.0% | **+5.0%** |
| SL | -4.0% | -4.0% | -4.0% | -4.0% | **-4.0%** |
| 持倉 | 15天 | 18天 | 18天 | 18天 | **18天** |
| 冷卻期 | 10天 | 10天 | 10天 | 10天 | **10天** |
| **Part A Sharpe** | **0.10** | **0.34** | **0.46** | **0.69** | **0.95** |
| **Part B Sharpe** | **0.60** | **12.85** | **-0.33** | **1.82** | **1.82** |
| **min(A,B) Sharpe** | **0.10** | **0.34** | **-0.33** | **0.69** | **0.95 ★** |
...
| EWZ-007 | `ewz_007_vol_transition_mr`      | Post-Capitulation Vol-Transition MR（1d cap surgical Petrobras filter）★ | ✅ 當前最佳 |
