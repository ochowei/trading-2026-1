...
from trading.experiments.sivr_003_pullback_wr.config import SIVRPullbackWRConfig

logger = logging.getLogger(__name__)


class SIVRPullbackWRSignalDetector(BaseSignalDetector):
    """
    SIVR 回檔 + Williams %R 訊號偵測器
    ...
    def __init__(self, config: SIVRPullbackWRConfig):
        self.config = config
    ...
    def detect_signals(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        logger.info("SIVR: Detected %d Pullback+WR reversion signals", signal_count)
        return df
