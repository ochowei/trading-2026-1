"""
XBI-020 訊號偵測器：^VIX Implied-Vol DIRECTION Regime Gate Pullback MR
...
6. (VIX_Close − VIX_Close.shift(vix_direction_lookback)) <= max_vix_change
   （XBI-020 新增 lesson #24 DIRECTION CEILING：排除 VIX 急升訊號）
7. 冷卻 cooldown_days 個交易日

設計依據：lesson #24 family DIRECTION 變體（repo 首次於 XBI）。XBI-017 為
BANDS 變體（VIX level 區間），XBI-020 試驗 DIRECTION 變體（VIX 上升速率
...
from trading.experiments.xbi_020_vix_direction_mr.config import XBI020Config

logger = logging.getLogger(__name__)


class XBI020VixDirectionMRDetector(DeclaredAuxiliaryData, BaseSignalDetector):
    """XBI-020 訊號偵測器"""

    def __init__(self, config: XBI020Config):
        self.config = config
    ...
    def detect_signals(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        if suppressed:
            ...
            logger.info(
                "XBI-020: %d duplicate signals suppressed by cooldown",
                len(suppressed),
            )

        signal_count = df["Signal"].sum()
        logger.info(
            "XBI-020: Detected %d VIX-direction-filtered MR signals",
            signal_count,
        )
        return df
