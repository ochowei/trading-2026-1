"""
GLD 回檔 + Williams %R + 反轉K線均值回歸策略
(GLD Pullback + Williams %R + Reversal Candle Mean Reversion Strategy)
...
from trading.experiments.gld_007_pullback_wr_reversal.signal_detector import (
    GLDPullbackWRReversalSignalDetector,
)
...
class GLDPullbackWRReversalStrategy(ExecutionModelStrategy):
    ...
    def create_detector(self) -> BaseSignalDetector:
        return GLDPullbackWRReversalSignalDetector(create_default_config())
    ...
    def _print_strategy_params(self, config: ExperimentConfig) -> None:
        if isinstance(config, GLDPullbackWRReversalConfig):
            print(
