"""
TSLA-012 訊號偵測器：BB Squeeze Breakout Variants
TSLA-012 Signal Detector: BB Squeeze Breakout Variants

Att3 進場條件（全部滿足）：
1. 過去 5 日內 BB Width 曾低於 60 日 30th 百分位（近期波動收縮）
2. 收盤價 > Upper BB(20,2.0)（突破上軌）
3. 收盤價 > SMA(100)（長期趨勢向上，比 SMA(50) 更保守）
4. 冷卻期 10 個交易日
"""
...
class TSLAVolumeBreakoutDetector(BaseSignalDetector):
    """TSLA BB Squeeze Breakout Variants 訊號偵測器"""

    def __init__(self, config: TSLAVolumeBreakoutConfig):
        self.config = config
    ...
    def compute_indicators(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        # Recent squeeze check
        recent = self.config.bb_squeeze_recent_days
        min_days = self.config.bb_squeeze_min_days
        if min_days <= 1:
            # Original logic: any day in recent window was a squeeze
            df["Recent_Squeeze"] = df["BB_Width_Pct"].rolling(recent, min_periods=1).max() >= 1.0
        else:
            # Extended squeeze: require min_days squeeze days in window
            df["Squeeze_Count"] = df["BB_Width_Pct"].rolling(recent, min_periods=1).sum()
            df["Recent_Squeeze"] = df["Squeeze_Count"] >= min_days
    ...
    def detect_signals(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        logger.info(
            "TSLA: Detected %d BB squeeze breakout signals (SMA %d)",
            signal_count,
            self.config.sma_trend_period,
        )
        return df
