"""
INDA-015 訊號偵測器：Forward-Looking Implied-Vol Regime Gate on
...
7. **隱含波動率 regime gate（INDA-015 核心新增）**：
...
class INDA015SignalDetector(BaseSignalDetector):
    ...
    def detect_signals(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        if suppressed:
            ...
            logger.info("INDA-015: %d signals suppressed by cooldown", len(suppressed))

        signal_count = df["Signal"].sum()
        logger.info(
            "INDA-015: Detected %d implied-vol-regime-gated capitulation signals",
            signal_count,
        )
        return df
