"""FXI–CNY Currency-Regime-Gated MR 訊號偵測器 (FXI-017)
...
7. FXI–CNY 貨幣 regime gate（FXI-017 核心新增）：
...
class FXI017SignalDetector(BaseSignalDetector):
    ...
    def detect_signals(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        if suppressed:
            ...
            logger.info("FXI-017: %d signals suppressed by cooldown", len(suppressed))

        signal_count = df["Signal"].sum()
        logger.info("FXI-017: Detected %d CNY-regime-gated MR signals", signal_count)
        return df
