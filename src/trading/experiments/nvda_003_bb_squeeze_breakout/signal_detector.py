"""
NVDA-003 訊號偵測器：Bollinger Band Squeeze Breakout
NVDA-003 Signal Detector: BB Squeeze Breakout
...
from trading.experiments.nvda_003_bb_squeeze_breakout.config import NVDABBSqueezeConfig

logger = logging.getLogger(__name__)


class NVDABBSqueezeDetector(BaseSignalDetector):
    """NVDA BB Squeeze Breakout 訊號偵測器"""

    def __init__(self, config: NVDABBSqueezeConfig):
        self.config = config
    ...
    def detect_signals(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        logger.info("NVDA: Detected %d BB squeeze breakout signals", signal_count)
        return df
