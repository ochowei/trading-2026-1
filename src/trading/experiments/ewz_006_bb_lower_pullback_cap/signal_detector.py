"""
EWZ-006 訊號偵測器：BB 下軌 + 回檔上限混合進場均值回歸
...
from trading.experiments.ewz_006_bb_lower_pullback_cap.config import EWZ006Config

logger = logging.getLogger(__name__)


class EWZ006SignalDetector(BaseSignalDetector):
    def __init__(self, config: EWZ006Config):
        self.config = config
    ...
    def detect_signals(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        if suppressed:
            ...
            logger.info("EWZ-006: %d signals suppressed by cooldown", len(suppressed))

        signal_count = df["Signal"].sum()
        logger.info("EWZ-006: Detected %d BB lower + cap hybrid signals", signal_count)
        return df
