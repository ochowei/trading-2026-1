...
from trading.experiments.inda_008_bb_lower_pullback_cap.config import INDA008Config

logger = logging.getLogger(__name__)


class INDA008SignalDetector(BaseSignalDetector):
    def __init__(self, config: INDA008Config):
        self.config = config
    ...
    def detect_signals(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        if suppressed:
            ...
            logger.info("INDA-008: %d signals suppressed by cooldown", len(suppressed))

        signal_count = df["Signal"].sum()
        logger.info("INDA-008: Detected %d BB Lower Band + Cap hybrid signals", signal_count)
        return df
