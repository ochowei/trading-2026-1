"""
INDA-014 訊號偵測器：DXY Direction Filter on Multi-Period Capitulation MR
...
7. **DXY N 日報酬 <= max_dxy_change（INDA-014 核心新增：USD 強度方向過濾）**
8. 冷卻期 7 個交易日
...
class INDA014SignalDetector(BaseSignalDetector):
    ...
    def detect_signals(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        if suppressed:
            ...
            logger.info("INDA-014: %d signals suppressed by cooldown", len(suppressed))

        signal_count = df["Signal"].sum()
        logger.info(
            "INDA-014: Detected %d DXY-direction-gated capitulation signals",
            signal_count,
        )
        return df
