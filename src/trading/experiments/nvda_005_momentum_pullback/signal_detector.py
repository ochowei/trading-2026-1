"""
NVDA-005 訊號偵測器：Momentum Pullback
NVDA-005 Signal Detector: Momentum Pullback
...
from trading.experiments.nvda_005_momentum_pullback.config import NVDAMomentumPullbackConfig

logger = logging.getLogger(__name__)


class NVDAMomentumPullbackDetector(BaseSignalDetector):
    """NVDA Momentum Pullback 訊號偵測器"""

    def __init__(self, config: NVDAMomentumPullbackConfig):
        self.config = config
    ...
    def detect_signals(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        logger.info("NVDA: Detected %d momentum pullback signals", signal_count)
        return df
