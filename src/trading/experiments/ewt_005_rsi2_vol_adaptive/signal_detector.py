"""
EWT-005 訊號偵測器：RSI(2) 波動率自適應均值回歸
...
from trading.experiments.ewt_005_rsi2_vol_adaptive.config import EWT005Config

logger = logging.getLogger(__name__)


class EWT005SignalDetector(BaseSignalDetector):
    def __init__(self, config: EWT005Config):
        self.config = config
    ...
    def detect_signals(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        df["Signal"] = cond_rsi & cond_reversal & cond_vol
        ...
        logger.info("EWT-005: Detected %d RSI(2) vol-adaptive signals", signal_count)
        return df
