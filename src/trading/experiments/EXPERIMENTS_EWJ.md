...
- EWJ-006 Att1：EWJ-005 Att2 框架 + ^VIX 3d DIRECTION <= +5.0（lesson #24 sweet-spot port）→ Part A 0.60/Part B 不變, min 0.60（-14%，誤殺 winners 保留 SL）
- EWJ-006 Att2：EWJ-005 Att2 框架 + ^VIX 3d DIRECTION <= +3.0（加嚴）→ Part A 0.42/Part B 3/3, min 0.42（-40%，誤殺更多 winners + Part B）
- EWJ-006 Att3：EWJ-005 Att2 框架 + ^VIX LEVEL CAP <= 25.0 → Part A 5/5 nominal 2.97 / Part B 3/3 **REJECT**（非外科式 attrition，9→5/-44%、4→3/-25%，連帶切 3 筆 +3.5% TP + 1 Part B winner）
...
- ~~^VIX forward-looking implied-vol regime gate（lesson #24 family）~~ → EWJ-006 三次嘗試（3d DIRECTION +5.0/+3.0、LEVEL CAP <=25.0）全部 FAILED/REJECT，^VIX 對 EWJ 結構性無區分力（2022-09-01 SL 非 ^VIX 任一維度 outlier）
...
| 參數 | EWJ-001 | EWJ-002 Att2 | EWJ-003 Att3 | EWJ-005 Att2★ | EWJ-006 (FAIL) |
|------|---------|---------------|---------------|---------------|----------------|
| 進場框架 | 固定回檔≥3% | 固定回檔 3-7% | BB(20,1.5) 下軌 + 回檔上限 7% | BB(20,1.5) 下軌 + 回檔上限 7% | 同 EWJ-005 Att2 |
...
| Part A Sharpe | 0.16 | 0.55 | 0.60 | **0.70** | 0.60/0.42/2.97† |
| Part B Sharpe | 0.24 | 2.06 | 0.00 (std=0) | 0.00 (std=0) | 0.00 (std=0) |
| Part B WR | 71.4% | 83.3% | 100.0% | **100.0%** | 100%/100%/100% |
| **min(A,B)** | 0.16 | 0.55 | 0.60 | **0.70★** (Part A 為約束) | 0.60/0.42/REJECT |
...
| EWJ-006 | `ewj_006_vix_implied_vol_mr` | ^VIX Forward-Looking Implied-Vol Regime-Gated MR（EWJ-005 Att2 + ^VIX gate，3 次嘗試均失敗） | 已完成（失敗，^VIX 對 EWJ 結構性無區分力） |
...
## EWJ-006：^VIX Forward-Looking Implied-Vol Regime-Gated MR（失敗，^VIX 對 EWJ 結構性無區分力）
...
3. **EWJ-005 Att2 仍為全域最優**（6 次實驗、18 次嘗試），其 1d floor
...
EWJ-006 (^VIX Forward-Looking Implied-Vol Regime-Gated MR, 3 次嘗試均失敗)
