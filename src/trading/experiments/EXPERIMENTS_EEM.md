...
**當前最佳：** **EEM-014 Att2（新全域最優）**（EEM-012 Att3 所有條件 + **2DD floor <= -0.5%**：BB(20,2.0) 下軌 + 回檔上限 -7% + WR(10)≤-85 + ClosePos≥40% + ATR>1.1 + **2DD<= -0.5%** + TP+3%/SL-3%/20天 + 冷卻10天，Part A Sharpe **0.73**，Part B Sharpe 0.56，min(A,B) **0.56**）★ Repo 第 2 次「2DD floor 方向」成功驗證（繼 USO-013 後），勝過 EEM-012 Att3 +65%（0.34→0.56）。**19 個實驗 49+ 次嘗試。**
