...
**當前最佳：** EEM-005 Att2（BB Squeeze 30th 百分位 + SMA(50) + TP3.0%/SL3.0%/20天 + 冷卻10天，Part A Sharpe 0.20，Part B Sharpe 0.18）★ 突破策略大幅勝過均值回歸。**11 個實驗 27+ 次嘗試確認此為全域最優。**
...
- **EEM 技術面天花板已確認**：11 個實驗覆蓋均值回歸（含嚴格跌幅+ATR交叉組合）、突破、動量、RS、趨勢、政權過濾、出場優化、波動率過濾、ClosePos有效性驗證。EEM 的 EM 宏觀事件驅動特性限制純技術策略上限約 Sharpe 0.18-0.20。
<!-- AI_CONTEXT_END -->
