"""
EWJ-006 ^VIX Forward-Looking Implied-Vol Regime-Gated MR 訊號偵測器
...
from trading.experiments.ewj_006_vix_implied_vol_mr.config import EWJ006Config

logger = logging.getLogger(__name__)


class EWJ006SignalDetector(BaseSignalDetector):
    """
    ...
    7. ^VIX N 日累計變化 <= max_vix_change（EWJ-006 核心：forward-looking
    ...
    def compute_indicators(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        # ^VIX forward-looking implied vol gate（EWJ-006 核心新增）
    ...
    def detect_signals(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        for idx in baseline_idx:
            row = df.loc[idx]
            logger.info(
                "EWJ-006 [VIX-calib] %s | VIX=%.2f | 3d=%+.2f | 5d=%+.2f | 10d=%+.2f",
                idx.strftime("%Y-%m-%d"),
                row["VIX_Close"],
                row["VIX_Change_3d"],
                row["VIX_Change_5d"],
        ...
        # ── EWJ-006 最終訊號：base-6 + ^VIX DIRECTION gate + 可選 LEVEL CAP ──
