"""
VGK-004 訊號偵測器：波動率自適應回檔+WR 均值回歸

進場條件（全部滿足）：
1. 10 日回檔 >= 3% 且 <= 7%（排除極端崩盤）
...
from trading.experiments.vgk_004_vol_adaptive_pullback_wr.config import VGK004Config

logger = logging.getLogger(__name__)


class VGK004SignalDetector(BaseSignalDetector):
    def __init__(self, config: VGK004Config):
        self.config = config
    ...
    def detect_signals(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        logger.info("VGK-004: Detected %d vol-adaptive pullback+WR signals", signal_count)
        return df
