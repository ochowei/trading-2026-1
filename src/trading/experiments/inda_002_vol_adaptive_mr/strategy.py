...
INDA-001 framework + ATR ratio filter (1.15) + pullback cap (7%).
No trailing stop - fixed TP/SL symmetric exit.
"""
...
from trading.experiments.inda_002_vol_adaptive_mr.signal_detector import (
    INDAVolAdaptiveMRDetector,
)
...
class INDAVolAdaptiveMRStrategy(ExecutionModelStrategy):
    ...
    def create_detector(self) -> BaseSignalDetector:
        return INDAVolAdaptiveMRDetector(create_default_config())

    def _print_strategy_params(self, config: ExperimentConfig) -> None:
        if isinstance(config, INDAVolAdaptiveMRConfig):
            print(
