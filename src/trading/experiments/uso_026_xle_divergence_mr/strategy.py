"""USO-XLE Cross-Asset Divergence Regime-Gated MR 策略 (USO-026)"""
...
from trading.experiments.uso_026_xle_divergence_mr.signal_detector import (
    USO026SignalDetector,
)
...
class USO026Strategy(ExecutionModelStrategy):
    ...
    def create_detector(self) -> BaseSignalDetector:
        return USO026SignalDetector(create_default_config())

    def _print_strategy_params(self, config: ExperimentConfig) -> None:
        if isinstance(config, USO026Config):
            print(
            ...
            if config.use_divergence_filter:
                print(
                    f"  Cross-asset divergence: USO {config.divergence_lookback}d ret"
                    f" - {config.benchmark_ticker} {config.divergence_lookback}d ret"
