"""
SIVR-008 訊號偵測器：Bollinger Band Squeeze Breakout
SIVR-008 Signal Detector: BB Squeeze Breakout
...
from trading.experiments.sivr_008_bb_squeeze_breakout.config import SIVRBBSqueezeConfig

logger = logging.getLogger(__name__)


class SIVRBBSqueezeDetector(BaseSignalDetector):
    """SIVR BB Squeeze Breakout 訊號偵測器"""

    def __init__(self, config: SIVRBBSqueezeConfig):
        self.config = config
    ...
    def detect_signals(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        logger.info("SIVR: Detected %d BB squeeze breakout signals", signal_count)
        return df
