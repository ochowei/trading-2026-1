...
- A/B 訊號比 1.2:1（與 baseline 相同），A/B 累計差 15.41pp（58.6% 相對差，與 baseline 36% 同數量級）
- 關鍵改善：2DD floor <= -1.5% 過濾 2022-01-25 SL（2d -0.46% 淺 2d drift）並透過 cooldown chain shift 將原被抑制的 2022-01-28 訊號活化為 TP，Part A 訊號數保持 9 不變但 WR 從 77.8%→88.9%
- **跨資產貢獻**：repo 第 6 次「2DD floor」方向成功驗證（繼 USO-013、EEM-014、INDA-010、VGK-008、EWJ-005 後），首次半導體驅動 EM 單一國家 ETF 驗證

**前任最佳：** EWT-008 Att1（BB 下軌+回檔上限混合進場均值回歸：BB(20, 2.0) 下軌 + 10日高點回檔上限-8% + WR(10)≤-80 + ClosePos≥40% + ATR(5)/ATR(20)>1.10，TP+3.5%/SL-4.0%/20d，冷卻10天）
