...
**當前最佳：** ★ **EWT-010 Att2**（EWT-EEM 2D Cross-Asset Divergence Filter on Vol-Transition MR：EWT-009 Att3 完整框架 + **EWT-EEM 20 日報酬差 ≥ +3.0pp AND 60 日報酬差 ≥ +5.0pp 同時成立 → 過濾**，TP+3.5%/SL-4.0%/20d/cd 10）★ **2026-05-09 新全域最優（10 次實驗、33+ 次嘗試）**
...
| EWT-010 | `ewt_010_ewt_eem_2d_divergence_mr` | EWT-EEM 2D Cross-Asset Divergence Filter on Vol-Transition MR（雙時框 20d AND 60d AND 條件）★ 當前最佳 | 已完成 |
