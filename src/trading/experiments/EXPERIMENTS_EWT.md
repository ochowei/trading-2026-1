...
**當前最佳：** EWT-004 Att1（2日急跌過濾+非對稱出場：10日回檔4-10% + WR(10)≤-80 + ClosePos≥40% + ATR(5)/ATR(20)>1.15 + 2日報酬≤-1.5%，TP+5.0%/SL-4.5%/20d，無追蹤停損）已確認全域最優（5 次實驗、18 次嘗試，含均值回歸 pullback+WR、BB Squeeze 突破、RSI(2) 三大策略類型驗證）
- Part A Sharpe: 0.15 | Part B Sharpe: 0.48 | min(A,B): 0.15
- Part A: 16 訊號（3.2/年）, WR 50.0%, 累計 +9.71%
- Part B: 8 訊號（4.0/年）, WR 75.0%, 累計 +16.18%
- vs EWT-002 Att1: min(A,B) 0.13 → 0.15（+15%），A/B 累計差距 6.47pp（改善自 21.69pp）
...
**已掃描的參數空間：**
- 進場（pullback+WR）：10日回檔 4-10% + WR(10) ≤-80 + ClosePos ≥40% + ATR > 1.15/1.1 + 2日跌幅 ≤-1.5%
- 進場（RSI(2)）：RSI(2)<10 + ClosePos≥40% + ATR>1.15 ± 2日跌幅≤-1.5%
- 出場：TP +4.0~5.0% / SL -3.5~5.5% / 持倉 12~20天 / 無追蹤停損
...
| 參數 | EWT-001 | EWT-002 Att1 | EWT-003 | EWT-004 Att1 ★ | EWT-005 Att3 |
|------|---------|-------------|---------|----------------|-------------|
| 策略類型 | 均值回歸（追蹤停損）| 均值回歸（ATR 自適應）| BB Squeeze 突破 | 均值回歸（2日急跌+非對稱）| RSI(2)（ATR 自適應）|
| 進場框架 | pullback+WR | pullback+WR | BB Squeeze | pullback+WR | RSI(2) |
...
| SMA 趨勢 | — | — | SMA(50) | — | — |
| TP | +4.5% | +4.5% | +3.5% | +5.0% | +4.5% |
| SL | -5.0% | -5.0% | -3.5% | -4.5% | -4.5% |
| 持倉天數 | 18 | 18 | 20 | 20 | 12 |
| 追蹤停損 | 啟動+2.5%/距離2% | 無 | 無 | 無 | 無 |
| 冷卻期 | 8 | 8 | 10 | 8 | 5 |
| Part A Sharpe | 0.10 | 0.13 | 0.35 | **0.15** | -0.00 |
| Part B Sharpe | 0.57 | 0.64 | -0.37 | **0.48** | 0.31 |
| min(A,B) | 0.10 | 0.13 | -0.37 | **0.15** | -0.00 |
