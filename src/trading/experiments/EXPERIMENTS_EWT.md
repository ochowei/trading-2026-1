...
**當前最佳：** EWT-007 Att1（RS 動量回調：EWT vs EEM 20日 RS≥3% + 5日高點回撤2-5% + SMA(50)，TP+3.5%/SL-4.0%/20d，冷卻10天）— 7 次實驗、24 次嘗試，含均值回歸、BB Squeeze 突破、RSI(2)、出場優化、**RS 動量**五大策略類型驗證
- Part A Sharpe: 0.42 | Part B Sharpe: 0.93 | min(A,B): **0.42**
- Part A: 19 訊號（3.8/年）, WR 78.9%, 累計 +25.99%
- Part B: 7 訊號（3.5/年）, WR 85.7%, 累計 +18.07%
- A/B 年化訊號比 1.09:1（優秀），A/B 累計差距 30.5%
- vs EWT-006 Att2: min(A,B) 0.28 → **0.42**（+50%），策略類型轉換（均值回歸→RS 動量）
- 關鍵發現：EEM 是 EWT RS 策略最佳參考基準（台灣在 EM 中的相對表現由半導體出口驅動，具結構性）
...
**尚未嘗試的方向（預期邊際效益極低）：**
- RS 出場優化（TP+4.0%/SL-3.5% 或延長持倉 25d）：可微調但 Att1 已很優，風險 > 收益
- RS + ATR 過濾交叉：可能在 Part A 改善但樣本已小
...
- 高流動性 ETF，滑價假設 0.1%
- **RS 動量（EWT vs EEM）是 EWT 最佳策略類型**：台灣在 EM 中的超額表現由半導體出口結構性驅動，A/B 年化訊號比 1.09:1 近乎完美
- **SMH 作為 RS 參考完全無效**：EWT 含大量非半導體成分，稀釋 RS 訊號品質
- ATR 甜蜜點 > 1.15（均值回歸框架下，RS 動量不需 ATR 過濾）
- BB Squeeze 突破無效（單一國家 ETF 地緣政治風險，同 INDA 結論）
- RSI(2) 無效（非美國單一國家 ETF，地緣政治事件導致）
- SL -4.0% 是 RS 動量甜蜜點（TP +3.5% / SL -4.0% / 20d）
- TP +3.5% 跨策略通用甜蜜點（均值回歸和 RS 動量均驗證）
<!-- AI_CONTEXT_END -->
...
| 參數 | EWT-001 | EWT-002 Att1 | EWT-003 | EWT-004 Att1 | EWT-005 Att3 | EWT-006 Att2 | **EWT-007 Att1 ★** |
|------|---------|-------------|---------|----------------|-------------|----------------|----------------|
| 策略類型 | 均值回歸（追蹤停損）| 均值回歸（ATR 自適應）| BB Squeeze 突破 | 均值回歸（2日急跌+非對稱）| RSI(2)（ATR 自適應）| 均值回歸（出場優化）| **RS 動量回調** |
| 進場框架 | pullback+WR | pullback+WR | BB Squeeze | pullback+WR | RSI(2) | pullback+WR | **RS Momentum** |
...
| TP | +4.5% | +4.5% | +3.5% | +5.0% | +4.5% | +3.5% | **+3.5%** |
| SL | -5.0% | -5.0% | -3.5% | -4.5% | -4.5% | -4.5% | **-4.0%** |
| 持倉天數 | 18 | 18 | 20 | 20 | 12 | 15 | **20** |
| 追蹤停損 | 啟動+2.5%/距離2% | 無 | 無 | 無 | 無 | 無 | 無 |
| 冷卻期 | 8 | 8 | 10 | 8 | 5 | 8 | **10** |
| Part A Sharpe | 0.10 | 0.13 | 0.35 | 0.15 | -0.00 | 0.28 | **0.42** |
| Part B Sharpe | 0.57 | 0.64 | -0.37 | 0.48 | 0.31 | 0.50 | **0.93** |
| min(A,B) | 0.10 | 0.13 | -0.37 | 0.15 | -0.00 | 0.28 | **0.42** |
