...
from trading.experiments.fxi_003_bb_squeeze_breakout.config import FXI003Config
...
class FXI003SignalDetector(BaseSignalDetector):
    ...
    def detect_signals(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        if suppressed:
            ...
            logger.info(
                "FXI-003: %d duplicate signals suppressed by cooldown",
                len(suppressed),
            )

        signal_count = df["Signal"].sum()
        logger.info("FXI-003: Detected %d BB squeeze breakout signals", signal_count)
        return df
