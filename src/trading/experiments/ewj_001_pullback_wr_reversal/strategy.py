"""
EWJ 回檔 + Williams %R + 反轉K線均值回歸策略
(EWJ Pullback + Williams %R + Reversal Candle Mean Reversion Strategy)
...
from trading.experiments.ewj_001_pullback_wr_reversal.signal_detector import (
    EWJPullbackWRReversalSignalDetector,
)
...
class EWJPullbackWRReversalStrategy(ExecutionModelStrategy):
    ...
    def create_detector(self) -> BaseSignalDetector:
        return EWJPullbackWRReversalSignalDetector(create_default_config())
    ...
    def _print_strategy_params(self, config: ExperimentConfig) -> None:
        if isinstance(config, EWJPullbackWRReversalConfig):
            print(
