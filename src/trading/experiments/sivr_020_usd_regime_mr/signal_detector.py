"""SIVR–USD Cross-Asset Divergence Regime-Gated MR 訊號偵測器 (SIVR-020)
...
6. SIVR–USD cross-asset divergence regime gate（SIVR-020 核心新增）：
...
class SIVR020SignalDetector(BaseSignalDetector):
    ...
    def detect_signals(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        if suppressed:
            ...
            logger.info(
                "SIVR-020: %d duplicate signals suppressed by cooldown",
                len(suppressed),
            )

        signal_count = df["Signal"].sum()
        logger.info("SIVR-020: Detected %d USD-regime-gated MR signals", signal_count)
        return df
