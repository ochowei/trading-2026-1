"""
SPY 回檔 + Williams %R + 反轉K線均值回歸策略
(SPY Pullback + Williams %R + Reversal Candle Mean Reversion Strategy)
...
from trading.experiments.spy_001_pullback_wr.config import (
    SPYPullbackWRConfig,
    create_default_config,
)
from trading.experiments.spy_001_pullback_wr.signal_detector import (
    SPYPullbackWRSignalDetector,
)
...
class SPYPullbackWRStrategy(ExecutionModelStrategy):
    ...
    def create_detector(self) -> BaseSignalDetector:
        return SPYPullbackWRSignalDetector(create_default_config())
    ...
    def _print_strategy_params(self, config: ExperimentConfig) -> None:
        if isinstance(config, SPYPullbackWRConfig):
            print(
