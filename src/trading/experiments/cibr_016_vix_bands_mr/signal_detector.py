"""
CIBR-016 訊號偵測器：^VIX Implied-Vol Regime BANDS Filter MR
...
6. ^VIX 收盤值 <= vix_low_threshold OR > vix_high_threshold
   （CIBR-016 新增 BANDS gate：排除中等 VIX 帶；XBI-017 跨資產移植）
...
from trading.experiments.cibr_016_vix_bands_mr.config import CIBR016Config

logger = logging.getLogger(__name__)


class CIBR016VixBandsMRDetector(BaseSignalDetector):
    """CIBR-016 訊號偵測器"""
    ...
    def compute_indicators(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        # === ^VIX BANDS gate（CIBR-016 核心新增；XBI-017 跨資產移植）===
    ...
    def detect_signals(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        if suppressed:
            ...
            logger.info(
                "CIBR-016: %d duplicate signals suppressed by cooldown",
                len(suppressed),
            )

        signal_count = df["Signal"].sum()
        logger.info(
            "CIBR-016: Detected %d VIX-bands-filtered MR signals",
            signal_count,
        )
        return df
