"""
XLU-004: Bollinger Band Squeeze Breakout 策略
XLU BB Squeeze Breakout Strategy

以波動收縮後的突破取代均值回歸，嘗試捕捉 XLU 公用事業 ETF 利率敏感動量上漲。
"""
...
from trading.experiments.xlu_004_bb_squeeze_breakout.signal_detector import (
    XLU004BBSqueezeDetector,
)
...
class XLU004BBSqueezeStrategy(ExecutionModelStrategy):
    ...
    def create_detector(self) -> BaseSignalDetector:
        return XLU004BBSqueezeDetector(create_default_config())

    def _print_strategy_params(self, config: ExperimentConfig) -> None:
        if isinstance(config, XLU004BBSqueezeConfig):
