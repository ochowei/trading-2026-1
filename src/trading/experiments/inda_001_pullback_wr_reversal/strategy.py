"""
INDA 回檔 + Williams %R + 反轉K線均值回歸策略
(INDA Pullback + Williams %R + Reversal Candle Mean Reversion Strategy)
...
from trading.experiments.inda_001_pullback_wr_reversal.config import (
    INDAPullbackWRReversalConfig,
    create_default_config,
)
from trading.experiments.inda_001_pullback_wr_reversal.signal_detector import (
    INDAPullbackWRReversalSignalDetector,
)


class INDAPullbackWRReversalStrategy(PrimaryBundleStrategyMixin, ExecutionModelStrategy):
    """INDA 回檔 + Williams %R + 反轉K線均值回歸策略 (INDA-001)"""

    bundle_trial_family = "INDA:pullback-wr-reversal"
    bundle_trial_hypothesis = (
        "INDA pullback and Williams %R reversal can be reproduced from a verified primary bundle."
    )
    ...
    def create_detector(self) -> BaseSignalDetector:
        return INDAPullbackWRReversalSignalDetector(create_default_config())
    ...
    def _print_strategy_params(self, config: ExperimentConfig) -> None:
        if isinstance(config, INDAPullbackWRReversalConfig):
            print(
