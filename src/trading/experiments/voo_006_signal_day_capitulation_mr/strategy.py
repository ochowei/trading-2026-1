"""
VOO-006: Signal-Day Capitulation-Strength Filter Mean Reversion Strategy
...
- VOO-006：**1d floor + 3d cap** 雙維度（**同 SPY-009**，驗證**同指數 ETF
...
from trading.experiments.voo_006_signal_day_capitulation_mr.signal_detector import (
    VOO006SignalDetector,
)
...
class VOO006Strategy(ExecutionModelStrategy):
    ...
    def create_detector(self) -> BaseSignalDetector:
        return VOO006SignalDetector(create_default_config())

    def _print_strategy_params(self, config: ExperimentConfig) -> None:
        if isinstance(config, VOO006Config):
            ...
            print(
                ...
                "（VOO-006 第一維度：要求訊號日具備足夠 1d capitulation）"
            )
            if config.threeday_return_cap > -0.50:
                print(
                    f"  3 日急跌上限: >= {config.threeday_return_cap:.1%}"
                    "（VOO-006 第二維度：排除 regime-shift 級別 3 日延續下跌）"
                )
            else:
