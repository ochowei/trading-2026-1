...
- 沿用分支歧異守則：最高 REAL local module = tsla_017 → 本實驗 #018，
  使用自有 module 名稱 `tsla_018_usd_regime_breakout`。
...
TSLA-017 Att3（min(A,B) 0.96）仍為 TSLA 全域最優；TSLA-018 REJECT。**
...
class TSLA018Config(ExperimentConfig):
    """TSLA-018 TSLA-USD(UUP) Direction Regime-Gated BB Squeeze Breakout 參數
    ...
    # === USD (UUP) Direction Regime Gate（TSLA-018 核心新增）===
...
def create_default_config() -> TSLA018Config:
    return TSLA018Config(
        name="tsla_018_usd_regime_breakout",
