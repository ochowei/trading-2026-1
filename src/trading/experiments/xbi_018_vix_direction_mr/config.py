"""
XBI-018: ^VIX Implied-Vol DIRECTION Regime Gate Pullback MR
...
策略方向（Strategy Direction）：
    ...
    **Repo 首次 ^VIX DIRECTION 維度應用於 XBI**——XBI-017 為 lesson #24
    family **BANDS** 變體（VIX level 區間），XBI-018 試驗 **DIRECTION** 變體
...
XBI-018 新增（lesson #24 family DIRECTION 變體；repo 首次於 XBI）：
...
class XBI018Config(ExperimentConfig):
    """XBI-018 ^VIX Implied-Vol DIRECTION Regime Gate Pullback MR 參數"""
    ...
    # === ^VIX DIRECTION CEILING gate（XBI-018 核心新增）===
    # 訊號通過條件：(VIX_Close − VIX_Close.shift(lookback)) <= max_vix_change
