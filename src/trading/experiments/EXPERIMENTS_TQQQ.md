...
**TQQQ-018 Att3 為新全域最優**（19 次實驗、7 大策略類型：均值回歸、趨勢/動量/突破、Gap-Down 資本化、盤中/加速/多日確認、**regime + transition combo filter**、**^VIX direction filter**（TQQQ-019 失敗））

**最近失敗實驗：** TQQQ-019（^VIX Direction Filter on TQQQ-018，**repo 第 7 次 lesson #24 family 跨資產 + 首次 ^VIX DIRECTION 變體於 leveraged ETF — 全部 REJECT/TIE**）— Att1 +5 min 0.28（過嚴反向選擇）/ Att2 +15 TIE（非綁定）/ Att3 +10 TIE 但 Part A 退化。確認**lesson #24 family DIRECTION cap 在 extreme capitulation framework 結構性失效**（capitulation 與 VIX spike 共線）。
...
> **最新實驗 (Latest):** TQQQ-020 `tqqq_020_vix_peak_passing_mr`（^VIX 1d peak-passing/momentum reversal filter，repo 首次 ^VIX 1d 維度於任何資產，3 次 REJECT/TIE — 整合 lesson #24 family v9 邊界擴展：VIX DIRECTION 維度全層級 (5d cumulative + 1d momentum) 對 TQQQ extreme capitulation framework 結構性失效）
