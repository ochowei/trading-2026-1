...
**TQQQ-018 Att3 為新全域最優**（22 次實驗、10 大策略類型：均值回歸、趨勢/動量/突破、Gap-Down 資本化、盤中/加速/多日確認、**regime + transition combo filter**、**^VIX direction filter**（TQQQ-019 失敗）、**^VIX peak-passing filter**（TQQQ-020 失敗）、**^MOVE LEVEL filter**（TQQQ-021 失敗）、**^MOVE DIRECTION filter**（TQQQ-021 失敗）、**QQQ-SPY cross-asset divergence FLOOR**（TQQQ-022 失敗））

**最近失敗實驗：** TQQQ-022（QQQ-SPY Cross-Asset Divergence FLOOR Filter on TQQQ-018，**repo 首次「leveraged ETF underlying-vs-broader divergence」於任何資產 — 全部 REJECT/TIE**，2026-05-09）— Att1 20d -3% min **0.39**（reverse selection 嚴重，Part A WR 90→71.4%）/ Att2 20d -5% min **0.80** TIE（loose floor，Part B 完全不變）/ Att3 10d -3% min **0.66**（縮短 lookback 加劇 reverse selection）。**確認 lesson #20 v3 family v11 邊界擴展**：cross-asset divergence regime gate 對「leveraged ETF underlying-vs-broader-benchmark」結構性失效——target SLs 與 winners 在 divergence 維度分布重疊（broad-market panic 期 QQQ ≈ SPY 同步下跌），違反「SLs 在 divergence 維度方向性集中」必要條件。整合前次 ^MOVE / ^VIX implied vol 全失敗，**TQQQ Part B 0.80 binding ceiling 確認需「非 implied vol + 非 underlying-vs-broader cross-asset divergence」維度**——剩餘未驗證假設：yield curve slope velocity（TLT-017 成功維度）或完全替代 framework（vol-transition MR / BB Squeeze）。
...
> **最新實驗 (Latest):** TQQQ-022 `tqqq_022_qqq_spy_divergence_cap`（QQQ-SPY 跨資產 divergence FLOOR filter，repo 首次「leveraged ETF underlying-vs-broader divergence」於任何資產，3 次 REJECT/TIE — lesson #20 v3 family v11 邊界擴展：broad panic regime 下 QQQ-SPY divergence 結構性壓縮，target SLs 與 winners 在 divergence 維度分布重疊）
...
| **TQQQ-018** | **`tqqq_018_regime_vol_gate`** ★ | **波動率 regime 閘門 + 進場前回撤過濾**（TLT-007 跨資產移植 + 新發現 first-day-of-decline filter） | **Att1 BB<0.50 min 0.49 (+36%); Att2 BB<0.48 min 0.73 (+103%); Att3 ★ +DD(T-5)<=-1% min 0.80 (+122%)** | ✅ **當前最佳** |
