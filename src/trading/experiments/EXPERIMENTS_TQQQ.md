<!-- AI_CONTEXT_START - 此區塊供 AI Agent 快速讀取，人工更新
  ...
  retirement_note_2026_08_26: Legacy experiment research is terminally retired. Historical metrics remain frozen for inspection; run, compare-driven documentation updates, analysis, evaluation, qualification, promotion, and result publication are no longer authorized. New research uses released workflows and src/trading/research_definitions/.
  phase9_note_2026_08_06: TQQQ-019, TQQQ-020, TQQQ-021, TQQQ-022, TQQQ-023, TQQQ-026, and TQQQ-027 now consume declared ^VIX/^MOVE/QQQ/SPY/^TYX/^TNX/SQQQ historical auxiliary bundles through the provider-free execution boundary; migration runs remain migration-pending and do not advance latest or followup qualification.
  phase9_note_2026_08_06_legacy_wave: TQQQ-004, TQQQ-005, TQQQ-007, TQQQ-012, TQQQ-014, and TQQQ-015 now consume declared ^VIX/QQQ historical auxiliary bundles through the same provider-free boundary; migration runs remain migration-pending and do not advance latest or followup qualification.
  note_2026_05_16_tqqq030: TQQQ-030 added 2026-05-16 (TQQQ Dual-Horizon Momentum Continuation, **策略方向：動量 (Momentum) — repo 首次 TQQQ-native 雙時框動量堆疊持續性確認**, TQQQ-006 為動量崩潰做空型 / TQQQ-015 為 QQQ ROC 源，皆非 TQQQ 自身多時框動量堆疊). 純新訊號框架：ROC(短) > 閾值 + ROC(中) > 閾值（動量持續堆疊）+ Close > SMA(trend) 趨勢確認 + 可選 SMA200 bull + 可選 BB 波動率上限，隔日開盤買 TQQQ. Three iterations all REJECT vs baseline 0.80 / 現任最佳 TQQQ-025 Att2. **Att1** (ROC10>5% & ROC20>8% + SMA50, TP+10%/SL-8%/15d/cd10) Part A **59/62.7% WR/Sharpe 0.31** / Part B **24/41.7% WR/Sharpe -0.11 cum -27.12%** / min(A,B) **-0.11 REJECT** — **動量追高買 near-top，3x 放大 reversion，Part B 災難 -27.12%**（choppy 2024-25 momentum-chase 連續 14 SL）；**Att2** (ROC10>8% & ROC20>12% 更強動量 + SMA200 bull regime) Part A **38/57.9% WR/Sharpe 0.25** / Part B **14/57.1% WR/Sharpe 0.12 cum +9.53%** / min(A,B) **0.12 REJECT** — 更強閾值 + bull filter 使 Part B 從 -27% 回正 +9.53%（WR 41.7→57.1%）但 Part A 退化 0.31→0.25，plateau 遠低 baseline；**Att3 ★ (saved as default)** (Att2 + 波動率上限 BB width/Close < 0.50) Part A **38/57.9%/Sharpe 0.25** / Part B **14/57.1%/Sharpe 0.12 cum +9.53% — 與 Att2 完全相同** / min(A,B) **0.12 REJECT** — **波動率上限完全非綁定**：強動量（ROC10>8% & ROC20>12%）+ SMA200 bull regime 之 conjunction 結構性已排除 extreme vol regime（強動量持續發生於 lower-vol uptrend，非 extreme-vol blow-off），vol cap 無增量區分力. **核心失敗發現（lesson #5 邊界第四次擴展，repo 首次 TQQQ-native 雙時框動量，本 session 5 大方向統一結論收尾）**: (1) **動量追高於 3x leveraged ETF 災難性**——Att1 Part B -27.12%（momentum continuation entry = buy strength = buy near-term top，3x 放大使任何 momentum-chase 在 choppy regime 連續觸 -8% SL）；(2) **更強閾值 + bull regime 使 Part B 可存活但結構性 plateau**——min(A,B) -0.11→0.12 但遠低 capitulation baseline 0.80，順勢動量於 3x ETF 無 risk-adjusted alpha；(3) **vol cap 對強動量+bull conjunction 非綁定**——強多時框動量結構性發生於 lower-vol uptrend，與 capitulation framework（vol regime gate TQQQ-018 為核心過濾器）形成鮮明對比：capitulation 訊號發生於 high-vol panic（需 vol regime gate 過濾），momentum 訊號發生於 low-vol uptrend（vol cap 自然非綁定）；(4) **本 session 統一跨資產結論（lesson #5 邊界第四次擴展，整合 TQQQ-026~030 五大方向）**：leveraged tech ETF（TQQQ 5-6% vol）之 **配對交易（TQQQ-026，inverse-pair RSI 共線無正交資訊）+ 單日反轉 MR（TQQQ-027，過稀或過噪）+ 突破（TQQQ-028，假突破 whipsaw）+ 趨勢跟蹤（TQQQ-029，buy near-top plateau）+ 動量（TQQQ-030/006/015，追高 reversion 災難）五大「順勢 / 鏡射 / 短週期反轉」方向結構性全 REJECT**——**唯一可在 3x leveraged ETF 產生 risk-adjusted alpha 的維度為「逆勢深度超賣 extreme capitulation」（DD≤-15% + RSI(5)<25 + Volume + vol regime gate + 正交 IV 維度，TQQQ-001~018/025）**，因 3x 放大將任何「buy strength / breakout / new-high / momentum / inverse-pair」進場轉為 buy local extreme → mean-revert；(5) **TQQQ 第 30 次實驗、新增「動量 Momentum（TQQQ-native 雙時框）」第 18 大策略類型**（REJECT）。TQQQ-025 Att2 仍為 min(A,B) 全域最優（30 次實驗、71+ 次嘗試）。
  note_2026_05_16_tqqq029: TQQQ-029 added 2026-05-16 (TQQQ Donchian Channel Trend-Following, **策略方向：趨勢跟蹤 (Trend Following) — repo 首次 Turtle 式 N 日高點 Donchian 趨勢跟蹤於 TQQQ**, 補足 repo 中 TQQQ 較少使用之趨勢跟蹤方向). 純新訊號框架：Close > 前 N 日 High rolling max（Donchian 突破）+ Close > SMA(trend) 趨勢確認 + 可選 SMA200 bull regime，隔日開盤買 TQQQ. Three iterations 漸進改善但全部 REJECT vs baseline 0.80 / 現任最佳 TQQQ-025 Att2. **Att1** (Donchian20 + SMA50, TP+12%/SL-8%/20d/cd10) Part A **50/56.0% WR/Sharpe 0.28** / Part B **19/42.1% WR/Sharpe -0.01 cum -9.67%** / min(A,B) **-0.01 REJECT** — Donchian whipsaw 嚴重，Part B 19 訊號 10 SL；**Att2** (Donchian55 Turtle System 2 + SMA100 + SMA200 bull regime, 同出場) Part A **34/61.8% WR/Sharpe 0.42** / Part B **16/43.8% WR/Sharpe 0.01 cum -4.52%** / min(A,B) **0.01 REJECT** — 長週期 + bull filter 減少 whipsaw（Part A 0.28→0.42, WR 56→62%）但 Part B 仍 44% WR 負報酬；**Att3 ★ (saved as default)** (Donchian100 ultra-long major-trend + SMA150 + SMA200 bull, 拉寬出場 TP+20%/SL-10%/30d) Part A **27/66.7% WR/Sharpe 0.59** / Part B **12/50.0% WR/Sharpe 0.16 cum +15.50%** / Part C 2/100%/Sharpe 2.50/+30.27% / min(A,B) **0.16 REJECT** — 漸進最佳（min -0.01→0.01→0.16）但 plateau 遠低於 capitulation baseline 0.80. **核心失敗發現（lesson #5 邊界擴展，repo 首次 Donchian 趨勢跟蹤於 leveraged tech ETF）**: (1) **Donchian 趨勢跟蹤於 TQQQ 漸進改善但結構性 plateau 遠低於 capitulation baseline**——拉長 lookback（20→55→100）+ bull regime filter + 拉寬趨勢跟蹤出場使 min(A,B) -0.01→0.01→0.16，但 Part B 始終 ≤ 50% WR（choppy 2024-25 近 coin-flip）；(2) **3x leverage 與趨勢跟蹤根本張力**——緊停損（-8%）被 uptrend 正常 intra-trend 回檔 whipsaw 洗出；拉寬停損（-10%）+ 長持倉 + ultra-selective Donchian(100) 改善 WR 至 50-67% 但訊號品質仍 regime-dependent；趨勢延續進場（N 日新高 = local 極端）在 3x ETF 上 = buy near-term top，3x 放大使正常回檔變停損；(3) **新跨資產規則（lesson #5 邊界精煉，整合 TQQQ-006/015/027/028/029）**：leveraged tech ETF（TQQQ 5-6% vol）趨勢跟蹤 / breakout / momentum continuation / single-day reversal 維度結構性 underperform mean-reversion / extreme capitulation——**僅「逆勢深度超賣 capitulation」維度可在 3x ETF 上產生 risk-adjusted alpha**，順勢維度（trend/momentum/breakout）全失敗；(4) **TQQQ 第 29 次實驗、新增「趨勢跟蹤 Trend Following（Donchian）」第 17 大策略類型**（REJECT）。TQQQ-025 Att2 仍為 min(A,B) 全域最優（29 次實驗、68+ 次嘗試）。
  note_2026_05_16_tqqq028: TQQQ-028 added 2026-05-16 (TQQQ BB Squeeze Breakout, **策略方向：突破 (Breakout) / 趨勢啟動 — repo 首次於 TQQQ 自身計算 BB squeeze + 上軌突破**, 直接驗證 TQQQ-024 列出未驗證假設「BB Squeeze Breakout 於 TQQQ 本身（TQQQ-015 為 QQQ → trade TQQQ，方向不同）」). 純新訊號框架：前日 BB(20,2) width/Close < squeeze 閾值（波動收縮）+ 當日 Close > BB_Upper（上軌突破 ignition），隔日開盤買 TQQQ，TP+10%/SL-8%/15d/cd5/slippage 0.1%. Three iterations all REJECT vs baseline 0.80 / 現任最佳 TQQQ-025 Att2. **Att1** (squeeze<0.20, breakout, vol off) Part A **22/59.1% WR/Sharpe 0.29 cum +56.85%** / Part B **11/45.5% WR/Sharpe -0.03 cum -6.48%** / Part C 0 / min(A,B) **-0.03 REJECT** — breakout 在 3x TQQQ whipsaw 嚴重，Part B 45.5% WR 假突破失敗即觸 -8% SL；**Att2** (squeeze<0.15 tighter + Volume>1.5x) Part A **1/100%/+10%** / Part B **0** / Part C **1** / **REJECT 無訊號流** — squeeze 0.15 對 3x leveraged ETF 過嚴（TQQQ BB width/Close 罕 < 0.15，3x vol 結構性使 squeeze 極稀），+量能後完全崩潰；**Att3 ★ (saved as default)** (squeeze<0.20 + Close>SMA200 bull regime trend filter, vol off) Part A **19/63.2% WR/Sharpe 0.42 cum +76.77%**（trend filter 移除 2022 熊市假突破，WR 59→63% 改善）/ Part B **11/45.5% WR/Sharpe -0.03 cum -6.48% 完全等於 Att1**（**SMA200 trend filter 對 Part B 完全非綁定**：2024-2025 為 bull regime，全部 11 Part B breakout 訊號 Close > SMA200，假突破發生於 bull regime 內部而非熊市，trend filter 無法過濾）/ min(A,B) **-0.03 REJECT**. **核心失敗發現（lesson #5 / TQQQ-015 邊界擴展，repo 首次 TQQQ 自身 BB squeeze breakout）**: (1) **BB squeeze breakout 於 TQQQ 自身結構性失敗**——3x leverage 放大假突破虧損（failed breakout → 快速 -8% SL），Part B 45.5% WR 近隨機且累計負報酬，**與 TQQQ-015（QQQ BB squeeze → trade TQQQ）失敗模式平行**：訊號源為 QQQ 或 TQQQ 自身皆失敗，breakout 維度對 leveraged tech ETF 無預測力；(2) **squeeze 閾值結構性兩難**——0.20 有訊號流但含大量假突破（Part B 負報酬）；0.15 對 3x vol ETF 過嚴（TQQQ BB width/Close 結構性 ≥ ~0.18，squeeze 0.15 幾乎不觸發），無 sweet spot；(3) **長期趨勢 regime filter 不能救 Part B**——SMA200 bull filter 改善 Part A（過濾 2022 熊市假突破）但對 Part B 完全非綁定（2024-2025 bull regime 內部假突破無法以長期趨勢區分），**確認 leveraged ETF breakout 假突破為 regime-internal 噪音、非 regime-classifiable**；(4) **新跨資產規則（lesson #5 邊界精煉）**：BB squeeze breakout（順勢趨勢啟動）於 high-vol leveraged ETF（TQQQ 5-6% vol）結構性失效——3x decay + 假突破 whipsaw 使 breakout 維度無預測力，**leveraged tech ETF 僅 mean-reversion / extreme capitulation 維度有效（TQQQ-001~018），breakout / momentum continuation / single-day reversal 維度全失敗（TQQQ-006/015/027/028）**；(5) **TQQQ 第 28 次實驗、新增「突破 Breakout（TQQQ-native）」第 16 大策略類型**（REJECT）。TQQQ-025 Att2 仍為 min(A,B) 全域最優（28 次實驗、65+ 次嘗試）。
  note_2026_05_16_tqqq027: TQQQ-027 added 2026-05-16 (QQQ Single-Day Momentum-Reversal MR → Trade TQQQ, **策略方向：均值回歸 / 單日動量反轉 (Single-Day Momentum Reversal MR) — repo 首次測試「QQQ 單日急跌 + 日內反轉」更短週期高頻純 MR 維度**, 直接驗證 TQQQ-019~024 AI_CONTEXT 多次列出未驗證假設「underlying QQQ short-term momentum reversal（單日反轉模式，未試）」). 純新訊號框架（非 capitulation 變體）：QQQ ROC(1) <= 閾值 + QQQ ClosePos = (Close-Low)/(High-Low) >= 閾值（日內反轉，多方尾盤承接），隔日開盤買 TQQQ，TP+7%/SL-8%/10d/cd3/slippage 0.1%。Three iterations all REJECT vs baseline 0.80 / 現任最佳 TQQQ-025 Att2. **Att1** (roc1<=-2.5%, closepos>=0.50) Part A **1 訊號 0% WR cum -8.09% (1 SL)** / Part B **3 訊號 100% WR cum +22.50%** / Part C 0 — **過嚴過稀（4 訊號/7yr）Part A 訊號流崩潰**，QQQ -2.5% 單日 + 收於上半部 co-occurrence 結構性極稀；**Att2 ★ (saved as default，唯一有實質訊號流迭代)** (roc1<=-2.0%, closepos>=0.40) Part A **7/57.1% WR/Sharpe 0.07 cum +1.77%** (3 SL/7) / Part B **4/75.0%/Sharpe 0.49 cum +12.59%** / min(A,B) **0.07 REJECT (-91% vs baseline 0.80)** — 放寬後訊號流恢復但**單日反轉結構性噪音大**，Part A 57% WR 近隨機；**Att3** (roc1<=-2.0%, closepos>=0.45, + QQQ Volume>1.3x SMA20) Part A **1 訊號 0% WR cum -8.09% (1 SL)** / Part B 3/100% cum +22.50% / min REJECT — **QQQ 量能濾器過度收斂**（與 TQQQ-016 Att3 量能濾後 Part B 完全不變平行），Part A 1 訊號全敗。**核心失敗發現（lesson #20a 邊界擴展，repo 首次 QQQ 單日反轉維度於任何資產）**: (1) **QQQ single-day momentum-reversal 結構性「過稀或過噪二擇一」**——緊閾值（ROC1<=-2.5% 或 +量能）→ 4 訊號/7yr Part A 訊號流崩潰至 1 訊號全敗；鬆閾值（ROC1<=-2.0%, closepos>=0.40）→ 訊號流恢復但 Part A WR 跌至 57% 近隨機，**無 threshold 組合可同時兼顧訊號流與 WR**；(2) **驗證並推廣 TQQQ-016 結論至 underlying QQQ 維度**——TQQQ-016（TQQQ 自身 gap-down + 日內反轉，3 次失敗）結論「QQQ 盤外流動性有限使日內反彈常為技術性反應而非真正底部」**推廣到 QQQ underlying 單日 ROC1 + ClosePos 維度同樣成立**：缺乏 -15% extreme drawdown + RSI(5)<25 深度超賣脈絡，單日 -2% 收上半部反轉約 50/50 reversion（Part A 57% WR）；(3) **新跨資產規則（lesson #20a v2 邊界精煉）**：single-day reversal pattern（不論 TQQQ gap-down 或 QQQ ROC1+ClosePos）對 leveraged tech ETF 結構性無預測力，**須深度 capitulation 脈絡（DD≤-15% + RSI(5)<25 + Volume）方能將單日反轉訊號從技術性噪音提升為高品質底部**——這正是 TQQQ-001~018 capitulation framework 之所以有效、而純單日反轉無效的結構性原因；(4) **TQQQ 第 27 次實驗、新增「單日動量反轉 MR」第 15 大策略類型**（REJECT）。TQQQ-025 Att2 仍為 min(A,B) 全域最優（27 次實驗、62+ 次嘗試）。
  note_2026_05_16_tqqq026: TQQQ-026 added 2026-05-16 (TQQQ/SQQQ Inverse-Pair Capitulation Confirmation, **策略方向：配對交易 (Pair Trading) — repo 中 TQQQ 未曾嘗試之方向；repo 首次「leveraged ETF inverse-pair（同一 underlying 反向槓桿）confirmation gate」於任何資產**, 直接驗證 TQQQ-021/024 AI_CONTEXT 明確列出未驗證假設「TQQQ vs SQQQ inverse pair（同一 underlying 槓桿配對，未試）」). Three iterations vs TQQQ-025 Att2 current best (雙 Part std=0 + 三項 acceptance criteria 全達標). **Att1** (use_sqqq_rsi_filter=True, min_sqqq_rsi=70, SQQQ RSI(5) FLOOR) Part A **10/90%/Sharpe 1.21 cum +68.97% 完全等於 baseline** / Part B **6/83.3%/Sharpe 0.80 cum +28.91% 完全等於 baseline** / min(A,B) **0.80 TIE baseline** — **SQQQ RSI FLOOR 70 完全非綁定**：全部 16 baseline capitulation 訊號當日 SQQQ RSI(5) >= 70（TQQQ -15% drawdown extreme capitulation 必然伴隨 SQQQ inverse-pair RSI 飆升至超買極端，**SQQQ RSI 與 capitulation 結構性共線**，與 ^VIX direction TQQQ-019 Att2 / ^MOVE LEVEL TQQQ-021 Att1 非綁定失敗模式完全平行）；**Att2 ★ (saved as default)** (min_sqqq_rsi=80 tighter) Part A **5/100% WR std=0 zero-var cum +40.26%**（過濾 2021-09-28 SL + 同時誤殺 4 winners，10→5 重度衰減）/ Part B **5/100% WR std=0 zero-var cum +40.26%**（過濾 2025-03-06 SL + 誤殺 1 winner，6→5）/ min(A,B)† **structurally NO LOSS（雙 Part 全勝零方差，依 EWT-010/IBIT-009 慣例）但未達 acceptance criteria**：A/B 年化 cum gap **~62% > 30% ❌**（Part A +40.26%/5yr ≈ 7.00%/yr vs Part B +40.26%/2yr ≈ 18.43%/yr，Part A 重度 winner attrition 使年化報酬塌縮）+ A/B 訊號比 1.0/yr vs 2.5/yr = **60% > 50% ❌** — **PARTIAL，僅達現任最佳 TQQQ-025 Att2 之 structural 品質但 gap 雙準則皆 fail，未超越**；**Att3** (min_sqqq_rsi=75 sweet spot 嘗試) Part A **10/90%/Sharpe 1.21 cum +68.97% 完全等於 baseline**（2021-09-28 SL 殘存，其 SQQQ RSI ∈ [75,80) 逃逸 FLOOR）/ Part B **5/100% std=0 cum +40.26%**（2025-03-06 SL 過濾，其 SQQQ RSI < 75）/ min(A,B)† **1.21 PARTIAL**（+51% vs baseline 0.80，與 TQQQ-025 Att1 pattern 相同但 Part A 仍 1 SL，且 A/B cum gap 大）. **核心失敗發現（repo 首次 inverse-pair confirmation gate、lesson #24 family / lesson #20 v3 family 邊界擴展）**: (1) **SQQQ inverse-pair RSI 與 TQQQ extreme capitulation 結構性共線**——SQQQ = -3x QQQ，TQQQ -15% drawdown 崩跌（QQQ -5% 量級）必使 SQQQ 暴漲 +15% 量級 → SQQQ RSI(5) 必然飆至 >70 超買極端；RSI≥70 FLOOR 對全部 16 baseline 訊號完全非綁定，**與 ^VIX/^MOVE DIRECTION 維度共線失敗模式完全平行**（capitulation = inverse-pair panic spike = 同一現象）；(2) **僅極端緊 FLOOR (≥80) 可過濾兩殘餘 SL 但觸發嚴重 reverse selection**——Part A 10→5（移除 4 winners 換 1 SL），與 TQQQ-019 Att1 (+5 cap reverse-select) / TQQQ-021 Att2 (3d DIR reverse-select) 同型；(3) **無單一 SQQQ RSI threshold 可同時過濾兩殘餘 SL 並維持 A/B 平衡準則**——70 非綁定 TIE、75 僅過濾 Part B SL（Part A SL SQQQ RSI ∈ [75,80) 逃逸）、80 雙 SL 過濾但 Part A 重度 attrition；(4) **新跨資產規則（repo 首次 inverse-pair confirmation gate 失敗類別）**：「同一 underlying 反向槓桿配對」(TQQQ/SQQQ) 的 inverse RSI 對 extreme capitulation framework 結構性共線——不同於 TQQQ-022 (QQQ-SPY，不同 underlying broad-vs-tech price divergence) 或 TQQQ-025 (^VXN/^VIX，cross-index IV divergence)；inverse-pair pure price/RSI 維度因 -3x/+3x 機械鏡像關係**資訊量等同 TQQQ 自身 RSI**（無正交新資訊）；(5) **配對交易方向結論**：TQQQ/SQQQ 因 -3x↔+3x 機械鏡射，SQQQ 衍生訊號對 TQQQ capitulation **無正交增量資訊**，配對交易（同一 underlying 反向槓桿）不適用於 leveraged ETF capitulation framework；(6) **TQQQ 第 26 次實驗、新增「配對交易 (Pair Trading)」第 14 大策略類型**（inverse-pair confirmation gate 失敗）。TQQQ-025 Att2 仍為 min(A,B) 全域最優（26 次實驗、59+ 次嘗試）。
...
**最新實驗：** TQQQ-030（TQQQ Dual-Horizon Momentum Continuation — **策略方向：動量 (Momentum)，repo 首次 TQQQ-native 雙時框動量堆疊，3 次全部 REJECT**）—— 詳見 AI_CONTEXT note_2026_05_16_tqqq030
- Att1 (ROC10>5%&ROC20>8%+SMA50) Part A 59/62.7%/0.31 + Part B 24/41.7%/-0.11/**-27.12%** → min **-0.11 REJECT**（動量追高 3x 放大 reversion 災難）
- Att2 (ROC10>8%&ROC20>12%+SMA200 bull) Part A 38/57.9%/0.25 + Part B 14/57.1%/0.12/+9.53% → min **0.12 REJECT**（Part B 回正但 plateau）
- Att3 ★ (+BB vol cap<0.50) **與 Att2 完全相同** min **0.12 REJECT**（vol cap 非綁定，強動量結構性發生於 low-vol uptrend）
- **本 session 統一結論**：TQQQ-026~030 五大方向（配對/單日反轉/突破/趨勢/動量）全 REJECT——3x leveraged ETF 唯一 alpha 維度為「逆勢深度超賣 extreme capitulation」，順勢/鏡射/短週期反轉全失敗

**前次實驗：** TQQQ-029（TQQQ Donchian Channel Trend-Following — **策略方向：趨勢跟蹤 (Trend Following)，repo 首次 Turtle 式 Donchian 趨勢跟蹤於 TQQQ，3 次漸進改善但全部 REJECT**）—— 詳見 AI_CONTEXT note_2026_05_16_tqqq029
- Att1 (Donchian20+SMA50) Part A 50/56%/0.28 + Part B 19/42.1%/-0.01/-9.67% → min **-0.01 REJECT**（whipsaw）
- Att2 (Donchian55+SMA100+SMA200 bull) Part A 34/61.8%/0.42 + Part B 16/43.8%/0.01/-4.52% → min **0.01 REJECT**
- Att3 ★ (Donchian100+SMA150+SMA200, TP+20/SL-10/30d) Part A 27/66.7%/0.59 + Part B 12/50%/0.16/+15.50% → min **0.16 REJECT**（漸進最佳但 plateau 遠低 baseline 0.80）
- 核心發現：3x leverage 與趨勢跟蹤根本張力（緊停損被 intra-trend 回檔洗、進場 N 日新高=buy near-term top）；整合 TQQQ-006/015/027/028/029——leveraged tech ETF 僅逆勢深度超賣 capitulation 維度可產生 alpha，順勢維度全失敗

**前次實驗：** TQQQ-028（TQQQ BB Squeeze Breakout — **策略方向：突破 (Breakout)，repo 首次 TQQQ 自身 BB squeeze 突破，3 次全部 REJECT**）—— 詳見 AI_CONTEXT note_2026_05_16_tqqq028
- Att1 (squeeze<0.20, breakout) Part A 22/59.1%/0.29 + Part B 11/45.5%/-0.03/-6.48% → min **-0.03 REJECT**（3x whipsaw）
- Att2 (squeeze<0.15 + Volume) Part A 1/Part B 0 → 無訊號流 REJECT（0.15 對 3x TQQQ 過嚴）
- Att3 ★ (squeeze<0.20 + SMA200 bull filter) Part A 19/63.2%/0.42 改善 + Part B 11/45.5%/-0.03 **完全不變** → min **-0.03 REJECT**（trend filter 對 Part B 非綁定）
- 核心發現：BB squeeze breakout 於 TQQQ 自身失敗（與 TQQQ-015 QQQ 源平行）——leveraged tech ETF 僅 MR/capitulation 維度有效，breakout/momentum/單日反轉維度全失敗（TQQQ-006/015/027/028）

**前次實驗：** TQQQ-027（QQQ Single-Day Momentum-Reversal MR → Trade TQQQ — **策略方向：均值回歸 / 單日動量反轉，repo 首次 QQQ 單日反轉維度，3 次全部 REJECT**）—— 詳見 AI_CONTEXT note_2026_05_16_tqqq027
- Att1 (QQQ ROC1≤-2.5%, ClosePos≥0.50) Part A 1/0%/-8.09% + Part B 3/100% — 過嚴過稀（4 訊號/7yr）Part A 訊號流崩潰 REJECT
- Att2 ★ (ROC1≤-2.0%, ClosePos≥0.40) min(A,B) **0.07 REJECT** — 唯一有實質訊號流迭代，但單日反轉噪音大 Part A 57% WR 近隨機
- Att3 (+ QQQ Volume>1.3x) Part A 1/0%/-8.09% + Part B 3/100% — 量能濾過度收斂 REJECT
- 核心發現：缺乏 -15% extreme drawdown + RSI(5)<25 深度脈絡，單日反轉約 50/50 reversion；推廣 TQQQ-016 結論至 QQQ underlying 維度——須深度 capitulation 脈絡方能將單日反轉從技術噪音提升為高品質底部

**前次實驗：** TQQQ-026（TQQQ/SQQQ Inverse-Pair Capitulation Confirmation — **策略方向：配對交易 (Pair Trading)，repo 首次 inverse-pair confirmation gate，3 次 TIE/PARTIAL 全部未超越現任最佳**）—— 詳見 AI_CONTEXT note_2026_05_16_tqqq026
- Att1 (SQQQ RSI(5) FLOOR ≥ 70) min(A,B) **0.80 TIE baseline** — SQQQ RSI 完全非綁定（與 capitulation 結構性共線，平行 ^VIX/^MOVE 非綁定失敗）
- Att2 ★ (RSI ≥ 80) min(A,B)† **structurally NO LOSS 但 PARTIAL** — 雙 Part std=0 全勝但 Part A 重度 attrition 使 A/B cum gap ~62% > 30% ❌ + signal gap 60% > 50% ❌
- Att3 (RSI ≥ 75) min(A,B)† **1.21 PARTIAL** — Part B 純化 std=0，Part A SL 殘存（SQQQ RSI ∈ [75,80) 逃逸）
- 核心發現：TQQQ/SQQQ 因 -3x↔+3x 機械鏡射，SQQQ 衍生 RSI 對 TQQQ capitulation **無正交增量資訊**，配對交易（同一 underlying 反向槓桿）不適用 capitulation framework

**前次實驗：** TQQQ-025（VXN-VIX Cross-Index IV Divergence + VVIX Direction Filter — **Att2 SUCCESS 雙 Part std=0 全勝，repo 首次正交雙 IV 維度 AND 突破 TQQQ Part B 0.80 ceiling**）—— 詳見 AI_CONTEXT note_2026_05_10_tqqq025
...
> **最新實驗 (Latest):** TQQQ-030 `tqqq_030_dual_momentum`（動量 Momentum — TQQQ-native 雙時框 ROC 動量堆疊，repo 首次 TQQQ 自身雙時框動量，3 次全部 REJECT min -0.11→0.12→0.12 — 動量追高 3x 放大 reversion 災難 (Att1 Part B -27%)；**本 session 統一結論：TQQQ-026~030 五大方向 (配對/單日反轉/突破/趨勢/動量) 全 REJECT，3x leveraged ETF 唯一 alpha = 逆勢深度 extreme capitulation**）
> **當前最佳 (Best):** TQQQ-025 `tqqq_025_vxn_vix_vvix_filter` ★ Att2（含成交模型 + TQQQ-018 框架 + ^VXN/^VIX FLOOR 1.10 + ^VVIX 5d FLOOR -5，雙 Part std=0 全勝 structurally NO LOSS，三項 acceptance criteria 全達標）/ TQQQ-018 Att3（前任最佳 min(A,B) 0.80）
...
| TQQQ-026 | `tqqq_026_sqqq_pair_divergence` | **配對交易 (Pair Trading)** — TQQQ/SQQQ Inverse-Pair Capitulation Confirmation on TQQQ-018（**repo 首次 leveraged inverse-pair confirmation gate 於任何資產**，直接測試 TQQQ-021/024 未驗證假設「TQQQ vs SQQQ inverse pair」） | Att1 SQQQ RSI≥70 min **0.80 TIE**（完全非綁定，SQQQ RSI 與 capitulation 結構性共線）; Att2 ★ RSI≥80 min(A,B)† **structurally NO LOSS 但 PARTIAL**（雙 Part std=0 但 A/B cum gap ~62%>30% + signal gap 60%>50%，reverse selection）; Att3 RSI≥75 min(A,B)† **1.21 PARTIAL**（Part B 純化，Part A SL 殘存） | ❌ 失敗（未超越 TQQQ-025 Att2） |
| TQQQ-027 | `tqqq_027_qqq_single_day_reversal` | **均值回歸 / 單日動量反轉 (Single-Day Momentum Reversal MR)** — QQQ ROC(1) + ClosePos → Trade TQQQ（**repo 首次 QQQ 單日反轉維度於任何資產**，直接測試 TQQQ-019~024 未驗證假設「underlying QQQ short-term momentum reversal」） | Att1 ROC1≤-2.5%/ClosePos≥0.50 過稀 Part A 1/0%（REJECT）; Att2 ★ ROC1≤-2.0%/ClosePos≥0.40 min **0.07 REJECT**（單日反轉噪音大 Part A 57% WR 近隨機）; Att3 +QQQ Volume 過度收斂 Part A 1/0%（REJECT） | ❌ 失敗（單日反轉須深度 capitulation 脈絡） |
| TQQQ-028 | `tqqq_028_bb_squeeze_breakout` | **突破 (Breakout) / 趨勢啟動** — TQQQ 自身 BB(20,2) squeeze + 上軌突破（**repo 首次 TQQQ-native BB squeeze breakout**，直接測試 TQQQ-024 未驗證假設「BB Squeeze Breakout 於 TQQQ 本身」） | Att1 squeeze<0.20 min **-0.03 REJECT**（Part B 45.5% WR 負報酬，3x whipsaw）; Att2 squeeze<0.15+Volume 無訊號流 REJECT（0.15 對 3x 過嚴）; Att3 ★ +SMA200 bull filter Part A 0.42 改善但 Part B 完全不變 min **-0.03 REJECT** | ❌ 失敗（breakout 於 leveraged ETF 結構性無預測力） |
| TQQQ-029 | `tqqq_029_donchian_trend` | **趨勢跟蹤 (Trend Following)** — TQQQ Donchian N 日高點突破 + 趨勢確認（**repo 首次 Turtle 式 Donchian 趨勢跟蹤於 TQQQ**，補足較少使用之趨勢跟蹤方向） | Att1 Donchian20+SMA50 min **-0.01 REJECT**（whipsaw）; Att2 Donchian55+SMA100+SMA200 min **0.01 REJECT**; Att3 ★ Donchian100+SMA150+寬出場 min **0.16 REJECT**（漸進最佳但 plateau 遠低 baseline 0.80） | ❌ 失敗（順勢維度於 leveraged ETF 全失敗） |
| TQQQ-030 | `tqqq_030_dual_momentum` | **動量 (Momentum)** — TQQQ 自身雙時框 ROC 動量堆疊持續性（**repo 首次 TQQQ-native 雙時框動量**，TQQQ-006 為崩潰做空型 / TQQQ-015 為 QQQ 源） | Att1 ROC10>5%&ROC20>8% Part B **-27.12%** min **-0.11 REJECT**（動量追高 3x 災難）; Att2 ROC10>8%&ROC20>12%+SMA200 bull min **0.12 REJECT**（Part B 回正但 plateau）; Att3 ★ +vol cap **與 Att2 同** min **0.12 REJECT**（vol cap 非綁定） | ❌ 失敗（順勢動量於 leveraged ETF 無 alpha） |
...
│
│   ── 配對交易系列 (Pair Trading Series，repo 首次 inverse-pair confirmation gate，3 次 TIE/PARTIAL 全部失敗) ──
├── TQQQ-026 tqqq_026_sqqq_pair_divergence (SQQQ RSI FLOOR 70/75/80，Att2 ★ 雙 Part std=0 但 PARTIAL，**SQQQ inverse-pair RSI 與 capitulation 結構性共線、-3x↔+3x 機械鏡射無正交增量資訊**——配對交易不適用 leveraged ETF capitulation framework)
│
│   ── 單日動量反轉 MR 系列 (Single-Day Momentum Reversal MR Series，repo 首次 QQQ 單日反轉維度，3 次全部 REJECT) ──
├── TQQQ-027 tqqq_027_qqq_single_day_reversal (QQQ ROC1+ClosePos→Trade TQQQ，Att2 ★ min 0.07，**單日反轉過稀或過噪二擇一、缺乏深度 capitulation 脈絡約 50/50 reversion**——推廣 TQQQ-016 結論至 QQQ underlying 維度)
│
│   ── 突破 Breakout 系列（TQQQ-native，repo 首次 TQQQ 自身 BB squeeze 突破，3 次全部 REJECT） ──
├── TQQQ-028 tqqq_028_bb_squeeze_breakout (TQQQ BB squeeze + 上軌突破，Att3 ★ min -0.03，**3x leverage 放大假突破 whipsaw、squeeze 閾值兩難、trend filter 對 Part B 非綁定**——breakout 於 leveraged tech ETF 結構性無預測力，與 TQQQ-015 平行)
│
│   ── 趨勢跟蹤 Trend Following 系列（Donchian，repo 首次 Turtle 式趨勢跟蹤於 TQQQ，3 次漸進改善但全部 REJECT） ──
├── TQQQ-029 tqqq_029_donchian_trend (Donchian N 日高點 + 趨勢確認，Att3 ★ min 0.16，**3x leverage 與趨勢跟蹤根本張力、進場 N 日新高=buy near-term top、plateau 遠低 baseline**——整合 TQQQ-006/015/027/028/029：leveraged ETF 僅逆勢深度 capitulation 維度可產生 alpha)
│
│   ── 動量 Momentum 系列（TQQQ-native 雙時框，repo 首次 TQQQ 自身雙時框動量，3 次全部 REJECT，本 session 5 方向統一結論收尾） ──
└── TQQQ-030 tqqq_030_dual_momentum (ROC 短+中 動量堆疊+趨勢，Att3 ★ min 0.12，**動量追高 3x 放大 reversion 災難 (Att1 Part B -27%)、vol cap 對強動量+bull 非綁定**——統一結論：TQQQ-026~030 五大方向全 REJECT，3x leveraged ETF 唯一 alpha = 逆勢深度 extreme capitulation)
```
...
## TQQQ-026: TQQQ/SQQQ Inverse-Pair Capitulation Confirmation（配對交易，3 次未超越）

### 動機
策略方向 **配對交易 (Pair Trading)** — repo 中 TQQQ 26 次實驗未曾嘗試之方向，
直接驗證 TQQQ-021/024 AI_CONTEXT 明確列出之未驗證假設「TQQQ vs SQQQ inverse
pair（同一 underlying 槓桿配對，未試）」。SQQQ 為 -3x QQQ inverse ETF，與
TQQQ（+3x QQQ）為同一 underlying（NDX）的反向槓桿配對。假設：真正的 TQQQ
capitulation 底部應同時對應 SQQQ 的 RSI 極端超買（inverse-pair panic exhaustion），
以 SQQQ RSI(5) FLOOR 作為「inverse-pair 恐慌確認」過濾低品質緩跌訊號。

### 策略設計
TQQQ-018 Att3 完整框架（DD≤-15% + RSI(5)<25 + Vol>1.5x + BB-width<0.48 +
DD(T-5)≤-1% + cd3）+ **SQQQ RSI(5) >= min_sqqq_rsi** FLOOR。出場 TP+7%/SL-8%/
10d/slippage 0.1%（成交模型同 TQQQ-018）。

### 迭代歷程（最多三次）

| Att | min_sqqq_rsi | Part A | Part B | min(A,B) | 判定 |
|-----|--------------|--------|--------|----------|------|
| 1 | 70 | 10/90%/Sharpe 1.21/+68.97%（=baseline） | 6/83.3%/0.80/+28.91%（=baseline） | **0.80 TIE** | 完全非綁定 |
| 2 ★ | 80 | 5/100%/std=0/+40.26% | 5/100%/std=0/+40.26% | **NO LOSS 但 PARTIAL** | 雙 SL 過濾但 gap 雙準則 fail |
| 3 | 75 | 10/90%/1.21/+68.97%（=baseline） | 5/100%/std=0/+40.26% | **1.21 PARTIAL** | Part B 純化、Part A SL 殘存 |

- **Att2** A/B 年化 cum gap ~62% > 30% ❌（Part A +40.26%/5yr ≈ 7.00%/yr vs
  Part B +40.26%/2yr ≈ 18.43%/yr）+ A/B 訊號比 1.0/yr vs 2.5/yr = 60% > 50% ❌

### 核心失敗發現（repo 首次 inverse-pair confirmation gate）

1. **SQQQ inverse-pair RSI 與 TQQQ extreme capitulation 結構性共線**——SQQQ =
   -3x QQQ，TQQQ -15% drawdown 崩跌必使 SQQQ 暴漲 → SQQQ RSI(5) 必飆 >70；
   RSI≥70 FLOOR 對全部 16 baseline 訊號完全非綁定，**與 ^VIX direction
   (TQQQ-019 Att2) / ^MOVE LEVEL (TQQQ-021 Att1) 非綁定失敗模式完全平行**
2. **僅極端緊 FLOOR (≥80) 可過濾兩殘餘 SL 但觸發嚴重 reverse selection**
   （Part A 10→5，移除 4 winners 換 1 SL），與 TQQQ-019 Att1 / TQQQ-021 Att2
   reverse-select 同型
3. **配對交易方向結論**：TQQQ/SQQQ 因 -3x↔+3x 機械鏡射，SQQQ 衍生 RSI 對
   TQQQ capitulation **無正交增量資訊**（資訊量等同 TQQQ 自身 RSI）——不同於
   TQQQ-022 QQQ-SPY（不同 underlying broad-vs-tech）或 TQQQ-025 ^VXN/^VIX
   （cross-index IV）；配對交易（同一 underlying 反向槓桿）不適用 leveraged
   ETF capitulation framework
4. **TQQQ 第 26 次實驗、新增「配對交易 (Pair Trading)」第 14 大策略類型**。
   TQQQ-025 Att2 仍為 min(A,B) 全域最優

---

## TQQQ-027: QQQ Single-Day Momentum-Reversal MR → Trade TQQQ（單日動量反轉，3 次 REJECT）

### 動機
策略方向 **均值回歸 / 單日動量反轉 (Single-Day Momentum Reversal MR)** — repo
中 TQQQ 27 次實驗皆為 -15% extreme capitulation 變體或多日動量/突破，**從未
測試「QQQ 單日急跌 + 日內反轉」這個更短週期高頻純 MR 維度**。直接驗證
TQQQ-019~024 AI_CONTEXT 多次列出未驗證假設「underlying QQQ short-term
momentum reversal（單日反轉模式，未試）」。

### 策略設計（純新訊號框架，非 capitulation 變體）
QQQ ROC(1) <= 閾值（單日急跌）+ QQQ ClosePos = (Close-Low)/(High-Low) >= 閾值
（收於日內區間上半部 = 日內反轉），隔日開盤買 TQQQ，TP+7%/SL-8%/10d/cd3/
slippage 0.1%（成交模型）。

### 迭代歷程（最多三次）

| Att | QQQ ROC1 | ClosePos | QQQ Vol | Part A | Part B | min(A,B) | 判定 |
|-----|----------|----------|---------|--------|--------|----------|------|
| 1 | ≤ -2.5% | ≥ 0.50 | off | 1/0%/-8.09% | 3/100%/+22.50% | REJECT | 過稀崩潰 |
| 2 ★ | ≤ -2.0% | ≥ 0.40 | off | 7/57.1%/0.07 | 4/75%/0.49 | **0.07 REJECT** | 噪音大 |
| 3 | ≤ -2.0% | ≥ 0.45 | >1.3x | 1/0%/-8.09% | 3/100%/+22.50% | REJECT | 量能過收斂 |

### 核心失敗發現（lesson #20a 邊界擴展，repo 首次 QQQ 單日反轉維度）

1. **QQQ single-day momentum-reversal 結構性「過稀或過噪二擇一」**——緊閾值
   → 4 訊號/7yr Part A 訊號流崩潰至 1 訊號全敗；鬆閾值 → 訊號流恢復但
   Part A WR 跌至 57% 近隨機，**無 threshold 組合可兼顧訊號流與 WR**
2. **驗證並推廣 TQQQ-016 結論至 underlying QQQ 維度**——缺乏 -15% extreme
   drawdown + RSI(5)<25 深度超賣脈絡，單日 -2% 收上半部反轉約 50/50
   reversion（這正是 TQQQ-001~018 capitulation framework 有效、純單日
   反轉無效的結構性原因）
3. **新跨資產規則（lesson #20a v2）**：single-day reversal pattern（不論
   TQQQ gap-down 或 QQQ ROC1+ClosePos）對 leveraged tech ETF 結構性無
   預測力，須深度 capitulation 脈絡方能將其從技術噪音提升為高品質底部
4. **TQQQ 第 27 次實驗、新增「單日動量反轉 MR」第 15 大策略類型**（REJECT）。
   TQQQ-025 Att2 仍為 min(A,B) 全域最優

---

## TQQQ-028: TQQQ BB Squeeze Breakout（TQQQ-native，突破，3 次 REJECT）

### 動機
策略方向 **突破 (Breakout) / 趨勢啟動** — TQQQ-015 為 QQQ BB squeeze →
trade TQQQ（訊號源 1x QQQ，3 次失敗），**從未於 TQQQ 自身計算 BB squeeze
+ 上軌突破**。直接驗證 TQQQ-024 列出未驗證假設「BB Squeeze Breakout 於
TQQQ 本身（方向不同）」。

### 策略設計（純新訊號框架）
前日 BB(20,2) width/Close < squeeze 閾值（波動收縮）+ 當日 Close > BB_Upper
（上軌突破 ignition），隔日開盤買 TQQQ，TP+10%/SL-8%/15d/cd5/slippage 0.1%。

### 迭代歷程（最多三次）

| Att | squeeze | 加項 | Part A | Part B | min(A,B) | 判定 |
|-----|---------|------|--------|--------|----------|------|
| 1 | <0.20 | — | 22/59.1%/0.29/+56.85% | 11/45.5%/-0.03/-6.48% | **-0.03** | 3x whipsaw |
| 2 | <0.15 | Volume>1.5x | 1/100%/+10% | 0 | REJECT | 無訊號流 |
| 3 ★ | <0.20 | SMA200 bull | 19/63.2%/0.42/+76.77% | 11/45.5%/-0.03/-6.48% | **-0.03** | trend 非綁定 |

### 核心失敗發現（lesson #5 / TQQQ-015 邊界擴展，repo 首次 TQQQ-native squeeze breakout）

1. **BB squeeze breakout 於 TQQQ 自身結構性失敗**——3x leverage 放大假
   突破虧損，Part B 45.5% WR 近隨機 + 累計負報酬，**與 TQQQ-015（QQQ 源）
   失敗模式平行**：訊號源 QQQ 或 TQQQ 自身皆失敗
2. **squeeze 閾值結構性兩難**——0.20 有訊號流但含大量假突破；0.15 對
   3x vol ETF 過嚴（TQQQ BB width/Close 結構性 ≥ ~0.18），無 sweet spot
3. **長期趨勢 regime filter 不能救 Part B**——SMA200 bull filter 改善
   Part A（過濾 2022 熊市假突破）但對 Part B 完全非綁定（2024-2025
   bull regime 內部假突破無法以長期趨勢區分），確認 leveraged ETF
   breakout 假突破為 regime-internal 噪音、非 regime-classifiable
4. **新跨資產規則（lesson #5 邊界精煉）**：leveraged tech ETF（TQQQ
   5-6% vol）僅 mean-reversion / extreme capitulation 維度有效
   （TQQQ-001~018），breakout / momentum continuation / single-day
   reversal 維度全失敗（TQQQ-006/015/027/028）
5. **TQQQ 第 28 次實驗、新增「突破 Breakout（TQQQ-native）」第 16 大
   策略類型**（REJECT）。TQQQ-025 Att2 仍為 min(A,B) 全域最優

---

## TQQQ-029: TQQQ Donchian Channel Trend-Following（趨勢跟蹤，3 次漸進但 REJECT）

### 動機
策略方向 **趨勢跟蹤 (Trend Following)** — repo 中 TQQQ 較少使用之方向
（TQQQ-006 動量崩潰做空型 / TQQQ-015 QQQ 動量 / TQQQ-028 BB squeeze，
皆非 Turtle 式 N 日高點 Donchian 趨勢跟蹤）。leveraged tech ETF 長期
secular uptrend，測試 Turtle 式順勢做多 + regime 過濾能否吃 3x 趨勢段。

### 策略設計（純新訊號框架）
Close > 前 N 日 High rolling max（Donchian 突破）+ Close > SMA(trend)
趨勢確認 + 可選 SMA200 bull regime，隔日開盤買 TQQQ（成交模型）。

### 迭代歷程（最多三次，漸進改善）

| Att | Donchian | 趨勢/regime | 出場 | Part A | Part B | min(A,B) |
|-----|----------|-------------|------|--------|--------|----------|
| 1 | 20 | SMA50 | +12/-8/20d | 50/56.0%/0.28 | 19/42.1%/-0.01/-9.67% | **-0.01** |
| 2 | 55 | SMA100+SMA200 | +12/-8/20d | 34/61.8%/0.42 | 16/43.8%/0.01/-4.52% | **0.01** |
| 3 ★ | 100 | SMA150+SMA200 | +20/-10/30d | 27/66.7%/0.59 | 12/50.0%/0.16/+15.50% | **0.16** |

### 核心失敗發現（lesson #5 邊界擴展，repo 首次 Donchian 趨勢跟蹤於 leveraged tech ETF）

1. **漸進改善但結構性 plateau 遠低於 capitulation baseline**——拉長
   lookback（20→55→100）+ bull regime + 拉寬趨勢跟蹤出場使 min(A,B)
   -0.01→0.01→0.16，但 Part B 始終 ≤ 50% WR（choppy 2024-25 近 coin-flip）
2. **3x leverage 與趨勢跟蹤根本張力**——緊停損（-8%）被 uptrend 正常
   intra-trend 回檔 whipsaw；拉寬（-10%）+ 長持倉 + ultra-selective
   Donchian(100) 改善 WR 但仍 regime-dependent；趨勢延續進場（N 日新高
   = local 極端）在 3x ETF 上 = buy near-term top
3. **新跨資產規則（lesson #5 邊界精煉，整合 TQQQ-006/015/027/028/029）**：
   leveraged tech ETF 趨勢跟蹤 / breakout / momentum / single-day
   reversal 維度結構性 underperform——**僅「逆勢深度超賣 capitulation」
   維度可在 3x ETF 產生 risk-adjusted alpha**，順勢維度全失敗
4. **TQQQ 第 29 次實驗、新增「趨勢跟蹤 Trend Following（Donchian）」第
   17 大策略類型**（REJECT）。TQQQ-025 Att2 仍為 min(A,B) 全域最優

---

## TQQQ-030: TQQQ Dual-Horizon Momentum Continuation（動量，3 次 REJECT，session 收尾）

### 動機
策略方向 **動量 (Momentum)** — TQQQ-006 為動量崩潰做空型 / TQQQ-015 為
QQQ ROC 源，**從未於 TQQQ 自身做雙時框（短+中）動量堆疊持續性確認**。
本實驗為本 session 5 大方向（配對/單日反轉/突破/趨勢/動量）最後一塊
拼圖，統一驗證 lesson #5 邊界。

### 策略設計（純新訊號框架）
ROC(短) > 閾值 + ROC(中) > 閾值（動量持續堆疊）+ Close > SMA(trend)
+ 可選 SMA200 bull + 可選 BB 波動率上限，隔日開盤買 TQQQ（成交模型）。

### 迭代歷程（最多三次）

| Att | ROC 短/中 | regime | Part A | Part B | min(A,B) |
|-----|-----------|--------|--------|--------|----------|
| 1 | 10>5%/20>8% | SMA50 | 59/62.7%/0.31 | 24/41.7%/-0.11/**-27.12%** | **-0.11** |
| 2 | 10>8%/20>12% | +SMA200 bull | 38/57.9%/0.25 | 14/57.1%/0.12/+9.53% | **0.12** |
| 3 ★ | 10>8%/20>12% | +BB vol cap | 38/57.9%/0.25 | 14/57.1%/0.12/+9.53% | **0.12** |

### 核心失敗發現（lesson #5 邊界第四次擴展，本 session 統一結論收尾）

1. **動量追高於 3x leveraged ETF 災難性**——Att1 Part B -27.12%
   （momentum continuation = buy strength = buy near-term top，3x 放大
   使 momentum-chase 在 choppy regime 連續觸 -8% SL）
2. **更強閾值 + bull regime 使 Part B 可存活但結構性 plateau**——
   min(A,B) -0.11→0.12 但遠低 capitulation baseline 0.80
3. **vol cap 對「強動量 + bull」conjunction 完全非綁定**——強多時框
   動量結構性發生於 lower-vol uptrend，與 capitulation（high-vol panic，
   需 vol regime gate 過濾）形成鮮明 mirror 對比
4. **本 session 統一跨資產結論（lesson #5 邊界第四次擴展，整合
   TQQQ-026~030 五大方向）**：leveraged tech ETF 之 配對交易（026）+
   單日反轉 MR（027）+ 突破（028）+ 趨勢跟蹤（029）+ 動量（030/006/015）
   五大「順勢 / 鏡射 / 短週期反轉」方向結構性全 REJECT——**唯一可在 3x
   leveraged ETF 產生 risk-adjusted alpha 的維度為「逆勢深度超賣 extreme
   capitulation」**（TQQQ-001~018/025），因 3x 放大將任何 buy strength /
   breakout / new-high / momentum / inverse-pair 進場轉為 buy local
   extreme → mean-revert
5. **TQQQ 第 30 次實驗、新增「動量 Momentum（TQQQ-native 雙時框）」第
   18 大策略類型**（REJECT）。TQQQ-025 Att2 仍為 min(A,B) 全域最優
...
<!-- 更新指引：
  Legacy experiment research retired 2026-08-26. Do not rerun or update outcome tables.
  Historical values remain frozen; new outcome-relevant work uses a released workflow Study.
-->
