...
**TQQQ-018 Att3 為新全域最優**（21 次實驗、9 大策略類型：均值回歸、趨勢/動量/突破、Gap-Down 資本化、盤中/加速/多日確認、**regime + transition combo filter**、**^VIX direction filter**（TQQQ-019 失敗）、**^VIX peak-passing filter**（TQQQ-020 失敗）、**^MOVE LEVEL filter**（TQQQ-021 失敗）、**^MOVE DIRECTION filter**（TQQQ-021 失敗））

**最近失敗實驗：** TQQQ-021（^MOVE Bond-Vol Regime Gate on TQQQ-018，**repo 首次 ^MOVE（bond vol）於 leveraged tech ETF / equity asset — 全部 REJECT/TIE**）— Att1 LEVEL<=130 TIE 0.80（非綁定）/ Att2 3d DIRECTION<=+5 min 0.66（reverse selection）/ Att3 5d DIRECTION<=+8 min 0.66（成功過濾 2025-03-06 SL 但 Part A heavy attrition + cooldown shift）。確認**lesson #24 family v9 boundary expansion**：所有 implied vol 維度（equity ^VIX + bond ^MOVE）對 TQQQ -15% extreme capitulation framework 結構性失效，需考慮非 implied vol 維度或完全替代 framework。
...
> **最新實驗 (Latest):** TQQQ-021 `tqqq_021_move_regime_gate`（^MOVE bond-vol LEVEL + DIRECTION filter，repo 首次 ^MOVE 於 leveraged tech ETF / equity asset，3 次 REJECT/TIE — 整合 lesson #24 family v9 邊界擴展：所有 implied vol 維度（equity ^VIX + bond ^MOVE）對 TQQQ -15% extreme capitulation framework 結構性失效）
