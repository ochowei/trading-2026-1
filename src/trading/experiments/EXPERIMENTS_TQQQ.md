...
**當前最佳（無成交模型）：** TQQQ-008（基線三條件進場 DD -15% + RSI(5)<25 + Vol 1.5x，TP +7% / SL -8%，持倉 10 天，無追蹤停利）
**當前最佳（含成交模型）：** TQQQ-010（重做 TQQQ-008 + 成交模型）
**全期間累計：** TQQQ-008: Part A +120.21%, Part B +45.44% / TQQQ-010: Part A +55.44%, Part B +47.59%

### 已證明無效（附有效條件）

| 結論 | 信心等級 | 測試範圍 | 有效前提 | 可重試條件 |
|------|---------|---------|---------|-----------|
| 放寬進場條件品質大幅下降 | ★★★ 高 | TQQQ-002(全放寬), TQQQ-009(僅DD) | TQQQ 恐慌抄底策略 | 基線 DD-15%/RSI<25/Vol1.5x 已是最優，不建議放寬 |
| 追蹤停利降低勝率與報酬 | ★★★ 高 | TQQQ-003(-4%), TQQQ-005(-6%) | TQQQ 恐慌抄底策略 | 結構性問題：TQQQ 極端波動下追蹤停利截斷獲利 |
| VIX ≥ 25 過嚴（訊號數暴降） | ★★ 中 | TQQQ-004 | 當前 VIX 分布 | VIX 分布若長期上移可重試 |
| VIX ≥ 20 改善有限 | ★★ 中 | TQQQ-005 | 搭配追蹤停利 | 可嘗試 VIX ≥ 20 搭配固定出場（未測試此組合） |
| 動能崩潰策略回撤過大 | ★★ 中 | TQQQ-006 | 多日下跌 pattern | 若能找到更好的風控可重試 |
| QQQ RSI 過濾 + 優化出場訊號太少 | ★★ 中 | TQQQ-013 | QQQ RSI(14)<35 + TP+7% | 過濾條件與高 TP 同時使用導致有效訊號不足 |

### 已掃描的參數空間

**進場**：基線三條件（DD/RSI/Vol）不可放寬（TQQQ-002, 009 驗證）；VIX 過濾效果不佳；QQQ RSI 過濾可提升品質但降低訊號數
**出場**：TP +5%~+12%, SL -6%~-10%, 持倉 7~12 天 → **TP +7%, SL -8%, 持倉 10 天最佳**
**追蹤停利**：-4%, -6% → **均失敗，不使用**

### Part C 即時回饋（2026-01-01 ~ 至今，上次更新 2026-03-27）

- 尚無 Part C 數據（TQQQ-001~009 無成交模型不支援 Part C，TQQQ-010~013 待累積）

### 尚未嘗試的方向（可探索）

- Put/Call Ratio 恐慌確認（期權市場情緒）
- 信用利差（HY spread）恐慌確認
- 多日 pattern + 固定出場（TQQQ-006 的進場 + TQQQ-008 的出場，未組合測試）
- VIX ≥ 20 + 固定出場（TQQQ-005 用了追蹤停利，未搭配 TQQQ-008 的固定出場）
- 波動率聚類（區分不同 vol regime 使用不同參數）

### 關鍵資產特性

- 3 倍槓桿 QQQ ETF，極端波動，日跌幅可達 -10% 以上
- 恐慌抄底策略的黃金標的：極端下跌後反彈力道強
- 進場條件極為敏感，任何放寬都會引入低品質訊號（已多次驗證）
- 追蹤停利在此資產類型已被驗證為有害
- 成交模型下 Part A 績效會大幅縮水（隔日開盤 vs 訊號日收盤），Part B 則略優
<!-- AI_CONTEXT_END -->
