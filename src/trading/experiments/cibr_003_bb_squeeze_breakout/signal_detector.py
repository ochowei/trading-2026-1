...
from trading.experiments.cibr_003_bb_squeeze_breakout.config import CIBRBBSqueezeConfig

logger = logging.getLogger(__name__)


class CIBRBBSqueezeSignalDetector(BaseSignalDetector):
    def __init__(self, config: CIBRBBSqueezeConfig):
        self.config = config
    ...
    def detect_signals(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        if suppressed:
            ...
            logger.info(
                "CIBR-003: %d duplicate signals suppressed by cooldown",
                len(suppressed),
            )

        signal_count = df["Signal"].sum()
        logger.info("CIBR-003: Detected %d BB squeeze breakout signals", signal_count)
        return df
