...
| FCX-013 | `fcx_013_regime_breakout`     | Multi-Week Regime-Aware BB Squeeze Breakout（FCX-004 + buffered SMA(20)≥k×SMA(60)，**Att3 SUCCESS k=1.00 嚴格 min(A,B) 0.55 +34%**，repo 第 3 次 lesson #22 試驗、首次商品/礦業單股、跨資產反向發現） | ✅ 突破策略前任最佳 |
| FCX-014 | `fcx_014_breakout_ceiling`    | Multi-Period Direction-Filter Regime Breakout（FCX-013 Att3 + **訊號日 3d ceiling <= +12.0%** rally exhaustion filter，**Att1 SUCCESS min(A,B) 0.64 +16%**，repo 首次 lesson #19 family ceiling 維度移植至 BB Squeeze Breakout 框架） | ★ 突破策略當前最佳 |
...
**結論：FCX-014 Att1 為 FCX 突破策略新全域最佳，min(A,B) 0.64 vs FCX-013 Att3 的 0.55（+16%），repo 首次 lesson #19 family ceiling 跨策略至 BB Squeeze Breakout 框架；threshold sweep 確認 [11%, 12%] robust sweet spot 區間。**
