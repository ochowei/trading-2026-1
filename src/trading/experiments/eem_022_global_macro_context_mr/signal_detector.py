"""
EEM-022 訊號偵測器：Global-Equity Macro-Context Confirmation Gate on Vol-Transition MR
...
7. **SPY N 日報酬 <= macro_return_threshold（EEM-022 核心新增）**
8. 冷卻期 10 個交易日
"""
...
class EEM022SignalDetector(BaseSignalDetector):
    ...
    def detect_signals(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        for d in base_dates:
            logger.info(
                "EEM-022 base-signal %s | Macro_Return(%dd)=%.4f",
                d.strftime("%Y-%m-%d"),
        ...
        if suppressed:
            ...
            logger.info("EEM-022: %d signals suppressed by cooldown", len(suppressed))

        signal_count = df["Signal"].sum()
        logger.info(
            "EEM-022: Detected %d signals (macro_lookback=%d, macro_return_threshold=%.4f)",
            signal_count,
            self.config.macro_lookback,
