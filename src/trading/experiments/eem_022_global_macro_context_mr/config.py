...
- 與既有 EEM-006（EEM-SPY RS 動量回調作**主訊號**，3 次嘗試全失敗，lesson #20）
  **本質不同**：EEM-006 用 EEM-SPY 相對強度差作 primary momentum entry signal；
  EEM-022 用 SPY **絕對** drawdown 作疊加於已驗證 MR 主訊號之上的 regime
...
@dataclass
class EEM022Config(ExperimentConfig):
    """EEM-022 Global-Equity Macro-Context Confirmation Gate on Vol-Transition MR 參數"""
    ...
    # === EEM-022 核心新增：SPY 寬基 macro-context confirmation gate ===
...
def create_default_config() -> EEM022Config:
    return EEM022Config(
        name="eem_022_global_macro_context_mr",
        experiment_id="EEM-022",
