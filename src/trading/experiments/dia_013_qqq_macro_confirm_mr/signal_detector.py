"""
DIA-013 訊號偵測器：QQQ Macro-Confirmation Gate MR
...
6. **QQQ 10 日報酬 <= macro_max_return（DIA-013 新增 macro confirmation）**
...
from trading.experiments.dia_013_qqq_macro_confirm_mr.config import DIA013Config
...
class DIA013SignalDetector(BaseSignalDetector):
    ...
    def compute_indicators(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        # ── DIA-013 新增：QQQ N 日報酬（QQQ_Close 由 strategy.run() 合併進來）──
        if "QQQ_Close" in df.columns:
            ...
            logger.warning("DIA-013: QQQ_Close 欄位缺失，macro confirmation 將停用")
    ...
    def detect_signals(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        logger.info(
            "DIA-013: Detected %d QQQ Macro-Confirmed signals",
            signal_count,
        )
        return df
