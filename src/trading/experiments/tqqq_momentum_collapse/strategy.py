...
捕捉連續多日緩慢洗盤的底部形態。
Captures multi-day momentum collapse bottoming patterns.
"""
...
class TQQQMomentumCollapseStrategy(BaseStrategy):
    ...
    假設：捕捉「連續多日緩慢洗盤」底部形態，並在 50MA 之下確認下跌趨勢中。
    Hypothesis: Captures "continuous multi-day wash-out" bottoming patterns while confirming a downtrend below the 50MA.
    """
    ...
    def _print_strategy_params(self, config: ExperimentConfig) -> None:
        ...
        print(f"  負日比例門檻 (Negative days threshold): ≥ {config.negative_days_threshold} / {config.lookback_period} days")
        print(f"  累計跌幅門檻 (Cumulative drop threshold): ≤ {config.cumulative_drop_threshold:.0%} over {config.lookback_period} days")
        print(f"  趨勢過濾 (Trend filter):         Close < SMA({config.sma_period})")
