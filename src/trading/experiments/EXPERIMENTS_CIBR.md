...
- **^VIX Implied-Vol Regime BANDS Filter MR（CIBR-016，3 次迭代全部 FAIL/REJECT，repo 第 2 次 lesson #24 BANDS 變體、首次 BANDS 失敗）**：XBI-017 Att1 跨資產移植於 CIBR-008 Att2 SL-bearing MR base
  ...
  - **核心結論**：forward-looking ^VIX BANDS U-shape **方向資產相依**（EEM-014 family 同構）——僅適用於 (a) 殘餘 SLs 構成真實 VIX U-shape **且** (b) 未被 realized-return/vol 維度先行隔離；CIBR 雙重違反（SL 與 winners 交錯 + 已被 CIBR-014 1d-cap 涵蓋）。BANDS 維度在交錯時可被濫用為 post-hoc 單點 notch。**CIBR 失敗策略類型達 10 種**
...
- ~~^VIX forward-looking implied-vol BANDS gate（XBI-017 跨資產移植）~~ → CIBR-016 三次迭代全部 FAIL/REJECT（Att1 17/22 方向相反 min 0.65、Att2 24.7/25.4 post-hoc 單點 notch 名目 4.38 REJECT、Att3 ablation 崩潰 -86%）。CIBR 唯一 capitulation-MR SL 與 winners 在 VIX 維度交錯（非 U-shape）且已被 CIBR-014 Att2 1d-cap 涵蓋，lesson #24 BANDS 變體方向資產相依，CIBR 結構性不適用
...
| CIBR-016 | `cibr_016_vix_bands_mr`                 | ^VIX Implied-Vol Regime BANDS Filter MR（XBI-017 跨資產移植於 CIBR-008 Att2 base，repo 第 2 次 lesson #24 BANDS 變體、首次 BANDS 失敗，3 次迭代全部 FAIL/REJECT）| 已完成（失敗）|
