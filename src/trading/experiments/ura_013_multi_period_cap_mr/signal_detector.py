"""
URA-013 signal detector: Multi-Period Capitulation-Strength Filter MR
...
6. 5-day cumulative return >= -10.5% (NEW: multi-period capitulation cap;
   filter signals where the recent 5 days had a deep multi-day
   continuation/acceleration; lesson #19 family extension)
...
from trading.experiments.ura_013_multi_period_cap_mr.config import URA013Config

logger = logging.getLogger(__name__)


class URA013SignalDetector(BaseSignalDetector):
    """URA-013 multi-period capitulation-strength filter signal detector"""

    def __init__(self, config: URA013Config):
        self.config = config
    ...
    def detect_signals(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        if suppressed:
            ...
            logger.info(
                "URA-013: %d duplicate signals suppressed by cooldown",
                len(suppressed),
            )

        signal_count = df["Signal"].sum()
        logger.info("URA-013: Detected %d multi-period cap MR signals", signal_count)
        return df
