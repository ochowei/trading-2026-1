"""
DIA 回檔 + Williams %R + 反轉K線均值回歸策略
(DIA Pullback + Williams %R + Reversal Candle Mean Reversion Strategy)
...
from trading.experiments.dia_001_pullback_wr_reversal.signal_detector import (
    DIAPullbackWRReversalDetector,
)
...
class DIAPullbackWRReversalStrategy(ExecutionModelStrategy):
    ...
    def create_detector(self) -> BaseSignalDetector:
        return DIAPullbackWRReversalDetector(create_default_config())
    ...
    def _print_strategy_params(self, config: ExperimentConfig) -> None:
        if isinstance(config, DIAPullbackWRReversalConfig):
            print(
