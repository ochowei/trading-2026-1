...
**當前最佳：** IBIT-001（回檔 12-22% + WR(10) ≤ -80 + 冷卻 15 天，Part A Sharpe 0.15，Part B Sharpe 0.37）**已確認全域最優**（5 次實驗、15 次嘗試，含均值回歸、波動率自適應、突破、趨勢回檔、短期動量五大策略類型）
...
**已掃描的參數空間：**
- 進場條件：回檔 12~15% + 上限 22% + WR(10) ≤ -80 + 冷卻 10~20 天 + RSI(2) < 12~15 + 2日跌幅 ≤ -5% + ATR(5)/ATR(20) > 1.0~1.05
- 回檔回看窗口：10日（最佳）、20日（Part B 訊號惡化，IBIT-005 Att1 驗證）
- 出場參數：TP +5~8% / SL -5.5~-9% / 持倉 15~20 天
- SL 完整掃描：-5.5%（過緊）、-6%（過緊）、-7%（最佳）、-8%（過寬，IBIT-005 Att3）、-9%（過寬）
- 策略類型：均值回歸（IBIT-001/002）、波動率自適應（IBIT-004 Att1/2）、2日急跌過濾（IBIT-004 Att3）、BB 擠壓突破（IBIT-003 Att1）、趨勢動量回檔（Att2）、RSI(5) 趨勢回檔（Att3）、短期動量（IBIT-005 Att2）
...
- 均值回歸（IBIT-001）仍為唯一可行策略類型；突破、趨勢回檔、波動率自適應、2日急跌過濾、短期動量均因數據不足或結構性問題而失敗
...
- **SL -7% 硬邊界確認**：-6% 過緊（翻轉勝利交易），-8% 過寬（停損交易均跌穿，只增加虧損），-7% 是唯一可行值
<!-- AI_CONTEXT_END -->
