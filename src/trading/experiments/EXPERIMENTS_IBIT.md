...
**當前最佳：** IBIT-001（回檔 12-22% + WR(10) ≤ -80 + 冷卻 15 天，Part A Sharpe 0.15，Part B Sharpe 0.37）
...
**已掃描的參數空間：**
- 進場條件：回檔 12~15% + 上限 22% + WR(10) ≤ -80 + 冷卻 10~15 天
- 出場參數：TP +5~7% / SL -7~-9% / 持倉 15~20 天
...
**尚未嘗試的方向（可探索）：**
- RSI(2) < 15 替代 WR 進場（短週期匹配 IBIT 平均持倉 4 天）
- 2 日跌幅 ≤ -5% 動量確認（但注意教訓 #6 邊際效益遞減）
- 非對稱出場 TP +5% / SL -5% / 20天（參考 URA-001 對稱架構）
- 冷卻 20 天（更長冷卻期是否進一步改善）
...
| ID       | 資料夾                    | 策略摘要                              | 狀態  |
|----------|--------------------------|--------------------------------------|-------|
| IBIT-001 | `ibit_001_pullback_wr`    | 回檔範圍 12-22% + Williams %R 均值回歸 | 已完成 |
...
```
IBIT-001 (回檔 12-22% + WR(10) ≤ -80 + 冷卻 15 天)
  ├── IBIT-002 (RSI(2) < 15 替代 WR 進場，匹配短持倉週���)
  ├── IBIT-003 (2 日跌幅 ≤ -5% 動量確認，需注意教訓 #6)
  └── IBIT-004 (對稱出場 TP/SL ±6%，參考 URA-001)
```
