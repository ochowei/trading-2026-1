...
- 策略類型：均值回歸（IBIT-001/002）、BB 擠壓突破（IBIT-003 Att1）、趨勢動量回檔（Att2）、RSI(5) 趨勢回檔（Att3）
...
- 數據僅 ~2 年（2024-01-11 起），訊號樣��量有限，是策略開發的根本瓶頸
- 高波動度使追蹤停損禁用（教訓 #2），冷卻期需較長以避免下跌趨勢連續進場
- A/B 訊號率比 1.25:1（優秀），Part B Sharpe 優於 Part A（無過擬合）
- 均值回歸（IBIT-001）仍為唯一可行策略類型；突破（IBIT-003 Att1）與趨勢回檔（Att2/Att3）均因訊號不足而失敗
<!-- AI_CONTEXT_END -->
