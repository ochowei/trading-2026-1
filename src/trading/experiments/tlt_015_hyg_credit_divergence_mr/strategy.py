"""TLT HYG Credit Divergence Regime-Gated MR 策略 (TLT-015)"""
...
from trading.experiments.tlt_015_hyg_credit_divergence_mr.signal_detector import (
    TLT015SignalDetector,
)
...
class TLT015HygCreditDivergenceMRStrategy(ExecutionModelStrategy):
    ...
    def create_detector(self) -> BaseSignalDetector:
        return TLT015SignalDetector(create_default_config())

    def _print_strategy_params(self, config: ExperimentConfig) -> None:
        if isinstance(config, TLT015Config):
            print(
            ...
            print(
                f"  Credit divergence gate: {config.credit_ticker} - TLT"
                f" {config.credit_lookback}d return diff <= {config.max_credit_outperformance:+.1%}"
            )
