"""
EWZ-007 Post-Capitulation Vol-Transition MR 訊號偵測器

在 EWZ-006 Att3 框架（BB 下軌 + 回檔上限 + WR + ClosePos + ATR）之上新增
...
from trading.experiments.ewz_007_vol_transition_mr.config import EWZ007Config

logger = logging.getLogger(__name__)


class EWZ007SignalDetector(BaseSignalDetector):
    """
    EWZ Post-Capitulation Vol-Transition MR 訊號偵測器
    ...
    def __init__(self, config: EWZ007Config):
        self.config = config
    ...
    def detect_signals(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        if suppressed:
            ...
            logger.info("EWZ-007: %d signals suppressed by cooldown", len(suppressed))

        signal_count = df["Signal"].sum()
        logger.info(
            "EWZ-007: Detected %d signals (mode=%s, threshold=%.4f)",
            signal_count,
