"""
TSM 極端超賣均值回歸訊號偵測器
TSM Extreme Oversold Mean Reversion Signal Detector
...
from trading.experiments.tsm_001_extreme_oversold.config import TSMExtremeOversoldConfig

logger = logging.getLogger(__name__)


class TSMExtremeOversoldDetector(BaseSignalDetector):
    """TSM 極端超賣訊號偵測器"""

    def __init__(self, config: TSMExtremeOversoldConfig):
        self.config = config
    ...
    def detect_signals(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        logger.info(f"TSM: Detected {signal_count} extreme oversold signals")
        return df
