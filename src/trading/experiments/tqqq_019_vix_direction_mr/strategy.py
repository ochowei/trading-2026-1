"""TQQQ-019 ^VIX Direction Filter on Vol-Regime-Gated Capitulation Buy 策略"""
...
from trading.experiments.tqqq_019_vix_direction_mr.signal_detector import (
    TQQQ019SignalDetector,
)
...
class TQQQ019VixDirectionStrategy(ExecutionModelStrategy):
    ...
    def _print_strategy_params(self, config: ExperimentConfig) -> None:
        ...
        print(
            f"  ^VIX 方向過濾 (VIX Direction):   {config.vix_ticker} "
            f"{config.vix_direction_lookback}d change <= "
            f"{config.max_vix_direction_change:+.1f}"
        )
