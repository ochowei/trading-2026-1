...
| FXI-015 | `fxi_015_ashr_divergence_mr`   | FXI-ASHR Cross-Asset Divergence Filter on ATR-Band MR（FXI 20d - ASHR 20d >= -8%，**Att2 ★ SUCCESS min(A,B) 1.21 +19.8% vs FXI-014**）| ✅ **當前最佳** |
| FXI-016 | `fxi_016_usdcnh_direction_mr`  | USDCNY Direction Filter on Cross-Asset Divergence MR（CNY=X 10d <= +X%，repo 首次 USDCNH/USDCNY direction filter，EWJ-006 跨資產移植 — 3 次迭代均 REJECT）| 已完成（失敗，定義 lesson #24 family v8 邊界：管理浮動匯率資產上 bilateral FX direction filter 結構性失效）|
| FXI-017 | `fxi_017_cny_regime_mr`        | FXI–CNY (Yuan) Currency-Regime-Gated MR（family v4，移植 GLD-016 GLD–USD 形式至政策驅動 EM，3 次迭代均失敗）| 已完成（documented-failure，predict→confirm 命中；延伸禁忌 #36e：country-specific 貨幣 regime gate 不可移植至政策驅動單一國家 EM 股票 ETF）|
