...
**當前最佳：** NVDA-003（BB Squeeze Breakout，TP+8%/SL-7%/20天）— Part A Sharpe 0.40/Part B Sharpe 0.47，大幅超越 NVDA-001 均值回歸（0.23/0.44）
**前任最佳：** NVDA-001（RSI(2) 極端超賣 + 2日急跌均值回歸）— Part A Sharpe 0.23/Part B Sharpe 0.44
**滾動窗口分析摘要：** NVDA-001 ✗✓（精準度突變 ΔWR 25.0pp，績效漸變，訊號極稀少統計可信度低）

**NVDA-003 vs NVDA-001 關鍵改善：**
- 訊號數量：Part A 6→15（+150%），Part B 4→8（+100%），統計可信度大幅提升
- Part A Sharpe：0.23→0.40（+74%），Part B Sharpe：0.44→0.47（+7%）
- 累計報酬：Part A +9.88%→+45.82%（+364%），Part B +13.21%→+25.36%（+92%）
- PF：Part A 1.58→2.18，Part B 2.37→2.61
- A/B 訊號比 1.88:1（可接受），A/B Sharpe 比 0.85:1（無過擬合）
- **突破策略首次超越均值回歸**（同 TSLA-005 結論，確認 lesson #71）
...
- TP +8% 是 NVDA 的硬上限（均值回歸和突破均驗證），SL -7% 是突破策略甜蜜點

**已掃描的參數空間：**
- 均值回歸進場：RSI(2)<5 + 2日跌幅≤-7%
- 均值回歸出場：TP+8%/SL-10%/15天（最佳均值回歸）、TP+10%/SL-10%/20天、TP+8%/SL-12%/25天
- 回撤範圍：[-35%, -10%] 60日回看（過濾好訊號）
- 突破進場：BB(20,2) 擠壓（60日 25th 百分位，5日內）+ Close > Upper BB + Close > SMA(50)
- 突破出場：TP+8%/SL-7%/20天（最佳）、TP+10%/SL-7%/20天、TP+8%/SL-5%/20天

**尚未嘗試的方向（可探索）：**
- 突破 + SL -8%（-7% 和均值回歸 -10% 之間的中間值）
- SMA(20) 替代 SMA(50) 趨勢確認
- 冷卻期調整（10天或20天 vs 現行15天）

**關鍵資產特性：**
- 日均波動 ~3.26%，為 GLD 的 2.92 倍（HIGH-BETA 個股）
- AI/半導體龍頭，動量效應強，突破策略優於均值回歸（同 TSLA）
- 2022 年經歷 -66% 峰谷回撤，均值回歸在此期間產生假訊號，突破策略只在 2022 產生 2 筆停損
- 訊號頻率：突破 3.0/年（vs 均值回歸 1.2/年），統計可信度顯著提升
<!-- AI_CONTEXT_END -->
...
| ID       | 資料夾                      | 策略摘要                               | 狀態    |
|----------|----------------------------|---------------------------------------|---------|
| NVDA-001 | `nvda_001_extreme_oversold` | RSI(2)<5 + 2日急跌≤-7%，TP+8%/SL-10%/15天 | 已完成（前任最優） |
| NVDA-002 | `nvda_002_capped_drawdown` | 出場參數探索（3次嘗試均失敗），確認 NVDA-001 為均值回歸最優 | 已完成 |
| NVDA-003 | `nvda_003_bb_squeeze_breakout` | BB(20,2) 擠壓突破 + SMA(50) 趨勢，TP+8%/SL-7%/20天 | 已完成（當前最優） |
...
## NVDA-003: Bollinger Band Squeeze Breakout（當前最優）
...
NVDA-003 (BB 擠壓突破 + SMA(50)，TP+8%/SL-7%/20天) ← 當前全域最優
