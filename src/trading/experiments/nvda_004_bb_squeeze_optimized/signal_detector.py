"""
NVDA-004 訊號偵測器：BB Squeeze Breakout Optimized
NVDA-004 Signal Detector: BB Squeeze Breakout Optimized
...
from trading.experiments.nvda_004_bb_squeeze_optimized.config import NVDABBSqueezeOptConfig

logger = logging.getLogger(__name__)


class NVDABBSqueezeOptDetector(BaseSignalDetector):
    """NVDA BB Squeeze Breakout Optimized 訊號偵測器"""

    def __init__(self, config: NVDABBSqueezeOptConfig):
        self.config = config
    ...
    def detect_signals(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        logger.info("NVDA: Detected %d BB squeeze breakout optimized signals", signal_count)
        return df
