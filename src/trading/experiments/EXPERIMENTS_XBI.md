...
**當前最佳：** XBI-005（回檔 8-20% + WR(10) ≤ -80 + ClosePos ≥ 35%，Part A Sharpe 0.36，Part B Sharpe 0.64）— **已確認為全域最優**（9 次實驗、29+ 次嘗試，含突破策略、ROC 策略、動量回調策略、配對交易策略、波動率自適應過濾、RSI(2) 獨立驗證）
...
**尚未嘗試的方向（預期效益極低）：**
- XBI-005 已確認為全域最優，突破、ROC、動量回調、配對交易、波動率自適應、RSI(2) 六大策略類型均已驗證無效
- 所有主要策略類型已窮盡（均值回歸、突破、動量回調、ROC、配對交易、波動率自適應）
