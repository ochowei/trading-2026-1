<!-- AI_CONTEXT_START - 此區塊供 AI Agent 快速讀取，人工更新
  ...
  phase9_note_2026_08_06: XBI-008, XBI-017, XBI-019, and XBI-020 now consume declared IBB/^VIX/XLV historical auxiliary bundles through the provider-free execution boundary; migration runs remain migration-pending and do not advance latest or followup qualification.
  phase9_note_2026_08_06_legacy_wave: XBI-016 now consumes its declared QQQ historical auxiliary bundle through the same provider-free boundary; the migration run remains migration-pending and does not advance latest or followup qualification.
  phase9_note_2026_08_07_primary: XBI-001, XBI-004 through XBI-007, and XBI-009 through XBI-015 now consume declared primary XBI bundles through the provider-free execution boundary; migration runs remain migration-pending and do not advance latest or followup qualification.
  ...
  note_2026_05_10_xbi019: XBI-019 added 2026-05-10 (XLV Sector Parent Trend Filter on VIX Bands MR — **Att3 ★ SUCCESS**, repo 首次「sector parent absolute momentum direction filter」於任何資產 + repo 首次「regime-conditional cross-asset gate」（高 VIX 帶內條件性套用 XLV panic-prep 條件）). Three iterations against XBI-017 Att1 baseline min(A,B) 0.64: Att1 (FLOOR direction, XLV 10d return >= -3.0%) Part A 5/80%/Sharpe **1.97** cum +14.71% / Part B 4/75%/Sharpe **0.36** cum +5.22% (含 2025-03-31 SL) / min **0.36** REJECT (-44%) — 2025-03-31 SL 之 XLV 10d return > -3% 過 filter，filter 反向移除多筆 Part A high-VIX 廣 panic V-bounce winners (XLV 10d 深度負值區域); Att2 (FLOOR direction strict, XLV 10d return >= -1.5%) Part A 4/75%/Sharpe **1.71** cum +10.83% / Part B 4/75%/Sharpe 0.36 (與 Att1 完全相同, 2025-03-31 SL 仍存) / min **0.36** REJECT — 加嚴 FLOOR 持續無 surgical selectivity，僅進一步傷害 Part A，**確認 FLOOR 方向結構性錯誤** (2025-03-31 XLV 10d > -1.5% 仍過 filter); **Att3 ★ SUCCESS (regime-conditional XLV panic gate)** — 低 VIX 帶 (<= 17) 沿用 XBI-017 BANDS（biotech isolated dip 不需 XLV 確認）；高 VIX 帶 (> 22) 加 XLV 10d return <= -2.0% panic V-bounce 預備條件 — Part A 9 訊號 / WR **100%** / std=0 zero-var / cum +36.29% / MDD -4.62% / Part B 4 訊號 / WR **100%** / std=0 zero-var / cum +14.75% / MDD -2.07% — **STRUCTURAL NO LOSS 雙 Part 全勝零方差**（沿用 IBIT-009/EWJ-005 Att2/EWT-008/SPY-009/IWM-013 慣例 †）/ min(A,B)† **= 結構性 NO LOSS** (1 Part A SL 過濾 + 1 Part B SL 2025-03-31 過濾)。**A/B 平衡完美達標**：Part A 9/yr 5 年 = 1.8/yr, 年化 cum 6.39%/yr / Part B 4 訊號 2 年 = 2.0/yr, 年化 cum 7.12%/yr → 年化 cum gap **|7.12-6.39|/7.12 = 10.3% << 30% ✓✓** (vs XBI-017 13.2%, 進一步改善)；訊號比 1.8:2.0 = **gap 10% << 50% ✓✓** (vs XBI-017 26.7%)。**核心發現（lesson #20 v3 / lesson #24 family v9 邊界擴展，repo 首次 regime-conditional cross-asset gate）**: (1) **Repo 首次「sector parent absolute momentum direction filter」於任何資產**——既往 cross-asset filter 為「相對強度（divergence, lesson #20 v3）」或「forward-looking implied vol（lesson #24 family）」維度；XBI-019 為 sector parent 自身 absolute momentum direction 維度，與兩者結構性正交。 (2) **Repo 首次 regime-conditional cross-asset gate**——XBI-018 Att3 FLOOR 失敗為 broad correction 期 winners 與 sub-sector persistent decline SL 在 RS_20d 維度重疊；XBI-019 Att3 解法為「regime-conditional 應用」——低 VIX 帶內 (biotech isolated dip) 不需 XLV 確認、高 VIX 帶內 (broad panic) 要求 XLV 已 deeply negative (V-bounce 預備區)。**filter 邏輯**：高 VIX + XLV 10d <= -2.0% = 真實廣 panic V-bounce 預備；高 VIX + XLV 10d > -2.0% = 早期 sector-specific 下跌（如 2025-03-31 Trump healthcare tariff 早期），filter 阻擋。 (3) **2025-03-31 SL 過濾機制驗證**：該日 ^VIX > 22 但 XLV 10d 處於 [-2%, 0%) 早期下跌帶（healthcare tariff fears 剛開始，XLV 尚未深跌）→ filter 阻擋。對照 Part A 高 VIX winners (2020-COVID 5/13、2022-bear 5/12、2022-08-30) XLV 10d 皆深於 -2.0%（broad panic 已成熟，V-bounce 預備區）→ filter 通過。 (4) **filter 過濾結構**：XBI-017 11 Part A signals → XBI-019 9 signals（移除 1 SL + 1 winner，淨提升 WR 90.9%→100%）；XBI-017 6 Part B signals → XBI-019 4 signals（移除 1 SL + 1 winner，淨提升 WR 83.3%→100%）。 (5) **XBI 結構性瓶頸首次完全突破**：XBI-017 Part B 1 SL (2025-03-31) 經 18 次實驗 56+ 次嘗試無單一 surgical filter 可清除（XBI-018 三次 RS divergence 失敗），XBI-019 Att3 為**首次完全 surgical 過濾**——關鍵在於「regime-conditional 邏輯」而非單維度 filter。 (6) **新跨資產假設（待驗證）**：(a) regime-conditional cross-asset gate 可能適用其他 sub-sector / sub-segment ETFs（KRE 區域銀行 + ^VIX + XLF panic、IGV 軟體 + ^VIX + XLK panic、SOXX 半導體 + ^VIX + XLK panic），threshold 需依 sub-sector vs sector parent 之動態結構調整；(b) 「regime-conditional cross-asset filter」為 lesson #20 v3 / lesson #24 family 之新通用變體——當單維度 cross-asset filter 對 broad regime 失敗時，加入 broad regime classifier (VIX / sector parent trend) 作為前置條件可繞過 broad correction overlap 問題。**XBI-019 Att3 為新全域最優（19 次實驗、59+ 次嘗試）**，取代 XBI-017 Att1。
  note_2026_05_10_xbi018: XBI-018 added 2026-05-10 (XBI-XLV Cross-Asset Divergence Filter on VIX Bands MR — **repo 第 1 次 lesson #20 v3 cross-asset divergence regime gate 應用於 XBI** + **repo 首次 sub-sector ETF (XBI) vs sector parent ETF (XLV) 配對** + **repo 首次 cross-asset divergence FLOOR 極性反轉變體**). Three iterations against XBI-017 Att1 baseline min(A,B) 0.64: Att1 (rs_lookback=20, max_rs_excess=+3% NVDA-021 直接移植 sub-component vs index 維度) Part A 9/88.9%/Sharpe **2.79** cum +31.63% / Part B 5/80.0%/Sharpe **0.52** cum +8.90% (1 SL 2025-03-31 殘留) / min **0.52** **REJECT (-19%)** — cap +3% 過緊：移除 2 Part A winners + 1 Part B winner（高 RS 健康修正期）但 1 Part B SL 之 RS_20d < +3% 完全非綁定; Att2 (rs_lookback=60, max_rs_excess=+5% INDA-012 中長期維度移植) Part A 7/85.7%/Sharpe **2.42** cum +22.88% / Part B 6/83.3%/Sharpe **0.64** cum +12.71%（**完全等於 XBI-017 baseline**）/ min **0.64** **TIE** — 60d cap 對 Part B 全部 6 訊號（含 SL）完全非綁定，Part A 反向移除 4 winners，**確認 CAP 極性對 XBI 結構性錯誤方向**——XBI 失敗模式不在「prior outperformance rally exhaustion」（NVDA/INDA/EWZ 同類）而在「sub-sector underperformance 持續性下跌」; **Att3 (rs_lookback=20, FLOOR 極性 min_rs_excess >= -2.0%, repo 首次 cross-asset divergence FLOOR 變體)** Part A 4 訊號 / WR **100%** / std=0 zero-var / Sharpe 0.00 / cum +14.75% / MDD -3.62% / Part B 4 訊號 / WR **100%** / std=0 zero-var / Sharpe 0.00 / cum +14.75% / MDD -2.07% — **STRUCTURAL NO LOSS 雙 Part 全勝零方差**（IBIT-009/EWT-010/TQQQ-025 慣例），**1 Part B SL (2025-03-31) 與多個 Part A SLs 全部過濾 ✓**，但 A/B annualized cum gap **60.9% > 30% ❌** (Part A 2.79%/yr vs Part B 7.13%/yr) + signal gap **60% > 50% ❌** (Part A 0.8/yr vs Part B 2.0/yr) **違反 acceptance criteria**——Part A 從 11 訊號崩至 4 訊號（FLOOR -2% 切除 7 個 winners 發生於 broad market dip period XBI-XLV RS_20d < -2%，含 broad COVID 反彈、broad bear V-bounce 等），Part B 從 6 訊號→4 訊號（過濾 1 SL + 1 winner）。**結論**：FLOOR -2% 對 Part B 結構性正確（surgical filter biotech-specific 持續弱勢 SL）但對 Part A 過嚴（broad market correction 期間之 winners 同樣呈現 RS_20d < -2%，FLOOR 無法區分 broad correction recovery vs sub-sector persistent decline）。**核心發現（lesson #20 v3 family v3 邊界擴展）**: (1) **Repo 第 1 次 sub-sector ETF (XBI) vs sector parent ETF (XLV) 配對驗證**——既往 lesson #20 v3 配對為 country-EM (INDA/EWZ-EEM)、stock-index (NVDA/TSLA-QQQ)、commodity-commodity (TLT-SPY、COPX-GLD、USO-XLE)；sub-sector vs sector parent 為新配對類型；(2) **Repo 首次 cross-asset divergence FLOOR 極性反轉發現**——既有 lesson #20 v3 全部為 CAP（過濾 prior outperformance），XBI-018 Att3 為 repo 首次嘗試 FLOOR（過濾 prior underperformance）；對 Part B 結構性正確但 Part A 過度嚴格；(3) **跨資產 lesson #20 v3 family 極性選擇規則新假設**：CAP 適用「prior outperformance → rally exhaustion」失敗模式（個股、country ETF、商品配對）；**FLOOR 適用「prior underperformance → persistent weakness」失敗模式**（sub-sector ETF 對 broader sector parent，因 sub-sector 受獨立 idiosyncratic risk 驅動可能持續性背離）；(4) **XBI 結構性瓶頸**：Part B 1 SL 無法在不犧牲 Part A 多個 broad-correction recovery winners 的前提下 surgical filter——broad correction 期間 winners 與 sub-sector persistent decline SL 在 RS_20d 維度重疊；需正交維度（如 XBI 自身 momentum、XLV trend、healthcare sector breadth）方可區分。**XBI-017 Att1 仍為當前最佳**（XBI-018 Att3 為「結構性 NO LOSS 但 A/B gap 違反 acceptance criteria」記錄為 cross-asset learning，未取代 XBI-017）。**新跨資產假設（待驗證）**：(a) FLOOR 變體可能適用其他 sub-sector ETF（KIE 保險 vs XLF、IGV 軟體 vs XLK、KRE 區域銀行 vs XLF），閾值依 sub-sector 對 parent 的 idiosyncratic risk 強度調整；(b) sub-sector 配對 sector parent 的 RS divergence 可能需搭配「parent sector trend filter」（XLV 趨勢健康時才允許 XBI MR 進場）方可繞過 broad correction recovery 與 persistent decline 重疊問題。XBI 第 14 個策略類型試驗（後於突破、ROC、動量回調、配對 IBB、ATR 自適應、RSI(2)、BB-lower 混合、RSI hook divergence、capitulation-accel、Gap-Down、2DD floor、broad-market macro context、VIX BANDS、**XBI-XLV cross-asset divergence**）。XBI-017 Att1 仍為全域最優（18 次實驗、56+ 次嘗試）。
  last_experiment_attempt: XBI-020 (3 iterations, **DOCUMENTED-FAILURE — predict→confirm 完全命中**；repo 首次 lesson #24 family **DIRECTION** 變體應用於 XBI（XBI-017 為 BANDS 變體）；XBI-017 Att1 min(A,B) 0.64 仍為全域最優)
  note_2026_05_17_xbi020: XBI-020 added 2026-05-17 (^VIX Implied-Vol DIRECTION Regime Gate Pullback MR, **documented-failure；repo 首次 lesson #24 family DIRECTION 變體於 XBI（XBI-017 為 BANDS 變體）+ XBI 殘餘 binding Part B SL 加入「idiosyncratic non-separable single residual SL」family**, predict→confirm 預分析完全命中). base = XBI-017 Att1 全域最優（min(A,B) 0.64，binding=Part B Sharpe 0.64）+ 第 6 條件 ^VIX (lookback-day change) DIRECTION CEILING。**強制 predict→confirm 預分析（先做）決定性命中**：執行模型實測 XBI-017 Att1 全 11+6 筆——Part A = 10 × (+3.50% TP) + 1 × (2021-07-19 -0.04% near-flat expiry)、Part B = 5 × (+3.50% TP) + 1 × (**2025-03-31 -5.10% SL = 2025-04-02「Liberation Day」對等關稅外生宏觀衝擊**）；**兩 Part 所有 winners 皆相同 +3.50% TP**（TLT-014 Att3 / XLU-013 zero-variance trap 同構，唯一變異源 = Part A 單筆 near-flat expiry + Part B 單筆 SL）。殘餘 binding Part B SL **2025-03-31** vs 5 Part B winners 逐維度 separability：VIX level（SL 22.28 ∈ [14.40,24.09]，2 winners 更高）/ VIX 3d(SL +3.95，winner 2024-12-19 **+9.40** 遠高) / 5d(SL +4.80，winner +10.17 遠高) / 10d(SL +1.77，winner +10.55 遠高) / SPY20d(SL -3.89 vs winner 2024-04-25 -3.76 僅差 0.13pp knife-edge) / XBI20d(SL -5.25 ∈ [-13.37,+10.82]) **全部維度皆完全交錯、無 ≥15pp robust plateau** → 預測 documented-failure。結構性根因：Part B winner **2024-12-19**（2024-12 鷹派 Fed dot-plot 拋售，VIX 24/3dΔ +9.40/5dΔ +10.17）為「高 VIX broad panic → systematic V-bounce → XBI MR 獲利」教科書 XBI U 型 regime（XBI-017 核心發現），其 VIX-rising signature 遠較 SL 極端，任何 ^VIX DIRECTION CEILING 要移除 SL 必先誤殺 2024-12-19 winner。三次迭代全 CONFIRMED vs baseline min 0.64：Att1（3d CEILING ≤ +5.0，XLU-013 sweet spot 類比）Part A 9/100%/Sharpe **0.00** zero-var cum +36.29% / Part B 5/80.0%/**0.52** cum +8.90% / min **0.52**（-19%）FAIL — SL 2025-03-31 (3dΔ +3.95 ≤ +5.0) **通過 ceiling 不被移除（非綁定）**、反誤殺 winner 2024-12-19 (3dΔ +9.40) + Part A 變異源 2021-07-19 expiry（TSLA-018 Att2 / FXI-015 Att1 同構：SL 非綁定 + gate 非外科式殺 winner）；Att2（3d CEILING ≤ +3.0，強制移除 SL +3.95 > +3.0）Part A 7/100%/**0.00** zero-var cum +27.23% / Part B 4/100%/**0.00** zero-var cum +14.75% / min **≈0.00** catastrophic REJECT — SL 移除但 winner 2024-12-19 (+9.40) + Part A 熊市 V-bounce TPs 同殺 → **雙 zero-var**（† 慣例不適用雙退化，XBI-017 Att3 / XLU-012 Att1/Att2 / EWZ-010 Att2 同構）；Att3（5d CEILING ≤ +5.0，lookback ablation）Part A 8/100%/**0.00** zero-var cum +31.68% / Part B 5/80.0%/**0.52** cum +8.90% / min **0.52** FAIL — SL 2025-03-31 (5dΔ +4.80 ≤ +5.0) 同樣**通過 ceiling 不被移除**、winner 2024-12-19 (5dΔ +10.17) 同殺 → 確認無可分 ^VIX DIRECTION lookback（3d/5d 皆 SL 非綁定）。**核心跨資產規則（lesson #24 family DIRECTION 變體邊界 + 「idiosyncratic non-separable single residual SL」family 擴展）**：(1) **repo 首次 lesson #24 family DIRECTION 變體於 XBI**（XBI-017 已驗證 BANDS 變體成功；DIRECTION 變體於 FDA 事件驅動生技板塊 ETF 失敗）——與 XBI-016（lesson #25 macro-confirm gate FAIL）**同根因**：XBI 生技 SLs 多發於 broad-correction 開端、winners 多發於 broad-panic V-bounce → 結構鏡像反向（vs IWM），forward-looking implied-vol DIRECTION 與 backward-looking macro-confirm 皆無區分力；(2) XBI 殘餘 binding Part B SL 2025-03-31（外生關稅宏觀衝擊）加入 EWJ-006/EEM-016/INDA-013/TSM-012/NVDA-018/DIA-013/FXI-015/EWZ-010「idiosyncratic non-separable single residual SL」family；(3) **zero-variance trap 同構（TLT-014 Att3 / XLU-013）**：Part B = 5 相同 +3.50% TPs + 1 SL，移除 SL 必致 Part B zero-var；任何 DIRECTION CEILING 非外科式（必同殺 broad-panic winner 2024-12-19）→ 雙 zero-var min 0.00。predict→confirm 軌跡 3 SUCCESS（GLD-016/VOO-005/URA-014）+ **15 documented-failure 全部命中**（XBI-020 為第 15）。XBI-017 Att1 仍為 XBI 全域最優（18 次實驗、56+ 次嘗試）。
...
**當前最佳：** ★ **XBI-019 Att3**（XLV Sector Parent Trend Filter on VIX Bands MR：XBI-017 Att1 完整框架 + **regime-conditional XLV panic gate**——低 VIX 帶 (<= 17) 沿用 BANDS（無 XLV 條件），高 VIX 帶 (> 22) 加 **XLV 10d return <= -2.0%** panic V-bounce 預備條件，TP+3.5%/SL-5.0%/15d/cd 10）★ **2026-05-10 新全域最優（19 次實驗、59+ 次嘗試）**
- Part A: 9 訊號 / WR **100%** / std=0 zero-var / cum +36.29% / MDD -4.62%
- Part B: 4 訊號 / WR **100%** / std=0 zero-var / cum +14.75% / MDD -2.07%
- min(A,B)†: **STRUCTURAL NO LOSS 雙 Part 全勝零方差**（沿用 IBIT-009/EWJ-005 Att2/EWT-008/SPY-009/IWM-013 慣例 †）
- A/B 年化 cum 6.39%/yr vs 7.12%/yr → gap **10.3% << 30% ✓✓**（vs XBI-017 13.2%，進一步改善）
- A/B 年化訊號比 1.8/yr vs 2.0/yr → gap **10% << 50% ✓✓**（vs XBI-017 26.7%，大幅改善）
- 關鍵改善：regime-conditional XLV panic gate 過濾**全部殘餘 1 Part A SL + 全部殘餘 1 Part B SL（2025-03-31 Trump healthcare tariff 早期下跌）**，雙 Part WR 提升至 100%
- **Repo 首次「sector parent absolute momentum direction filter」於任何資產**：既往 cross-asset filter 為相對強度（lesson #20 v3）或 forward-looking implied vol（lesson #24 family）維度；XBI-019 為 sector parent 自身 absolute momentum direction 維度，與兩者結構性正交
- **Repo 首次「regime-conditional cross-asset gate」**：高 VIX + XLV 10d <= -2.0% = 真實廣 panic V-bounce 預備（filter 通過）；高 VIX + XLV 10d > -2.0% = 早期 sector-specific 下跌（filter 阻擋，如 2025-03-31）；低 VIX 不需 XLV 條件（biotech isolated dip）
- **核心發現**：XBI 結構性瓶頸（XBI-017 Part B 2025-03-31 SL）首次完全突破——關鍵在於「regime-conditional 邏輯」繞過 broad correction recovery 與 sub-sector persistent decline 的單維度重疊問題
- **新跨資產假設**：regime-conditional cross-asset gate 可能適用其他 sub-sector ETFs（KRE/IGV/SOXX + 對應 sector parent panic threshold），為 lesson #20 v3 / lesson #24 family 通用新變體

**前任最佳：** XBI-017 Att1（VIX Implied-Vol Regime Bands Filter Pullback MR：XBI-015 Att2 完整框架 + **^VIX <= 17.0 OR > 22.0** BANDS 過濾，TP+3.5%/SL-5.0%/15d/cd 10）
- Part A: 11 訊號 / WR 90.9% / Sharpe 3.12 / cum +41.00% / MDD -4.62%
- Part B: 6 訊號 / WR 83.3% / Sharpe **0.64** / cum +12.71%
- min(A,B) **0.64**（已被 XBI-019 Att3 透過 regime-conditional XLV panic gate 進一步精煉至 STRUCTURAL NO LOSS）
- **Repo 第 1 次 lesson #24 family BANDS 變體**：既往為 LEVEL CAP / DIRECTION，XBI-017 試驗 BANDS 變體驗證 U-shape regime hypothesis

**前前任最佳：** XBI-015 Att2（Multi-Week Regime-Aware Pullback MR：XBI-005 + ATR(20)/ATR(60) ≤ 1.10 vol stability gate，min(A,B) 0.46）— repo 第 1 次 lesson #22 跨策略類型移植（BB Squeeze / MBPC → MR）

**XBI-018（lesson #20 v3 cross-asset divergence FLOOR 變體首次試驗，未取代 XBI-017）**: 18 次實驗、56+ 次嘗試。三次迭代測試 XBI-XLV cross-asset divergence regime gate：
- Att1（20d cap +3% NVDA-021 港）: min(A,B) **0.52** REJECT — cap 過緊移除 winners 但 1 Part B SL 殘留
- Att2（60d cap +5% INDA-012 港）: min(A,B) **0.64** TIE — Part B 完全非綁定，**確認 CAP 極性對 XBI 結構性錯誤方向**
- Att3（**20d FLOOR -2%**, repo 首次 cross-asset divergence FLOOR 變體）: Part A 4/100%/std=0/+14.75%, Part B 4/100%/std=0/+14.75%, **STRUCTURAL NO LOSS** ✓ but A/B annualized cum gap **60.9% > 30% ❌** + signal gap **60% > 50% ❌** 違反 acceptance criteria — Part A 從 11→4 訊號（FLOOR -2% 切除 broad market dip period winners 含 COVID 反彈/bear V-bounce），Part B 從 6→4（過濾 1 SL + 1 winner）
- **核心發現（lesson #20 v3 family v3 邊界擴展）**：
  - **Repo 第 1 次 sub-sector ETF (XBI) vs sector parent ETF (XLV) 配對驗證**——既往 lesson #20 v3 為 country-EM、stock-index、commodity-commodity 配對；sub-sector 為新類型
  - **Repo 首次 cross-asset divergence FLOOR 極性反轉發現**——CAP 適用「prior outperformance → rally exhaustion」（個股、country ETF），FLOOR 適用「prior underperformance → persistent weakness」（sub-sector ETF 對 broader sector parent，因 sub-sector 受獨立 idiosyncratic risk 驅動可能持續性背離）
  - **XBI 結構性瓶頸**：broad correction recovery winners 與 sub-sector persistent decline SL 在 RS_20d 維度重疊；需正交維度（XBI 自身 momentum、XLV trend、healthcare sector breadth）方可區分
- **新跨資產假設（待驗證）**：FLOOR 變體可能適用其他 sub-sector ETF（KIE 保險 vs XLF、IGV 軟體 vs XLK、KRE 區域銀行 vs XLF），閾值依 sub-sector idiosyncratic risk 強度調整；或需搭配 parent sector trend filter（XLV 趨勢健康時才允許 XBI MR 進場）方可繞過 broad correction overlap 問題


**最新嘗試（documented-failure，未撼動）：** XBI-020（^VIX DIRECTION CEILING，repo 首次 lesson #24 family DIRECTION 變體於 XBI；XBI-017 為 BANDS 變體）— predict→confirm 預分析完全命中。binding=Part B 殘餘單筆 SL **2025-03-31**（2025-04 對等關稅外生衝擊），與 5 Part B winners（皆 +3.50% TP）在 VIX level/3d/5d/10d、SPY20d、XBI20d **全維度交錯無 ≥15pp robust plateau**；broad-panic V-bounce winner 2024-12-19（3dΔ +9.40）VIX-rising 遠較 SL（3dΔ +3.95）極端 → 任何 CEILING 移除 SL 必同殺該 winner。Att1（3d ≤+5.0）min **0.52**（SL 非綁定，winner 同殺）/ Att2（3d ≤+3.0）**雙 zero-var min ≈0.00 REJECT** / Att3（5d ≤+5.0）min **0.52**。**zero-variance trap 同構（TLT-014 Att3 / XLU-013）**+ 殘餘 SL 加入「idiosyncratic non-separable single residual SL」family；與 XBI-016 lesson #25 失敗同根因（XBI 生技 SLs 多發於 broad-correction 開端、winners 多發於 broad-panic V-bounce，鏡像反向 vs IWM）
**前前任最佳：** XBI-005（回檔 8-20% + WR(10) ≤ -80 + ClosePos ≥ 35%，Part A Sharpe 0.36，Part B Sharpe 0.64）— 14 次實驗、44+ 次嘗試已確認為基線
**前前前任最佳：** XBI-001（同進場無 ClosePos，Part A Sharpe 0.11，Part B Sharpe 0.23）
...
- **Macro-Confirmed Pullback MR QQQ 10d gate -1.5%（XBI-016 Att1，IWM-015 直接 port）**：Part A 8/75.0%/Sharpe 0.36 / Part B 3/**33.3%**/Sharpe **-0.55** / min **-0.55**（崩壞 -220% vs baseline 0.46），QQQ gate 對 XBI 殘餘 SLs 過濾力弱（2/3 SLs 仍通過 QQQ 同步深度修正期），副作用嚴重移除 Part B biotech-specific 反彈 winners（6→1）並引入 cooldown shift 新 SL
- **Macro-Confirmed Pullback MR SPY 10d gate -3.0%（XBI-016 Att2）**：Part A 5/100% WR zero-var/cum +18.77% / Part B 2/100% WR zero-var/cum +7.12% / 雙 zero-var 但 over-filter（signal count 21→7，-67%）、A/B 訊號比 60% > 50% ✗、A/B cum 差 62% > 30% ✗，統計顯著性嚴重不足
- **Macro-Confirmed Pullback MR SPY 10d gate -1.5%（XBI-016 Att3）**：Part A 9/66.7%/Sharpe **0.34** / Part B 2/100% zero-var / min **0.34** (-26% vs baseline 0.46)，SPY -1.5% 放寬後 Part A 3 SLs 重新出現含 2022-04-19/2023-09-21 原殘餘 SL，三次迭代確認 lesson #25 broad-market macro context confirmation gate 對 XBI biotech sub-segment ETF **結構性失敗**（XBI vs IWM 結構鏡像反向）
...
| XBI-017 | `xbi_017_vix_bands_mr` | VIX Implied-Vol Regime Bands Filter Pullback MR（XBI-015 Att2 + ^VIX <= 17.0 OR > 22.0 BANDS 過濾，**Att1 SUCCESS**，repo 第 1 次 lesson #24 family BANDS 變體） | 前任最佳（Att1 SUCCESS，min(A,B) **0.64**，+39% vs XBI-015 Att2 0.46） |
| XBI-018 | `xbi_018_xbi_xlv_divergence_mr` | XBI-XLV Cross-Asset Divergence Filter on VIX Bands MR（XBI-017 + RS divergence cap/floor）+ 3 次嘗試（Att3 STRUCTURAL NO LOSS 但 A/B gap 違反 acceptance） | 已完成（未取代 XBI-017，repo 首次 cross-asset divergence FLOOR 變體 + sub-sector 配對 sector parent） |
| XBI-019 | `xbi_019_xlv_trend_mr` | XLV Sector Parent Trend Filter on VIX Bands MR（XBI-017 + regime-conditional XLV 10d <= -2.0% panic gate 於高 VIX 帶內，**Att3 SUCCESS**，repo 首次 sector parent absolute momentum direction filter + repo 首次 regime-conditional cross-asset gate） | ✅ **當前最佳**（Att3 SUCCESS，STRUCTURAL NO LOSS 雙 Part 全勝零方差，min(A,B)† 結構最優，A/B gap 10.3%/10%） |
| XBI-020 | `xbi_020_vix_direction_mr` | ^VIX Implied-Vol DIRECTION Regime Gate Pullback MR（XBI-017 Att1 + ^VIX 3d/5d change CEILING，repo 首次 lesson #24 family DIRECTION 變體於 XBI） | ❌ **documented-failure**（predict→confirm 完全命中：Att1 min 0.52 / Att2 雙 zero-var ≈0.00 REJECT / Att3 min 0.52；XBI-017 Att1 0.64 未撼動；殘餘 SL idiosyncratic non-separable + zero-var trap） |
...
## XBI-017: VIX Implied-Vol Regime Bands Filter Pullback MR ★ 當前最佳（Att1 SUCCESS）

### 目標 (Goal)

在 XBI-015 Att2（前任最佳，min(A,B) 0.46）基礎上，疊加 **^VIX forward-looking
implied-vol BANDS regime gate**，過濾 XBI-015 Att2 殘餘 3 個 Part A SLs
（2021-05-06 / 2022-04-19 / 2023-09-21，均處於中等 VIX 帶 17.5-21.4），首次驗證
**lesson #24 family BANDS 變體**（既往為 LEVEL CAP 或 DIRECTION 維度）。

### 進場條件 (Entry Conditions)

| 條件 | 指標 | 閾值 | 說明 |
|------|------|------|------|
| 1 | 10 日高點回檔 | ∈ [-8%, -20%] | 同 XBI-015 Att2 |
| 2 | Williams %R(10) | ≤ -80 | 同 XBI-015 Att2 |
| 3 | ClosePos | ≥ 35% | 同 XBI-015 Att2 |
| 4 | ATR(20) / ATR(60) | ≤ 1.10 | 同 XBI-015 Att2 vol stability gate |
| 5 | **^VIX 收盤** | **≤ 17.0 OR > 22.0** | XBI-017 BANDS gate（排除中等 VIX 帶） |
| 6 | 冷卻期 | 10 天 | 同 XBI-015 Att2 |

### 出場參數 (Exit Parameters)

| 參數 | 值 | 說明 |
|------|------|------|
| 獲利目標 (TP) | +3.5% | 同 XBI-015 Att2 |
| 停損 (SL) | -5.0% | 同 XBI-015 Att2 |
| 持倉天數 | 15 | 同 XBI-015 Att2 |
| 滑價 | 0.1% | 高流動性板塊 ETF |

### 設計理念 (Design Rationale)

XBI-015 Att2 殘餘 3 筆 Part A SLs 經實測呈現 **U 型 VIX level 分布**：
- 2021-05-06 (SL): VIX=18.4，SPY 20d +2.58%（broad 健康，XBI 落後 -7.77%）
- 2022-04-19 (SL): VIX=21.4，SPY 20d +0.15%（broad 平盤）
- 2023-09-21 (SL): VIX=17.5，SPY 20d -2.28%（broad 輕度修正）

全部 3 筆 SLs 落於 VIX ∈ [17.5, 21.4] 中等 VIX 帶。對照 XBI-015 Att2 winners
從各 regime 抽樣（VIX 14.4 / 15.6 / 24.1 / 23.5 / 35.3 / 31.8）— 大多落於兩端
極端 regime（< 17 calm 或 > 22 panic）。

**核心假說（U-shape regime hypothesis）**：

XBI capitulation MR 在 broad market 兩個極端 regime 才結構性有效：
- **(a) 低 VIX（< 17）**：broad market calm，XBI 因 biotech 特定事件（FDA / 臨床
  數據）isolated dip → 板塊內快速回補，MR 成功
- **(b) 高 VIX（> 22）**：broad panic capitulation → systematic V-bounce 帶動
  XBI 隨 risk asset 反彈，MR 成功

中等 VIX 帶（17 < VIX <= 22）為「complacency creep」regime：broad market 表面
健康但內部分化（small caps / sector rotation / 政策不確定），XBI biotech 板塊性
stress 無 broad capitulation 助力 → MR 失效。

### 與其他 XBI 試驗的區分

- **XBI-015 Att2**：ATR(20)/ATR(60) ≤ 1.10（XBI 自身 backward-looking realized
  vol gate），過濾 vol expansion 期 SLs；XBI-017 額外加 ^VIX BANDS（broad
  market forward-looking implied vol regime classifier），二者**正交**
- **XBI-016**：QQQ/SPY broad-market macro confirmation gate（require broad
  market 同步弱勢，IWM-015 pattern port）→ FAILED min(A,B) -0.55；XBI-017 BANDS
  方向**正交**——XBI-016 為單向「broad 須同步弱」，XBI-017 為雙向「broad 須
  在極端 regime（不論方向）」
- **lesson #5 邊界**：XBI-017 使用「broader market regime classifier」（implied
  vol bands），非 XBI 自身趨勢過濾——延續 TLT-014 lesson #5 邊界精煉，
  cross-asset regime gate 為合法 MR 過濾維度

### 迭代嘗試紀錄 (Iteration Log)

#### Att1 ★（vix_low=17.0, vix_high=22.0）— SUCCESS

對齊 3 SLs VIX 分布 [17.5, 21.4] 包覆區。
...
**A/B 平衡**：
- 年化 cum: Part A 7.10%/yr vs Part B 6.16%/yr（gap **13.2% < 30% ✓**）
- 年化訊號比: 2.2/yr vs 3.0/yr = 1.0:1.36（gap 26.7% < 50% ✓）

**過濾效果**：
- 全部 3 個 XBI-015 Att2 殘餘 Part A SLs 過濾（2021-05-06 / 2022-04-19 / 2023-09-21）
- 1 筆中性 winner 過濾（XBI-015 Part A 15→11 訊號，淨 -1 SL -3 winner）
- Part B 6/6 完全不變，VIX bands 對 Part B 訊號日 VIX 分布全部落於極端 regime

#### Att2（vix_low=18.0, vix_high=21.0 加嚴 1pt）— FAILED

**結果**：
- Part A: 13 訊號 / WR 84.6% / Sharpe **0.62** / cum +27.40%
- Part B:  6 訊號 / WR 83.3% / Sharpe 0.64 / cum +12.71%（同 Att1）
- min(A,B) **0.62**（-3% vs Att1 0.64）

**失敗分析**：
- 加嚴 1pt 反向放回 2 訊號（11→13）：1 winner + 1 loser
- 淨 Sharpe 退步，A/B cum 差 19.5%（仍 < 30% ✓）但 Part A WR 從 90.9% 降至 84.6%

#### Att3（vix_low=16.0, vix_high=23.0 放寬 1pt）— FAILED（雙 zero-var）

**結果**：
- Part A:  9 訊號 / WR **100%** / Sharpe 0.00 zero-var / cum +36.29% / MDD -4.62%
- Part B:  5 訊號 / WR **100%** / Sharpe 0.00 zero-var / cum +18.77%
- min(A,B) 退化為 **0.00**（雙 Part 零方差結構）

**失敗分析**：
- 兩端各放寬 1pt 過濾掉 2 winners + 1 loser，純化但 Part A/B 雙 zero-var
- A/B 年化 cum 6.40%/yr vs 8.99%/yr（gap 28.8% < 30% ✓）但雙 zero-var 退化
- † 慣例適用單 Part 場景（EWJ-003/SPY-009/DIA-012/IWM-013），雙 Part 退化
  結構不適用統計顯著性標準

### 跨資產貢獻（cross-asset findings）

1. **Repo 第 1 次 lesson #24 family BANDS 變體**：既往 lesson #24 跨資產驗證皆為
   LEVEL CAP（TLT-013 ^MOVE <= 130）或 DIRECTION（XLU-013 ^MOVE 3d、USO-025
   ^OVX 3d、GLD-015 ^GVZ 10d），XBI-017 試驗 BANDS 變體驗證 U-shape regime
   hypothesis
2. **U-shape regime hypothesis 驗證成立**：XBI capitulation MR 在 broad market
   兩個極端 regime（低 VIX biotech isolated dip / 高 VIX broad panic V-bounce）
   才結構性有效，中等 VIX 為 complacency creep regime XBI MR 失效
3. **(17, 22) 為 XBI 結構性甜蜜點**：對齊殘餘 3 SLs VIX 分布；加嚴或放寬 1pt
   皆退步（加嚴反向放回 winners、放寬過濾 winners 過多致雙 zero-var）
4. **lesson #24 family v5 維度三類**：
   - LEVEL CAP（TLT-013 ^MOVE <= 130）：filter 持續高 vol regime
   - DIRECTION（XLU-013 / USO-025 / GLD-015）：filter vol regime trajectory
   - **BANDS（XBI-017）**：filter 中等 vol「complacency creep」regime（**新**）

### 待驗證的跨資產假設

VIX BANDS 變體可能適用於其他「structural U-shape regime」資產：
- Sector ETFs（XLF 金融、XLE 能源、XLK 科技、XLV 健康照護）
