...
**當前最佳：** XBI-005（回檔 8-20% + WR(10) ≤ -80 + ClosePos ≥ 35%，Part A Sharpe 0.36，Part B Sharpe 0.64）— **已確認為全域最優**（8 次實驗、26+ 次嘗試，含突破策略、ROC 策略、動量回調策略、配對交易策略獨立驗證）
...
**尚未嘗試的方向（預期效益極低）：**
- XBI-005 已確認為全域最優，突破策略、ROC 策略、動量回調策略、配對交易策略均已驗證無效
- 所有主要策略類型已窮盡（均值回歸、突破、動量回調、ROC、配對交易）
