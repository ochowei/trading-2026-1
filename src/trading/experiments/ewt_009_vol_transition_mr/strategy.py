"""
EWT-009: Post-Capitulation Vol-Transition MR Strategy

延伸 EWT-008 Att1 框架，新增「Capitulation strength filter」（1日/2日報酬下限）。
...
from trading.experiments.ewt_009_vol_transition_mr.signal_detector import (
    EWT009SignalDetector,
)
...
class EWT009Strategy(ExecutionModelStrategy):
    ...
    def create_detector(self) -> BaseSignalDetector:
        return EWT009SignalDetector(create_default_config())

    def _print_strategy_params(self, config: ExperimentConfig) -> None:
        if isinstance(config, EWT009Config):
