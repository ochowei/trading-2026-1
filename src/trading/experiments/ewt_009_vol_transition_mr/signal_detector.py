"""
EWT-009 Post-Capitulation Vol-Transition MR 訊號偵測器

在 EWT-008 Att1 框架（BB 下軌 + 回檔上限 + WR + ClosePos + ATR）之上新增
...
from trading.experiments.ewt_009_vol_transition_mr.config import EWT009Config

logger = logging.getLogger(__name__)


class EWT009SignalDetector(BaseSignalDetector):
    """
    EWT Post-Capitulation Vol-Transition MR 訊號偵測器
    ...
    def __init__(self, config: EWT009Config):
        self.config = config
    ...
    def detect_signals(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        if suppressed:
            ...
            logger.info("EWT-009: %d signals suppressed by cooldown", len(suppressed))

        signal_count = df["Signal"].sum()
        logger.info(
            "EWT-009: Detected %d signals (mode=%s, threshold=%.4f)",
            signal_count,
