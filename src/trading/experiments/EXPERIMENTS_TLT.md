...
| TLT-013 | `tlt_013_move_implied_vol_mr`    | ^MOVE forward-looking implied vol regime 閘門 MR（**repo 首次 ^MOVE 過濾於任何資產**，Att1 SUCCESS）| 已完成（被 TLT-014 超越）|
| TLT-014 | `tlt_014_tlt_spy_divergence_mr`  | TLT-SPY cross-asset divergence regime 閘門 MR（**repo 首次 cross-asset divergence regime gate using equity benchmark**，Att3 SUCCESS）| ✅ **當前最佳**（min(A,B)† 0.14→0.69，+393% Sharpe）|
| TLT-015 | `tlt_015_hyg_credit_divergence_mr` | HYG credit cross-asset divergence regime 閘門 MR（**repo 首次 HYG 高收益債作為跨資產 divergence 過濾器於任何資產 + 第二 cross-asset anchor 疊加於 TLT-SPY**，3 次迭代均 FAIL/TIE）| 已完成（未改善，揭示 cross-asset divergence stacking 結構性冗餘）|
| TLT-016 | `tlt_016_move_multi_window_direction_mr` | ^MOVE multi-window IV direction regime 閘門 MR（**cross-strategy port from USO-028 Att1，repo 首次 multi-window IV DIRECTION combo 於 TLT**，Att1 PARTIAL A/B 平衡達標 / Att2-3 REJECT）| 已完成（未超越 TLT-014 Att3 0.69，Att1 A/B 累計平衡 37%→14.7% 為新發現）|
