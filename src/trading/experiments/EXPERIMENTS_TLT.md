...
- 出場參數：TP +2.0~3.0% / SL -3.5~4.0% / 持倉 20 天
- Trailing stop：+1.5% 啟動 / 1.0% 距離（失敗）、+2.0% 啟動 / 1.5% 距離（失敗）

**尚未嘗試的方向（可探索）：**
- 非對稱出場（SL -5.0%）給予更多呼吸空間
- 利率相關指標（如 10Y yield 變化率）作為過濾器（需外部資料源）
...
- **TLT 的核心限制**：2022 升息產生大量無法被技術面過濾器區分的假訊號，深度回檔/TP調整/中期跌幅過濾均無法有效改善 Part A
<!-- AI_CONTEXT_END -->
