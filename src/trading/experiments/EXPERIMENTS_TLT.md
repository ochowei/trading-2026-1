...
**當前最佳：** ★ **TLT-014 Att3（2026-05-02 新全域最優）**（TLT-013 Att1 完整框架 + **TLT 20d 報酬 - SPY 20d 報酬 >= -4% cross-asset divergence regime gate**，TP +2.5% / SL -3.5% / 20 天）— **+393% Sharpe vs TLT-013 Att1，repo TLT 結構性最大幅度突破**
- Part A: 5 訊號 / WR **80.0%** / Sharpe **0.69** / 累計 +6.56% (vs TLT-013 Att1 11/54.5%/0.14/+3.53%，**+393% Sharpe**)
- Part B: 4 訊號 / WR 100% / Sharpe 0.00 (std=0 zero variance) / 累計 +10.38%
- min(A,B)† **0.69**（Part A binding，Part B zero-var 沿用 EWJ-003/SPY-009/DIA-012/IWM-013 † 慣例；+393% vs TLT-013 Att1 的 0.14）
- A/B 累計差 |6.56-10.38|/10.38 = **37%**（從 baseline 61% → Att1 49% → Att3 37%，每次迭代均改善但未達 30% 目標，Part B 期間 rate cycle 反轉效果天然優於 Part A 為結構性差異）
- A/B 訊號比 5:4 = **20% gap < 50% ✓**
- **Repo 首次驗證**：cross-asset divergence regime gate using equity benchmark——前次跨資產過濾嘗試 (TLT-008 IEF pair 同 duration、TLT-009 ^TNX velocity same-driver) 皆於同類資產失敗；TLT-014 證明使用**結構性對立資產（equity benchmark）**作為 cross-asset regime classifier 可有效捕捉 reflation/risk-on regime 並過濾 TLT MR 結構性失效訊號
- threshold -0.04 為 20d lookback 甜蜜點：-0.05（Att1）過鬆殘留 cooldown shift SL；-0.03（Att2）過嚴反向移除過多正常 winners
- 三層 regime gate 完全正交架構：TLT-007 BB-width（自身 backward-looking realized vol）+ TLT-013 ^MOVE（bond options forward-looking implied vol）+ TLT-014 TLT-SPY divergence（cross-asset performance differential regime）

**最新實驗：** TLT-016（^MOVE Multi-Window IV Direction Regime-Gated MR — cross-strategy port from USO-028 Att1，三次迭代 Att1 PARTIAL：Part A 不變但 A/B cum gap 37%→14.7% ✓ A/B 平衡首次達標；Att2 surgical cap -1.7 樣本崩壞 REJECT；Att3 multi-window combo 5d+3d Sharpe 0.61 REJECT）。**前次：** TLT-015 (HYG Credit Cross-Asset Divergence MR，三次嘗試 Att1 TIE / Att2-3 REJECT)

**前任最佳：** TLT-013 Att1（TLT-007 Att2 完整框架 + ^MOVE Close <= 130 forward-looking implied volatility regime gate，Part A Sharpe **0.14** / Part B Sharpe **0.65** / min(A,B) **0.14**）★ **repo 首次 forward-looking implied vol derivative regime gate**（已被 TLT-014 Att3 超越）
