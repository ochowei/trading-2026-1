"""EWT–EEM Cross-Asset Divergence Regime-Gated MR 訊號偵測器 (EWT-010)
...
from trading.experiments.ewt_010_eem_divergence_regime_mr.config import EWT010Config

logger = logging.getLogger(__name__)


class EWT010SignalDetector(BaseSignalDetector):
    """EWT-010：EWT-009 Att3 框架 + EWT–EEM 跨資產 divergence regime gate"""
    ...
    def compute_indicators(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        # ===== EWT–EEM 跨資產 divergence regime gate（EWT-010 核心新增）=====
    ...
    def detect_signals(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        logger.info(
            "EWT-010: Detected %d signals (divergence_gate=%s, mode=%s, %dd, thr=%.4f)",
            signal_count,
