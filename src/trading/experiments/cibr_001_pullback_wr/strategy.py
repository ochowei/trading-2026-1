"""
CIBR 回檔 + Williams %R 均值回歸策略 (CIBR Pullback + Williams %R Mean Reversion Strategy)
...
Exit uses fixed TP/SL (no trailing stop - daily vol 1.53% is at boundary).
"""
...
from trading.experiments.cibr_001_pullback_wr.signal_detector import (
    CIBRPullbackWRSignalDetector,
)
...
class CIBRPullbackWRStrategy(ExecutionModelStrategy):
    ...
    def create_detector(self) -> BaseSignalDetector:
        return CIBRPullbackWRSignalDetector(create_default_config())

    def _print_strategy_params(self, config: ExperimentConfig) -> None:
        if isinstance(config, CIBRPullbackWRConfig):
            print(
