"""
TSLA-020 訊號偵測器：TSLA-USD(UUP) Direction Regime-Gated BB Squeeze Breakout
...
6. **USD regime gate（TSLA-020 核心新增）**：
...
class TSLA020USDRegimeBreakoutDetector(BaseSignalDetector):
    ...
    def detect_signals(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        if suppressed:
            ...
            logger.info("TSLA-020: %d signals suppressed by cooldown", len(suppressed))

        signal_count = df["Signal"].sum()
        logger.info("TSLA-020: Detected %d USD-regime-gated breakout signals", signal_count)
        return df
