...
| TSM-020 | `tsm_020_soxx_divergence_rs`    | TSM-SOXX 20d Cross-Asset Divergence CEILING Regime-Gated RS Momentum Pullback（**repo 首次「sector-internal anchor」變體於任何資產**，3 次迭代全部 REJECT/TIE，min(A,B) 最佳 0.83 TIE / 失敗根因 Part B SLs 在 TSM-SOXX 維度落於 winners 分布中段） | ❌ 失敗 |
| TSM-021 | `tsm_021_qqq_macro_health_gate` | QQQ Macro-Health Gate on RS Momentum Pullback（**repo 第 2 次 lesson #25 broad-market context confirmation gate 跨資產驗證 + 首次跨策略框架延伸 (capitulation MR → RS Momentum) + TSM 結構性 0.83 ceiling 首次突破**：QQQ 10d <= +2% CEILING，Att2 SUCCESS min(A,B) **1.14 (+37% vs 0.83)**, A/B 平衡達標） | **★ 最佳** |
