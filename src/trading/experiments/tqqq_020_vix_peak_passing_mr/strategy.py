"""TQQQ-020 ^VIX Peak-Passing Filter on Vol-Regime-Gated Capitulation Buy 策略"""
...
from trading.experiments.tqqq_020_vix_peak_passing_mr.config import (
    TQQQ020Config,
    create_default_config,
)
from trading.experiments.tqqq_020_vix_peak_passing_mr.signal_detector import (
    TQQQ020SignalDetector,
)


class TQQQ020VixPeakPassingStrategy(AuxiliaryBundleStrategyMixin, ExecutionModelStrategy):
    bundle_trial_family = "TQQQ:vix-peak-passing"
    bundle_trial_hypothesis = (
        "TQQQ capitulation entries improve when ^VIX has passed its daily peak."
    )

    """TQQQ-020：TQQQ-018 框架 + ^VIX peak-passing filter（含成交模型）
    ...
    def _print_strategy_params(self, config: ExperimentConfig) -> None:
        ...
        print(
            f"  ^VIX peak-passing (1d change):   {config.vix_ticker} "
            f"1d change <= {config.max_vix_1d_change:+.2f}"
        )
