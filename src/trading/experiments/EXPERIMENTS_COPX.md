<!-- AI_CONTEXT_START - 此區塊供 AI Agent 快速讀取，人工更新
  last_validated: 2026-03-30
  gradient_validated: 2026-03-30
  data_through: 2025-12-31
-->
## AI Agent 快速索引

**當前最佳：** COPX-003（20日回檔 10-20% + WR(10) ≤ -80 + SL -4.5% / 20天持倉，Part A Sharpe 0.39，Part B Sharpe 0.35）
**前最佳：** COPX-002（同進場，SL -5.0%，Part A Sharpe 0.34，Part B Sharpe 0.30）
**滾動窗口分析摘要：** COPX-001 ✓✓ 雙漸變（ΔWR max 9.2pp，10/12 正報酬窗口）；COPX-002/003 待分析

**已證明無效（禁止重複嘗試）：**
- 回檔 ≥ 8% 搭配 WR-80（Part A WR 59.5% = 盈虧平衡線，Sharpe 0.00，累計 -2.99%）
- 收盤位置 ≥ 40% 反轉K線過濾（ClosePos + 收窄回檔 + 緊SL → 雙部分皆負，已實測確認無效）
- RSI(2) < 10 + 2日跌幅 ≥ 4%（產生過多低品質訊號，Part A -44%，完全不適合 COPX）
- 60日回撤 + RSI(10) + SMA乖離 三重極端超賣架構（FCX-001 風格，Part A Sharpe -0.21，Part B -0.18，WR 50%/50%，TP +5%/SL -7% 盈虧比過差）
- 20日回檔 11%（vs 10%）：Part A 下降至 0.26，Part B 升至 0.85 但訊號僅 8 筆，A/B 訊號比 3.5:1（過度不平衡）
- TP +4.0%（COPX-003 Att1）：Part A 2筆達標→到期/停損，Sharpe 0.34→0.25，Part B 改善至 0.34 但 Part A 退化更嚴重
- SL -4.0%（COPX-003 Att3）：太緊，Part B WR 從 72.7% 崩至 54.5%（2筆好交易被提前停損），Sharpe 0.01

**已掃描的參數空間：**
- 進場條件：回檔 8~11% + 上限 16~20% + 回看 10~20日 + WR(10) ≤ -80~-85 + RSI(2) < 10~15 + RSI(10) < 30 + SMA(50) 乖離 -6% + 2日跌幅 3~4% + ClosePos ≥ 40% + 60日回撤 15-30%
- 出場參數：TP +3.5~5.0% / SL -4.0~-7.0% / 持倉 15~20 天
- 最佳組合：20日回檔 10-20% + WR-80 + TP +3.5% / SL -4.5% / 20天（WR 74.2%/72.7%，Sharpe 0.39/0.35）

**尚未嘗試的方向（可探索）：**
- 銅價或工業金屬指數作為宏觀過濾器

**關鍵資產特性：**
- COPX (Global X Copper Miners ETF) 日波動約 2.25%，GLD 比率 1.87x
- 銅礦 ETF，受銅價、中國需求、全球工業景氣驅動
- 均值回歸效果穩健，COPX-003 A/B Sharpe 比 1.11x（近乎完美平衡）
- A/B 訊號率比 1.13:1（COPX-003，優秀），WR 差距僅 1.5pp
- 20日回看窗口是關鍵改進（vs 10日），提供更高的參考高點使回檔訊號更可靠
- SL -4.5% 是甜蜜點：-5.0% 每筆虧損多 0.5% 浪費，-4.0% 太緊翻轉 Part B 好交易
<!-- AI_CONTEXT_END -->

# COPX 實驗總覽 (COPX Experiments Overview)

## 標的特性 (Asset Characteristics)

- **COPX (Global X Copper Miners ETF)**：追蹤 Solactive Global Copper Miners Total Return Index，持有全球銅礦公司
- 日均波動約 2.25%，GLD 波動比率 1.87x，屬中等波動度
- 銅礦板塊受銅價週期、中國基建需求、全球工業景氣、能源轉型（電動車/再生能源）驅動
- 2020 年 COVID 崩盤跌至 ~$8.5，2022 年銅價回落造成多筆停損
- 數據起始：2010-04-20（ETF 成立日）

## 實驗列表 (Experiment List)

| ID       | 資料夾                    | 策略摘要                              | 狀態  |
|----------|--------------------------|--------------------------------------|-------|
| COPX-001 | `copx_001_pullback_wr`    | 回檔範圍 9-18% + Williams %R 均值回歸 | 已完成 |
| COPX-002 | `copx_002_deep_drawdown`  | 20日回檔 10-20% + Williams %R + 20天持倉 | 已完成 |
| COPX-003 | `copx_003_exit_optimized` | 同 COPX-002 進場 + SL -4.5% 出場優化     | 已完成 ✓ 當前最佳 |

---

## COPX-001: 回檔 + Williams %R 均值回歸 (Pullback + Williams %R Mean Reversion)

### 目標 (Goal)

建立 COPX 首個均值回歸實驗。參考 XBI-001 / SIVR-005 回檔 + Williams %R 架構，
按 COPX 波動度（~2.25%，GLD 比率 1.87x）縮放參數。

### 進場條件 (Entry Conditions)

| 條件 | 指標 | 閾值 | 說明 |
|------|------|------|------|
| 1 | 10 日高點回檔 | ≥ 9% | 過濾淺回檔 |
| 2 | 回檔上限 | ≤ 18% | 過濾極端崩盤（如 2020 COVID -35.9%）|
| 3 | Williams %R(10) | ≤ -80 | 超賣確認 |
| 4 | 冷卻期 | 10 天 | 避免重複進場 |

### 出場參數 (Exit Parameters)

| 參數 | 值 | 說明 |
|------|------|------|
| 獲利目標 (TP) | +3.5% | 均值回歸幅度 |
| 停損 (SL) | -5.0% | 非對稱寬停損（銅礦板塊需呼吸空間）|
| 最長持倉 | 15 天 | 高波動 → 更快回歸 |
| 追蹤停損 | 無 | 日波動 2.25%，禁用區域 |

### 成交模型 (Execution Model)

| 項目 | 設定 |
|------|------|
| 進場模式 | 隔日開盤市價 (next_open_market) |
| 止盈委託 | 限價賣單 Day (limit_order_day) |
| 停損委託 | 停損市價 GTC (stop_market_gtc) |
| 到期出場 | 隔日開盤市價 (next_open_market) |
| 滑價 | 0.15% |
| 悲觀認定 | 是（同根 K 線 stop+target 皆觸發 → 停損優先）|

### 設計理念 (Design Rationale)

- **回檔 9-18%**：9% 下限排除淺回檔低品質訊號（8% 的 Sharpe 為 0.00），18% 上限過濾 COVID 等極端崩盤
- **WR(10) ≤ -80**：標準超賣門檻，與 SIVR-005/XBI-001 一致
- **SL -5.0%**：非對稱設計（TP/SL = 0.7:1），需 WR > 59% 才能獲利
- **無追蹤停損**：日波動 2.25% 在禁用區域，根據跨資產教訓 #2

### 迭代嘗試紀錄 (Iteration Log)

| # | 變更 | Part A Sharpe | Part B Sharpe | A/B 訊號 | A WR / B WR | 結論 |
|---|------|-------------|-------------|----------|------------|------|
| 1 | 回檔 8-18%, WR-80, TP3.5/SL5 | 0.00 | 0.21 | 37/13 | 59.5%/69.2% | Part A 恰在盈虧平衡線 |
| 2 | 回檔 9-18%, WR-80, TP3.5/SL5 (最終版) | 0.08 | 0.36 | 30/10 | 63.3%/70.0% | 雙正、Part B 遠優於 A |
| — | 回檔 8-18%, WR-85, TP3.5/SL5 | ~0.07 | ~0.12 | 34/13 | 61.8%/69.2% | 不如 9% 門檻 |
| — | 回檔 8-18%, WR-80, drop2d≤-3%, TP3.5/SL5 | ~0.38 | ~0.07 | 32/9 | 71.9%/66.7% | Part A 過擬合 |
| — | 回檔 8-18%, WR-80, RSI(2)<15, TP3.5/SL5 | ~-0.06 | ~0.04 | 33/11 | 57.6%/63.6% | RSI(2) 反效果 |
| — | 回檔 8-18%, WR-80, drop2d≤-3%, TP4/SL5 | ~0.09 | ~-0.06 | 32/9 | 59.4%/55.6% | TP+4% 過高 |
| — | 回檔 10-16%, WR-80, ClosePos≥40%, TP4/SL4.5/15d | -0.14 | -0.15 | 19/7 | 47.4%/42.9% | 反轉K線+收窄範圍+緊SL三重過濾災難 |
| — | 回檔 9-18%, WR-80, drop2d≤-3%, TP3.5/SL5/20d | 0.15 | 0.24 | 27/7 | 66.7%/71.4% | Part A 提升但 Part B 訊號減少，A/B 更平衡但平均 Sharpe 略低 |
| — | RSI(2)<10, drop2d≤-4%, TP4.5/SL5/20d | -0.25 | 0.01 | 44/13 | 40.9%/53.8% | RSI(2) 產生過多低品質訊號，Part A 災難 |

### 回測結果 (Backtest Results)

| 指標 | Part A (2019-2023) | Part B (2024-2025) | Part C (2026-) |
|------|-------------------|-------------------|----------------|
| 訊號數 | 30 | 10 | 2 |
| 訊號/年 | 6.0 | 5.0 | 8.7 |
| 勝率 | 63.3% | 70.0% | 50.0% |
| 平均報酬 | +0.35% | +1.28% | -0.82% |
| 累計報酬 | +8.06% | +12.89% | -1.82% |
| 盈虧比 | 1.18 | 2.10 | 0.68 |
| Sharpe | 0.08 | 0.36 | -0.19 |
| Sortino | 0.11 | 0.55 | -0.23 |
| Calmar | 0.04 | 0.18 | -0.15 |
| MDD | -9.09% | -7.23% | -5.47% |
| 最大連續虧損 | 3 | 1 | 1 |

**A/B 分析**：
- 訊號率比 6.0:5.0 = 1.2:1（優秀，策略穩健）
- WR 從 63.3% 提升至 70.0%（Part B 更好，無過擬合）
- Sharpe 從 0.08 提升至 0.36（Part B 顯著更好）
- 最大連續虧損 Part A 3，Part B 僅 1（穩定）
- Part A 期間包含 2022 年銅價回落，多筆停損拉低績效
- Part B 盈虧比 2.10（7 勝 / 2 停損 / 1 到期）

**結論**：COPX 的回檔 + Williams %R 均值回歸策略有效。1% 的回檔門檻提高（8%→9%）移除 7 筆低品質 Part A 訊號，使 Sharpe 從 0.00 提升至 0.08。Part B Sharpe 0.36 顯著優於 Part A，無過擬合跡象。A/B 訊號比 1.2:1 表明策略對市場狀態變化穩健。

---

## COPX-002: 20日回檔 + Williams %R + 延長持倉 (20-Day Pullback + Williams %R + Extended Holding)

### 目標 (Goal)

改善 COPX-001 的 Part A 績效與 A/B 平衡。透過延長回看窗口（10日→20日），使回檔參考自更高的近期高點，
過濾短期震盪產生的淺回檔假訊號。搭配延長持倉（15天→20天）給予更充裕的反彈時間。

### 進場條件 (Entry Conditions)

| 條件 | 指標 | 閾值 | 說明 |
|------|------|------|------|
| 1 | 20 日高點回檔 | ≥ 10% | 從更高參考點過濾淺回檔 |
| 2 | 回檔上限 | ≤ 20% | 過濾極端崩盤 |
| 3 | Williams %R(10) | ≤ -80 | 超賣確認 |
| 4 | 冷卻期 | 12 天 | 避免重複進場 |

### 出場參數 (Exit Parameters)

| 參數 | 值 | 說明 |
|------|------|------|
| 獲利目標 (TP) | +3.5% | 同 COPX-001，已證明的 TP 上限 |
| 停損 (SL) | -5.0% | 同 COPX-001 |
| 最長持倉 | 20 天 | 延長 5 天，讓慢速反彈有時間達標 |
| 追蹤停損 | 無 | 日波動 2.25%，禁用區域 |

### 成交模型 (Execution Model)

| 項目 | 設定 |
|------|------|
| 進場模式 | 隔日開盤市價 (next_open_market) |
| 止盈委託 | 限價賣單 Day (limit_order_day) |
| 停損委託 | 停損市價 GTC (stop_market_gtc) |
| 到期出場 | 隔日開盤市價 (next_open_market) |
| 滑價 | 0.15% |
| 悲觀認定 | 是 |

### 設計理念 (Design Rationale)

- **20日回看 vs 10日**：20日高點提供更高的參考基準，同樣 10% 回檔門檻代表更深的真實回調，有效過濾 COPX-001 中短期震盪產生的假訊號
- **回檔 10-20%**：10% 門檻比 COPX-001 的 9% 稍高，配合更高的參考點效果更佳；20% 上限比 18% 稍寬以適應更廣的回看窗口
- **持倉 20 天 vs 15 天**：COPX-001 有 1 筆到期虧損（-4.73% on day 15），延長持倉可挽救部分慢速反彈交易
- **冷卻 12 天 vs 10 天**：略微增加以配合更長的回看窗口

### 迭代嘗試紀錄 (Iteration Log)

| # | 變更 | Part A Sharpe | Part B Sharpe | A/B 訊號 | A WR / B WR | 結論 |
|---|------|-------------|-------------|----------|------------|------|
| Att1 | 60日回撤≥15% + RSI(10)<30 + SMA50乖離≤-6%, TP5/SL7/20d | -0.21 | -0.18 | 14/4 | 50%/50% | FCX-001 架構不適合 COPX，WR 50% 加上 SL>TP 為負期望值 |
| Att2 | 20日回檔 10-20%, WR-80, TP3.5/SL5/20d (最終版) | 0.34 | 0.30 | 31/11 | 74.2%/72.7% | Part A Sharpe +325%，A/B 近乎完美平衡 |
| Att3 | 20日回檔 11-20%, WR-80, TP3.5/SL5/20d | 0.26 | 0.85 | 28/8 | 71.4%/87.5% | Part B 優秀但訊號僅 8 筆，A/B 比 3.5:1 過度不平衡 |

### 回測結果 (Backtest Results)

| 指標 | Part A (2019-2023) | Part B (2024-2025) | Part C (2026-) |
|------|-------------------|-------------------|----------------|
| 訊號數 | 31 | 11 | 2 |
| 訊號/年 | 6.2 | 5.5 | 8.4 |
| 勝率 | 74.2% | 72.7% | 50.0% |
| 平均報酬 | +1.27% | +1.14% | -0.82% |
| 累計報酬 | +44.64% | +12.40% | -1.82% |
| 盈虧比 | 1.96 | 1.82 | 0.68 |
| Sharpe | 0.34 | 0.30 | -0.19 |
| Sortino | 0.49 | 0.43 | -0.23 |
| Calmar | 0.14 | 0.16 | -0.15 |
| MDD | -8.91% | -7.23% | -5.47% |
| 最大連續虧損 | 3 | 2 | 1 |

**A/B 分析**：
- 訊號率比 6.2:5.5 = 1.13:1（優秀，近乎完美平衡）
- WR 差距僅 1.5pp（74.2% vs 72.7%），穩健無過擬合
- Sharpe 差距僅 0.04（0.34 vs 0.30），A/B 比 1.13x
- Part A 從 COPX-001 的 +8.06% 大幅提升至 +44.64%（+453%）
- Part B 維持穩定：+12.89% → +12.40%（僅 -3.8%）
- 關鍵改進：20日回看窗口新增多筆高品質勝出訊號，同時減少停損（10→8）

**vs COPX-001 改善摘要**：
| 指標 | COPX-001 A/B | COPX-002 A/B | 變化 |
|------|-------------|-------------|------|
| Sharpe | 0.08/0.36 | 0.34/0.30 | Part A +325%, Part B -17% |
| WR | 63.3%/70.0% | 74.2%/72.7% | Part A +10.9pp, Part B +2.7pp |
| Cumulative | +8.06%/+12.89% | +44.64%/+12.40% | Part A +453%, Part B -3.8% |
| PF | 1.18/2.10 | 1.96/1.82 | Part A +66%, Part B -13% |
| A/B Sharpe比 | 0.22x | 1.13x | 大幅改善 |

---

## COPX-003: 出場優化（SL -4.5%）(Exit Optimization)

### 目標 (Goal)

改善 COPX-002 的風險調整後報酬。COPX-002 所有停損交易均觸及 -5.0% 完整停損，
測試收窄至 -4.5% 是否為純粹改善（減少每筆虧損而不影響勝率）。

### 進場條件 (Entry Conditions)

同 COPX-002（20日高點回檔 10-20% + WR(10) ≤ -80 + 冷卻 12 天）。

### 出場參數 (Exit Parameters)

| 參數 | COPX-002 | COPX-003 | 說明 |
|------|----------|----------|------|
| TP | +3.5% | +3.5% | 維持 |
| SL | -5.0% | **-4.5%** | 收窄 0.5%，甜蜜點 |
| 持倉 | 20 天 | 20 天 | 維持 |

### 成交模型 (Execution Model)

同 COPX-002（隔日開盤市價進場、限價 Day 止盈、停損市價 GTC、悲觀認定、滑價 0.15%）。

### 設計理念 (Design Rationale)

- **SL -4.5% vs -5.0%**：COPX-002 所有 8 筆 Part A 停損和 3 筆 Part B 停損均觸及完整 -5.0% 停損。收窄至 -4.5% 每筆虧損減少 0.5%，8 筆 × 0.5% = +4.0% Part A 改善
- **SL -4.0% 太緊**：已測試（Att3），Part B 2 筆好交易被提前停損，WR 72.7%→54.5%，Sharpe 崩至 0.01
- **TP +4.0% 不可行**：已測試（Att1），Part A 2 筆達標交易無法觸及 +4.0%，Sharpe 下降 26%

### 迭代嘗試紀錄 (Iteration Log)

| # | 變更 | Part A Sharpe | Part B Sharpe | A/B 訊號 | A WR / B WR | 結論 |
|---|------|-------------|-------------|----------|------------|------|
| Att1 | TP +4.0% / SL -5.0% / 20d | 0.25 | 0.34 | 31/11 | 67.7%/72.7% | TP +4.0% 使 Part A 2 筆從達標→到期/停損，Part A Sharpe 退化 26% |
| **Att2** | **TP +3.5% / SL -4.5% / 20d** | **0.39** | **0.35** | **31/11** | **74.2%/72.7%** | **最佳：WR 不變，每筆虧損減少 0.5%，純粹改善** |
| Att3 | TP +3.5% / SL -4.0% / 20d | 0.37 | 0.01 | 31/11 | 71.0%/54.5% | SL -4.0% 太緊，Part B 2 筆好交易被提前停損 |

### 回測結果 (Backtest Results)

| 指標 | Part A (2019-2023) | Part B (2024-2025) | Part C (2026-) |
|------|-------------------|-------------------|----------------|
| 訊號數 | 31 | 11 | 2 |
| 訊號/年 | 6.2 | 5.5 | 8.4 |
| 勝率 | 74.2% | 72.7% | 50.0% |
| 平均報酬 | +1.40% | +1.28% | -0.57% |
| 累計報酬 | +50.85% | +14.19% | -1.30% |
| 盈虧比 | 2.17 | 2.01 | 0.75 |
| Sharpe | 0.39 | 0.35 | -0.14 |
| Sortino | 0.59 | 0.53 | -0.17 |
| Calmar | 0.16 | 0.18 | -0.10 |
| MDD | -8.91% | -7.23% | -5.47% |
| 最大連續虧損 | 3 | 2 | 1 |
| 達標/停損/到期 | 23/8/0 | 8/3/0 | 1/1/0 |

**A/B 分析**：
- 訊號率比 6.2:5.5 = 1.13:1（同 COPX-002，優秀）
- WR 與 COPX-002 完全一致（74.2%/72.7%），SL 收窄未翻轉任何交易
- Sharpe 差距僅 0.04（0.39 vs 0.35），A/B 比 1.11x（優於 COPX-002 的 1.13x）
- Part A 從 COPX-002 的 +44.64% 提升至 +50.85%（+14%）
- Part B 從 COPX-002 的 +12.40% 提升至 +14.19%（+14%）

**vs COPX-002 改善摘要**：
| 指標 | COPX-002 A/B | COPX-003 A/B | 變化 |
|------|-------------|-------------|------|
| Sharpe | 0.34/0.30 | 0.39/0.35 | +15%/+17% |
| WR | 74.2%/72.7% | 74.2%/72.7% | 不變 |
| Cumulative | +44.64%/+12.40% | +50.85%/+14.19% | +14%/+14% |
| PF | 1.96/1.82 | 2.17/2.01 | +11%/+10% |
| A/B Sharpe比 | 1.13x | 1.11x | 改善 |

**結論**：SL -4.5% 是 COPX 的甜蜜點，類似 URA-002 從 -6.0% 收窄至 -5.5% 的純粹改善。WR 完全不受影響，每筆停損虧損減少 0.5%，所有風險調整指標均提升。SL -4.0% 太緊會翻轉 Part B 好交易。

---

## 演進路線圖 (Roadmap)

```
COPX-001 (10日回檔 9-18% + WR(10) ≤ -80, 15天)
  ├── ✗ 收盤位置 ≥ 40% 反轉K線過濾（已實測：雙部分皆負）
  ├── ✗ 急跌確認 drop2d ≤ -3%（已實測：Part A 提升但 Part B 下降）
  ├── ✗ RSI(2) < 10 全新架構（已實測：產生過多低品質訊號）
  └── COPX-002 (20日回檔 10-20% + WR(10) ≤ -80, 20天)
        ├── ✗ 60日回撤 + RSI(10) + SMA乖離 三重條件（Att1：Sharpe -0.21/-0.18）
        ├── ✗ 回檔 11%（Att3：A/B 嚴重不平衡 3.5:1）
        └── COPX-003 (SL -4.5% 出場優化) ← 當前最佳
              ├── ✗ TP +4.0%（Att1：Part A 退化 26%）
              ├── ✗ SL -4.0%（Att3：Part B WR 崩至 54.5%）
              └── COPX-004? (銅價/工業金屬宏觀過濾器)
```

---

## COPX-001 滾動窗口績效分析

> **分析日期：** 2026-03-30
> **窗口：** 2 年，步進 6 個月（共 12 個窗口）

### 滾動窗口績效表

| 窗口 | 訊號 | 勝率 | 平均報酬 | 累計報酬 | MDD | ΔWR |
|------|------|------|----------|----------|-----|-----|
| 2019-01~2020-12 | 7 | 57.1% | -0.20% | -2.05% | -8.91% | — |
| 2019-07~2021-06 | 10 | 60.0% | +0.04% | -0.47% | -8.91% | +2.9pp |
| 2020-01~2021-12 | 13 | 69.2% | +0.87% | +10.83% | -8.91% | +9.2pp |
| 2020-07~2022-06 | 16 | 62.5% | +0.60% | +8.65% | -7.08% | -6.7pp |
| 2021-01~2022-12 | 18 | 61.1% | +0.16% | +1.34% | -9.09% | -1.4pp |
| 2021-07~2023-06 | 16 | 62.5% | +0.29% | +3.22% | -9.09% | +1.4pp |
| 2022-01~2023-12 | 14 | 64.3% | +0.41% | +4.68% | -9.09% | +1.8pp |
| 2022-07~2024-06 | 9 | 66.7% | +0.62% | +4.93% | -8.14% | +2.4pp |
| 2023-01~2024-12 | 11 | 72.7% | +1.31% | +14.62% | -6.33% | +6.1pp |
| 2023-07~2025-06 | 10 | 70.0% | +1.28% | +12.89% | -7.23% | -2.7pp |
| 2024-01~2025-12 | 10 | 70.0% | +1.28% | +12.89% | -7.23% | +0.0pp |
| 2024-07~2026-03 | 11 | 63.6% | +0.70% | +7.09% | -7.23% | -6.4pp |

### 預測精準度表

| 窗口 | WR | 平均贏利 | 平均虧損 | 盈虧比 | 差點成功 |
|------|------|----------|----------|--------|----------|
| 2019-01~2020-12 | 57.1% | +3.50% | -5.14% | 0.91 | — |
| 2019-07~2021-06 | 60.0% | +3.50% | -5.14% | 1.02 | — |
| 2020-01~2021-12 | 69.2% | +3.50% | -5.04% | 1.56 | 0/1 |
| 2020-07~2022-06 | 62.5% | +3.50% | -4.24% | 1.38 | 0/2 |
| 2021-01~2022-12 | 61.1% | +3.50% | -5.08% | 1.08 | 0/1 |
| 2021-07~2023-06 | 62.5% | +3.50% | -5.07% | 1.15 | 0/1 |
| 2022-01~2023-12 | 64.3% | +3.50% | -5.14% | 1.23 | — |
| 2022-07~2024-06 | 66.7% | +3.50% | -5.14% | 1.36 | — |
| 2023-01~2024-12 | 72.7% | +3.50% | -4.52% | 2.07 | 0/1 |
| 2023-07~2025-06 | 70.0% | +3.50% | -3.89% | 2.10 | 0/1 |
| 2024-01~2025-12 | 70.0% | +3.50% | -3.89% | 2.10 | 0/1 |
| 2024-07~2026-03 | 63.6% | +3.50% | -4.20% | 1.46 | 0/1 |

### 漸變性評估

- **勝率範圍**：57.1% ~ 72.7%（ΔWR 標準差 4.6pp，最大跳動 9.2pp）
- **盈虧比範圍**：0.91 ~ 2.10（ΔPF 標準差 0.35）
- **累計報酬範圍**：-2.05% ~ +14.62%（ΔCum 標準差 5.37%）
- **平均贏利範圍**：+3.50% ~ +3.50%（Δ標準差 0.00%，完全穩定）
- **平均虧損範圍**：-3.89% ~ -5.14%（虧損幅度隨市場波動略有變化）

**判定：**
- ✓ 預測精準度漸變（勝率最大跳動 9.2pp ≤ 20pp 閾值）
- ✓ 下游績效漸變（累計報酬最大跳動 11.30% ≤ 3σ = 16.11%）

### 分析解讀

1. **早期窗口偏弱**：2019-01~2020-12 窗口累計 -2.05%，唯一負報酬窗口，受 2020 疫情影響銅價大幅震盪
2. **中期穩定**：2021-2023 窗口勝率穩定在 61-66%，表現平穩但累計報酬偏低（+1.3%~+4.9%）
3. **近期表現強勁**：2023 後窗口勝率攀升至 70-73%，累計報酬達 +12.9%~+14.6%
4. **平均贏利完全固定**：+3.50% TP 在所有窗口一致觸發，策略進場精準度高
5. **漸變性優異**：ΔWR 最大僅 9.2pp，遠低於 20pp 閾值，勝率變化極為平滑
6. **10/12 正報酬窗口**：僅最早兩個窗口為負或接近零，整體穩健
