...
**當前最佳（依場景）：**
- **Part B 績效最佳：** COPX-001（回檔 9-18% + WR(10) ≤ -80，Part A Sharpe 0.08，Part B Sharpe 0.36）
- **A/B 平衡最佳：** COPX-002（回檔 10-18% + WR(10) ≤ -80，Part A Sharpe 0.21，Part B Sharpe 0.14，Sharpe 差距僅 0.07）
...
**已掃描的參數空間：**
- 進場條件：回檔 8~10% + 上限 16~18% + WR(10) ≤ -80~-85 + RSI(2) < 10~15 + 2日跌幅 3~4% + ClosePos ≥ 40% + 60日回撤 15% + RSI(10) < 30 + SMA(50) 乖離 -6%
- 出場參數：TP +3.5~4.5% / SL -4.5~-5.0% / 持倉 15~20 天
- 最佳組合（Part B 優先）：回檔 9-18% + WR-80 + TP +3.5% / SL -5.0% / 15天（WR 63.3%/70.0%）
- 最佳組合（A/B 平衡）：回檔 10-18% + WR-80 + TP +3.5% / SL -5.0% / 15天（WR 69.2%/60.0%，Sharpe gap 0.07）
- 2日跌幅 ≤ -3% + 回檔9-18% + 20天持倉：Part A 提升至 Sharpe 0.15 但 Part B 降至 0.24（A/B 更平衡但平均略差）
- 2日跌幅 ≤ -3% + 回檔9-18% + 15天持倉：Part A 0.15 但 Part B 崩至 0.09（2d-drop 在 15天持倉下更嚴重傷害 Part B）
...
- 回檔下限 9%→10% 大幅改善 Part A（Sharpe 0.08→0.21）但損失 Part B 部分 WR（70%→60%）
- COPX 參數空間已被充分探索，入場/出場參數已接近邊界
<!-- AI_CONTEXT_END -->
