...
**當前最佳：** COPX-003（20日回檔 10-20% + WR(10) ≤ -80 + SL -4.5%，Part A Sharpe 0.39，Part B Sharpe 0.35，已確認為全域最優）
...
- 冷卻期：10~12 天
- 全域最佳：20日回檔 10-20% + WR-80 + TP +3.5% / SL -4.5% / 20天 / 冷卻12天（Sharpe 0.39/0.35，WR 74.2%/72.7%）

**尚未嘗試的方向（可探索但預期邊際效益極低）：**
- 銅價或工業金屬指數作為宏觀過濾器（但跨資產教訓 #6 建議謹慎：確認指標常不提升品質）
- 趨勢跟蹤（SMA 交叉/Donchian 通道）：BB 突破已失敗，更簡單的趨勢策略預期同樣失敗

**關鍵資產特性：**
- COPX (Global X Copper Miners ETF) 日波動約 2.25%，GLD 比率 1.87x
- 銅礦 ETF，受銅價、中國需求、全球工業景氣驅動
- 均值回歸效果穩健，20日回看框架 + SL 收窄使 Part A/B 同時大幅提升
- A/B 訊號率比 1.13:1（優秀），A/B Sharpe 差距僅 0.04（極佳平衡）
- 20日回看 vs 10日回看是關鍵突破：更長回看窗口捕捉更有意義的回檔
- **突破策略無效**：COPX 日波動 2.25% 處於突破有效範圍下邊界（FCX 2-4%、TSLA 3-4%、NVDA 3.26% 均成功），ETF 分散化可能削弱個股動量
- **配對交易無效**：COPX/FCX 比率有結構性漂移（FCX 在銅牛市中因純銅礦暴露而跑贏 ETF），z-score 均值回歸不可靠
- **動量回檔無效**：SMA(50) 趨勢濾波無法區分正常回檔 vs 趨勢反轉，Part A WR 僅 40%
- **RSI(2) 進場品質低於 WR(10)+回檔**：RSI(2)<15+2日跌幅 產生 9.4 訊號/年（vs COPX-003 的 6.2/年），WR 57.4%（vs 74.2%），Sharpe 0.01（vs 0.39）
- COPX 均值回歸、突破、配對交易、動量回檔、RSI(2) 均已驗證，**已確認為全域最優**（6 次實驗、30+ 次嘗試，含均值回歸、突破、配對交易、動量回檔四大策略類型）
<!-- AI_CONTEXT_END -->
...
| COPX-003 | `copx_003_exit_optimized` | 20日回檔 10-20% + WR + 出場優化（全域最佳）| ✅ 當前最佳 |
...
COPX-003（Sharpe 0.39/0.35）確認為全域最優，已嘗試 6 次實驗、30+ 次嘗試，涵蓋均值回歸、突破、配對交易、動量回檔四大策略類型。
