"""
CIBR 20日回看窗口均值回歸訊號偵測器

結構性變更：使用 20日回看窗口（vs 標準 10日），搭配 WR(14) 取代 WR(10)。
保留 ATR(5)/ATR(20) > 1.15 和 ClosePos >= 40% 過濾。
"""
...
from trading.experiments.cibr_005_20d_lookback_mr.config import CIBR20DLookbackMRConfig

logger = logging.getLogger(__name__)


class CIBR20DLookbackMRSignalDetector(BaseSignalDetector):
    """
    CIBR 20日回看窗口均值回歸訊號偵測器

    四條件同時成立時觸發訊號：
    1. 收盤價相對 20日最高價回檔 >= 6%
    2. Williams %R(14) <= -80（超賣）
    3. ClosePos >= 40%（日內反轉確認）
    4. ATR(5)/ATR(20) > 1.15（波動率急升）
    """

    def __init__(self, config: CIBR20DLookbackMRConfig):
        self.config = config
    ...
    def detect_signals(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        logger.info("CIBR: Detected %d 20D-Lookback Pullback+WR signals", signal_count)
        return df
