"""
IBIT-001: 回檔 + Williams %R 均值回歸策略
(IBIT Pullback + Williams %R Mean Reversion Strategy)
...
from trading.experiments.ibit_001_pullback_wr.signal_detector import (
    IBITPullbackWRSignalDetector,
)
...
class IBITPullbackWRStrategy(ExecutionModelStrategy):
    ...
    def create_detector(self) -> BaseSignalDetector:
        return IBITPullbackWRSignalDetector(create_default_config())

    def _print_strategy_params(self, config: ExperimentConfig) -> None:
        if isinstance(config, IBITPullbackWRConfig):
            print(
