"""
EWJ-003: BB Lower Band Mean Reversion Strategy

使用 BB(20,1.5) 下軌觸及 + 10日高點回檔上限 7% 作為混合進場訊號，搭配 WR+ClosePos+ATR 品質過濾。
...
from trading.experiments.ewj_003_bb_lower_mr.signal_detector import (
    EWJ003SignalDetector,
)
...
class EWJ003Strategy(ExecutionModelStrategy):
    ...
    def create_detector(self) -> BaseSignalDetector:
        return EWJ003SignalDetector(create_default_config())

    def _print_strategy_params(self, config: ExperimentConfig) -> None:
        if isinstance(config, EWJ003Config):
