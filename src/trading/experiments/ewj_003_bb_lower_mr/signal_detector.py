"""
EWJ-003 BB 下軌均值回歸訊號偵測器

在 BB(20,2.0) 下軌觸及時買入，搭配 WR+ClosePos+ATR 三重品質過濾。
相較於 EWJ-002 的固定 3-7% 回檔門檻，BB 下軌是統計自適應門檻。
"""
...
from trading.experiments.ewj_003_bb_lower_mr.config import EWJ003Config

logger = logging.getLogger(__name__)


class EWJ003SignalDetector(BaseSignalDetector):
    """
    EWJ BB 下軌均值回歸訊號偵測器
    ...
    def __init__(self, config: EWJ003Config):
        self.config = config
    ...
    def detect_signals(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        if suppressed:
            ...
            logger.info("EWJ-003: %d signals suppressed by cooldown", len(suppressed))

        signal_count = df["Signal"].sum()
        logger.info("EWJ-003: Detected %d BB Lower Band MR signals", signal_count)
        return df
